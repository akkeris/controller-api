...
async function http_update(pg_pool, req, res, regex) {
  ...
  try {
    await update_action(pg_pool, [action.action, payload.name, payload.description, payload.events]);
  } catch (err) {
...
async function http_runs_create(pg_pool, req, res, regex) {
  ...
  const created_by = req.headers['x-username'] || 'unknown';
  const action_run = await common.trigger_action(pg_pool, app_key, action_key, created_by);
