...
async function http_list_all(pg_pool, req, res, regex) {
  ...
  attachments = attachments.map((attachment) => {
    let addon = addon_services.service_by_id_or_name(attachment.addon);
...
async function http_update(pg_pool, req, res, regex) {
  ...
  let addon_service = addon_services.service_by_id_or_name(attachments[0].addon);
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, addon, user) {
  ...
  await addons.addon_exists(pg_pool, attachments[0].service, attachments[0].app);
  ...
  try {
    ...
    let service_attachment = await addon_service.attach(pg_pool, target_app, Object.assign(plan, {"primary":!existing_installation}), {created:new Date(), updated:new Date(), service:attachments[0].service, foreign_key:attachments[0].foreign_key, config_vars:{}}, false)
...
async function http_create(pg_pool, req, res, regex) {
  ...
  return httph.created_response(res, JSON.stringify(await create(pg_pool, app.app_uuid, app.app_name, app.space_name, '', app.org_name, payload.addon, req.headers['x-username'])))
}
