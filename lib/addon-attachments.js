...
async function http_update(pg_pool, req, res, regex) {
  ...
  if(payload.name && payload.name !== '' && payload.name.toLowerCase() !== attachments[0].name.toLowerCase()) {
    await update_service_attachment_name(pg_pool, [app.app_uuid, addon_attachment_id, payload.name.toLowerCase()])
    redeploy = true
  }
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, addon, user, addon_attachment_name) {
  ...
  if(addon_attachment_name) {
    if(!(/^[a-zA-Z][A-Za-z0-9_-]+$/).test(addon_attachment_name)) {
      throw new common.UnprocessibleEntityError("This addon's attachment name is invalid.")
    }
  }
  ...
  try {
    ...
    let service_attachment = await addon_service.attach(pg_pool, target_app, addon_plan, service, false, addon_attachment_name)
