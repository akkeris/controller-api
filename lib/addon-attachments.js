...
const select_all_service_attachments = query.bind(query, fs.readFileSync('./sql/select_all_service_attachments.sql').toString('utf8'), (r) => r);
...
async function http_list_all(pg_pool, req, res /* regex */) {
  ...
  attachments = attachments.map((attachment) => {
    const addon = common.service_by_id_or_name(attachment.addon);
    const plan = common.plan_by_id_or_name(attachment.plan);
...
async function list_by_app(pg_pool, app_uuid) {
  const attachments = await select_service_attachments(pg_pool, [app_uuid]);
...
async function http_get(pg_pool, req, res, regex) {
  const app_key = httph.first_match(req.url, regex);
  const addon_attachment_id = httph.second_match(req.url, regex);
  const app = await common.app_exists(pg_pool, app_key);
...
async function http_get_by_id(pg_pool, req, res, regex) {
  const addon_attachment_id = httph.first_match(req.url, regex);
...
async function http_update(pg_pool, req, res, regex) {
  ...
  const attachments = await select_service_attachment(pg_pool, [app.app_uuid, addon_attachment_id]);
  if (attachments.length === 0) {
    throw new common.NotFoundError(`The specified addon attachments ${addon_attachment_id} could not be found.`);
  }
  if (attachments.length !== 1) {
  ...
  const addon_service = common.service_by_id_or_name(attachments[0].addon);
  const plan = common.plan_by_id_or_name(attachments[0].plan);
  const addon = Object.assign(attachments[0], { addon_service, plan });
  const attachment = transform_attachment(addon_service, plan, attachments[0]);
  ...
  if (redeploy) {
    setTimeout(() => {
      ...
      lifecycle
        .restart_and_redeploy_app(pg_pool, app.app_uuid, app.app_name, app.space_name, app.org_name, `Updated ${attachment.name}`)
        .catch(() => { /* do nothing */ });
    }, 10);
  }
...
async function http_delete(pg_pool, req, res, regex) {
  const addon_attachment_id = httph.first_match(req.url, regex);
  const attachments = await select_service_attachment(pg_pool, [addon_attachment_id, null]);
  if (attachments.length === 0) {
    throw new common.NotFoundError(`The specified addon attachments ${addon_attachment_id} could not be found.`);
  }
  if (attachments.length !== 1) {
  ...
  const target_app = {
    ...
    app: attachments[0].app,
    name: attachments[0].app_name,
    space: attachments[0].space,
    org: attachments[0].org,
  };
  const addon = Object.assign(attachments[0], { addon_service, plan });
  const service_attachment = await addon_service.detach(
    pg_pool, target_app, Object.assign(plan, { primary: attachments[0].primary }), addon,
  );
  setTimeout(() => {
    ...
    lifecycle
      .restart_and_redeploy_app(
        pg_pool,
        attachments[0].app,
        attachments[0].app_name,
        attachments[0].space,
        attachments[0].org,
        `Detached ${service_attachment.name}`,
      )
      .catch(() => { /* do nothing */ });
  }, 10);
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, addon, user, addon_attachment_name) {
  ...
  const attachments = await select_service_attachment_owner(pg_pool, [addon]);
  if (attachments.length === 0) {
  ...
  const addon_service = common.service_by_id_or_name(attachments[0].addon);
  const source_app = await common.app_exists(pg_pool, attachments[0].app);
  const target_app = await common.app_exists(pg_pool, app_uuid);
  ...
  const owner_region = await common.alamo.region_name_by_space(pg_pool, attachments[0].space);
  const target_region = await common.alamo.region_name_by_space(pg_pool, space_name);
  ...
  const existing_addons = (await select_addons(pg_pool, [app_uuid]))
    .concat(await select_service_attachments(pg_pool, [app_uuid]));
  ...
  const addon_plan = Object.assign(plan, { primary: !existing_installation });
  const service = {
    created: new Date(),
    updated: new Date(),
    service: attachments[0].service,
    foreign_key: attachments[0].foreign_key,
    config_vars: {},
  };
  ...
  try {
    ...
    if (e instanceof common.NoFormationsFoundError) {
      ...
      service_attachment = await addon_service.attach(
        pg_pool, target_app, addon_plan, service, false, addon_attachment_name,
      );
    } else {
      throw e;
    }
  }
  await update_app_updated_at(pg_pool, [target_app.app_uuid]);
  ...
  setTimeout(() => {
    ...
    lifecycle
      .restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org, `Attached ${service_attachment.name}`)
      .catch(() => { /* do nothing */ });
  }, 10);
...
async function http_create(pg_pool, req, res /* regex */) {
  ...
  return httph.created_response(res, JSON.stringify(
    await create(
      pg_pool, app.app_uuid, app.app_name, app.space_name, '', app.org_name, payload.addon, req.headers['x-username'], payload.name,
    ),
  ));
}
