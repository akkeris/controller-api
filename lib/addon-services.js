...
async function addons_get(pg_pool, req, res, regex) {
  ...
  let attached_apps = await select_service_attached_apps(pg_pool, [service_id]);
  let attached_apps_parsed = [];
  let results = await Promise.all(attached_apps.map(async (app) => {
        attached_apps_parsed.push({
          'id':app.app,
  ...
  transformed_service.attached_to = attached_apps_parsed;
