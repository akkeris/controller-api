...
function add_service(service, pg_pool) {
  ...
  if(typeof service === 'function') {
    ...
    try {
      ...
      console.log(`Error: the service ${found_service.id} was already registered.`)
    } catch (e) {
      ...
      if(global.plans.filter(plan => plans.includes(plan)).length > 0) {
        console.log(`Error: the service ${service.info().id} ${service.info().name} includes one or more plans with an id that already exists ${plans.join(',')}`)
        return
      }
      global.plans = global.plans.concat(plans)
...
function refresh_services(pg_pool) {
  ...
  try {
    global.services = [];
    global.plans = [];
    add_service(require('./addons/osb-addons.js'), pg_pool);
...
function service_by_id_or_name(addon_id_or_name) {
  ...
  if(addons_info.length !== 1) {
    throw new common.NotFoundError(`The specified addon ${addon_id_or_name} could not be found.`);
  }
