...
async function addons_create(pg_pool, app_uuid, app_name, space_name, space_tags, org, plan, req) {
  let addon = null;
  try {
...
async function http_create(pg_pool, req, res, regex) {
  ...
  httph.created_response(res, 
    JSON.stringify(await addons_create(pg_pool, app.app_uuid, app.app_name, app.space_name, space.tags, app.org_name, payload.plan, req)));
}
...
async function addons_delete_by_app(pg_pool, app_uuid, app_name, space_name, org) {
  let services = await select_services(pg_pool, [app_uuid]);
  let app = { id:app_uuid, name:app_name, space:space_name, org:org };
