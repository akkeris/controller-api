...
filtered_plans[0].attached_to = apps;
...
async function addons_get(pg_pool, req, res, regex) {
  ...
  let attached_apps = [];
  await Promise.all(results.map(async (app) => {
        attached_apps.push({
          'id':app.app,
          'name':`${app.name}-${app.space}`,
          'owned': app.owned
