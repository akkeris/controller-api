...
async function plans_get(pg_pool, req, res, regex) {
  ...
  let filtered_plans = plans.filter((plan) => { return plan.id === plan_id_or_name || plan.name === plan_id_or_name || plan.key === plan_id_or_name });
...
async function service_exists(pg_pool, service_id, app_uuid) {
  ...
  assert.ok(addon, "The specified addon does not exist.");
...
async function addons_create(pg_pool, app_uuid, app_name, space_name, space_tags, org, plan, user) {
  ...
  try {
    ...
    assert.ok(addon, 'The specified addon could not be found.');
...
async function service_config_vars(pg_pool, app_uuid, space_name, app_name) {
  ...
  for(let i=0; i < services.length; i++) {
    ...
    try {
      ...
      console.error('Error getting service config vars:')
      console.error(e)
  ...
  for(let i=0; i < service_attachments.length; i++) {
    ...
    try {
      ...
      console.error('Error getting attached service config vars:')
      console.error(e)
      // could happen if a addon is removed.
    }
  }
