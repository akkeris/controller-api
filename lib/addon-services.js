...
function plan_by_id_or_name(plan_id_or_name) {
  ...
  console.assert(found_plans.length === 1, 'The specified plan could not be found.');
...
async function addons_create(pg_pool, app_uuid, app_name, space_name, space_tags, org, plan) {
  let addon = null;
  try {
...
async function http_create(pg_pool, req, res, regex) {
  ...
  httph.created_response(res, 
    JSON.stringify(await addons_create(pg_pool, app.app_uuid, app.app_name, app.space_name, space.tags, app.org_name, payload.plan)));
}
...
async function addons_delete(pg_pool, req, res, regex) {
  ...
  if(service.service.attachments > 1) {
    throw new common.UnprocessibleEntityError('This addon is attached to multiple apps, remove the attachments from other apps and try again, or detach this addon from this app.')
  }
  let app_obj = {id:app.app_uuid, name:app.app_name, space:app.space_name, org:app.org_name}
...
async function addons_delete_by_app(pg_pool, app_uuid, app_name, space_name, org, cb) {
  let services = await select_services(pg_pool, [app_uuid]);
  let app = { id:app_uuid, name:app_name, space:space_name, org:org };
...
module.exports = {
  ...
  addon_by_id_or_name:addon_by_id_or_name,
  plan_by_id_or_name:plan_by_id_or_name,
  addons:{
    list:addons_list,
    get:addons_get,
    http:{
