...
function transform_addon(app, addon_definition, addon_plan, service) {
  let base_web_url = config.appkit_ui_url ? config.appkit_ui_url : ""
  return {
...
async function addons_create(pg_pool, app_uuid, app_name, space_name, space_tags, org, plan, user, attachment_name) {
  ...
  if(attachment_name) {
    if(!(/^[a-zA-Z][A-Za-z0-9_-]+$/).test(attachment_name)) {
      throw new common.UnprocessibleEntityError("This addon's attachment name is invalid.")
    }
  }
...
async function addons_list(pg_pool, app_uuid, app_name, space_name, org_name) {
  ...
  return (await Promise.all(addons.map(async (addon) => {
    try {
      ...
      addon.state = await addon_service.get_state(plan.id, addon.service, app_uuid)
...
async function http_update(pg_pool, req, res, regex) {
  ...
  if(payload.attachment && payload.attachment.name) {
    if(!(/^[a-zA-Z][A-Za-z0-9_-]+$/).test(payload.attachment.name)) {
      throw new common.UnprocessibleEntityError("This addon's attachment name is invalid.")
    }
    await update_service_attachment_name(pg_pool, [app.app_uuid, addon_id, payload.attachment.name.toLowerCase()])
    redeploy = true
  }
