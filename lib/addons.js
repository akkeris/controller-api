...
async function addons_create(pg_pool, app_uuid, app_name, space_name, space_tags, org, plan, user) {
  ...
  if(proposed_service.supports_multiple_installations === false && 
    addons.filter((s) => { return plan.addon_service.id === s.addon }).length > 0)
  {
  ...
  let service = null;
  try {
    service = await addon_service.provision(pg_pool, app, plan);
  } catch (err) {
  ...
  setTimeout(() => {
    common.notify_hooks(pg_pool, app_uuid, 'addon_change', JSON.stringify({
      ...
      'change':'create',
      'changes':[
        addon_services.transform_service(app, addon_service.info(), plan, service)
      ]
    }), user ? user : "System");
  ...
  return addon_services.transform_service(app, addon_service.info(), plan, service);
}
...
async function addons_list(pg_pool, app_uuid, app_name, space_name, org_name) {
  ...
  return addons.map((addon) => {
    try {
      ...
      assert.ok(addon_service, 'The specified addon does not exist.');
      assert.ok(plan, 'The specified addon plan does not exist.');
      return addon_services.transform_service(app, addon_service, plan, addon);
    } catch (e) {
...
async function addons_get(pg_pool, req, res, regex) {
  ...
  let service_config_vars = await addon.addon_service.config_vars(pg_pool, addon, app.space_name, app.app_name);
  addon.config_vars = {};
  for(let key in service_config_vars) {
    if(key !== 'spec') {
      addon.config_vars[key] = service_config_vars[key];
    }
  }
  ...
  let transformed_service = addon_services.transform_service(app_obj, addon.addon_service.info(), addon.plan, addon);
...
async function addons_delete(pg_pool, req, res, regex) {
  ...
  let app_obj = {id:app.app_uuid, name:app.app_name, space:app.space_name, org:app.org_name}
  let result = await addon.addon_service.unprovision(pg_pool, app_obj, addon.plan, addon)
  setTimeout(() => {
    common.notify_hooks(pg_pool, app.app_uuid, 'addon_change', JSON.stringify({
      ...
      'change':'delete',
      'changes':[
        addon_services.transform_service(app_obj, addon.addon_service.info(), addon.plan, addon)
      ]
  ...
  return httph.ok_response(res, JSON.stringify(addon_services.transform_service(app_obj, addon.addon_service.info(), addon.plan, addon)));
}
...
async function addons_delete_by_app(pg_pool, app_uuid, app_name, space_name, org, user) {
  ...
  await Promise.all(addon_attachments.map(async (addon) => {
    ...
    let plan =  addon_services.plan_by_id_or_name(addon.plan);
    detached_addons.push(addon_services.transform_service(app, addon_service.info(), plan, addon));
    return await addon_service.detach(pg_pool, app, plan, addon)
  }))
  ...
  let results = await Promise.all(addons.map(async (addon) => {
    ...
    if(addon.owned) {
      deleted_addons.push(addon_services.transform_service(app, addon_service.info(), plan, addon));
      return await addon_service.unprovision(pg_pool, app, plan, addon);
    } else {
...
async function service_config_vars(pg_pool, app_uuid, space_name, app_name) {
  ...
  for(let i=0; i < addons.length; i++) {
    ...
    try {
      let addon_service = addon_services.service_by_id_or_name(addon.addon);
      let data = await addon_service.config_vars(pg_pool, addon, space_name, app_name);
