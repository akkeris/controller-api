...
const select_all_addons = query.bind(query, fs.readFileSync('./sql/select_all_services.sql').toString('utf8'), (r) => { return r; });
...
async function addons_list_all(pg_pool) {
  let addons = await select_all_addons(pg_pool, []);
  return (await Promise.all(addons.map(async (addon) => {
    try {
      let app = {id:addon.app, name:addon.app_name, space:addon.space_name, org:addon.org_name};
      let addon_service = common.service_by_id_or_name(addon.addon);
      let plan = common.plan_by_id_or_name(addon.plan);
      assert.ok(addon_service, 'The specified addon does not exist.');
      assert.ok(plan, 'The specified addon plan does not exist.');

      let addon_state = await addon_service.get_state(plan.id, addon.service, app.id)
      addon.state = addon_state.state
      addon.state_description = addon_state.description
      return transform_addon(app, addon_service.info(), plan, addon);
    } catch (e) {
...
async function http_get_by_id(pg_pool, req, res, regex) {
  ...
  let attachment_info = await select_service_attachment_owner(pg_pool, [addon_id]);
  try {
    assert.ok(attachment_info.length === 1, 'The specified addon could not be found.');
  } catch (e) {
    throw new common.NotFoundError(e.message);
  }
  let app = await common.app_exists(pg_pool, attachment_info[0].app_uuid);
