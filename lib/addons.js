...
function transform_addon(app, addon_definition, addon_plan, service) {
  ...
  return {
    ...
    id: service.service,
    name: service.name,
    service_attachment: service.service_attachment,
    plan: {
      id: addon_plan.id,
...
async function http_update(pg_pool, req, res, regex) {
  ...
  if (payload.plan && payload.plan !== addon.plan.id) {
    ...
    if (new_plan.id !== addon.plan.id) {
      let service = await addon.addon_service.update(pg_pool, app, new_plan, addon);
      if (service.state === 'provisioning') {
        await tasks.add(pg_pool, [service.service, 'resync-addon-state', app.app_uuid]);
      }
      addon.plan = new_plan;
      redeploy = true;
    }
