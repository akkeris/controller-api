...
async function provision(addon_definition, pg_pool, app, addon_plan) {
  ...
  try {
    ...
    if(!addon_plan.primary) {
      ...
      // remap the config vars to be returned
      let [bindtype, bindname] = service.foreign_key.split(':')
      let config_service_vars = {};
      let raw_service_vars = await common.alamo.mapped_service_config_vars(pg_pool, app.space, app.name, bindname, bindtype)
      raw_service_vars.forEach((cvar) => {
        if(cvar.name !== 'spec') {
          config_service_vars[cvar.name] = cvar.value;
        }
      })
      service.config_vars = config_service_vars
    }
...
async function attach(addon_definition, pg_pool, target_app, addon_plan, service, owner) {
  ...
  if(!addon_plan.primary) {
      ...
      // remap the config vars to be returned
      let [bindtype, bindname] = service_attachment[0].foreign_key.split(':')
      let config_service_vars = {};
      let raw_service_vars = await common.alamo.mapped_service_config_vars(pg_pool, target_app.space, target_app.name, bindname, bindtype)
      raw_service_vars.forEach((cvar) => {
        if(cvar.name !== 'spec') {
          config_service_vars[cvar.name] = cvar.value;
        }
      })
      service_attachment[0].config_vars = config_service_vars
    }
