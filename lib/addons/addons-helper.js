...
const update_service_attachment = query.bind(query, fs.readFileSync('./sql/update_service_attachment.sql').toString('utf8'), null);
...
function info(addon_definition) {
  const available_regions = addon_definition.plans
    .map((x) => x.regions)
...
async function get_plans(type, pg_pool) {
  ...
  return common.alamo.service_plans(pg_pool, type);
}

function create_service_attachment_name(addon_definition) {
...
async function action(addon_definition, pg_pool, plan, service, app, action_id, req_url, payload) {
  return addon_definition.action(pg_pool, plan, service, app, action_id, req_url, payload);
}
...
async function config_vars(addon_definition, pg_pool, service, space_name, app_name) {
  ...
  return common.alamo.mapped_service_config_vars(pg_pool, space_name, app_name, foreign_id, addon_definition.alamo_name);
}

const select_services_by_app_and_type = query.bind(query, fs.readFileSync('./sql/select_services_by_app_and_type.sql').toString('utf8'), (r) => r);
...
async function provision(addon_definition, pg_pool, app, addon_plan, addon_attachment_name) {
  ...
  const service = await addon_definition.provision(
    pg_pool, addon_definition.alamo_name, app.id, app.name, app.space, app.org, addon_plan, service_uuid,
  );
  ...
  try {
    ...
    if (!addon_plan.primary) {
      const map_ids = await addon_definition.demote(pg_pool, app, addon_plan, service);
      await update_service_attachment(pg_pool, [service.service_attachment, false, map_ids.join(',')]);
    }
    try {
      const addon_service = common.service_by_id_or_name(addon_definition.id);
    ...
    try {
      await addon_definition.unprovision(
        pg_pool, addon_definition.alamo_name, app.id, app.name, app.space, app.org, addon_plan, service,
      );
    } catch (unprovision_error) {
      console.error('FATAL ERROR: Unable to rollback provisioning, we successfully created, failed to record, and successfuly deleted. WE HAVE A STRAGGLER! ', service);
...
async function demote(addon_definition, pg_pool, app, addon_plan, addon) {
  ...
  const addons_of_type = await select_services_by_app_and_type(pg_pool, [app.id, addon_plan.addon_service.id]);
  if (addon.primary === true && addons_of_type.length > 1) {
    const new_primary = addons_of_type.filter((x) => x.service !== addon.service)[0];
    ...
    return addon_definition.demote(pg_pool, app, addon_plan, addon);
  }
  return [];
}
...
const update_service = query.bind(query, fs.readFileSync('./sql/update_service.sql').toString('utf8'), (r) => r);
async function update(addon_definition, pg_pool, app, addon_plan, addon) {
  ...
  const new_addon = await addon_definition.update(pg_pool, app, addon_plan, addon);
  await update_service(pg_pool, [
    addon.service,
    addon_plan.id,
    addon_plan.name,
    addon_plan.price.cents,
    new_addon.foreign_key,
  ]);
  return new_addon;
}

const select_service_attachment = query.bind(query, fs.readFileSync('./sql/select_service_attachment.sql').toString('utf8'), (r) => r);
const update_service_attachment_name = query.bind(query, fs.readFileSync('./sql/update_service_attachment_name.sql').toString('utf8'), (r) => r);
async function remap(addon_definition, pg_pool, app, addon, new_name) {
  ...
  if (addon.primary === false) {
    const attachment = await select_service_attachment(pg_pool, [app.id, addon.service_attachment]);
    ...
    const map_ids = await addon_definition.remap(pg_pool, app, addon, new_name);
...
async function promote(addon_definition, pg_pool, app, addon_plan, addon) {
  ...
  await Promise.all(addons_of_type.map(async (addon_of_type) => {
    if (addon_of_type.primary === true && addon_of_type.service !== addon.service) {
      const map_ids = await addon_definition.demote(pg_pool, app, addon_plan, addon_of_type);
      return update_service_attachment(pg_pool, [addon_of_type.service_attachment, false, map_ids.join(',')]);
    } if (addon_of_type.primary === false && addon_of_type.service === addon.service) {
...
const select_service_attachments = query.bind(query, fs.readFileSync('./sql/select_service_attachments.sql').toString('utf8'), (r) => r);
async function attach(addon_definition, pg_pool, target_app, addon_plan, service, owner, addon_attachment_name) {
  const attachments = await select_service_attachments(pg_pool, [service.service]);
  if (addon_attachment_name && attachments.some((x) => x.name.toLowerCase() === addon_attachment_name.toLowerCase())) {
    throw new common.ConflictError('This addon attachment name already exists for this service.');
  }
  if (attachments.some((x) => x.app === target_app.id)) {
  ...
  await addon_definition.attach(pg_pool, target_app, addon_plan, service);
  const service_attachment_uuid = uuid.v4();
  const created_updated = new Date();
  addon_attachment_name = addon_attachment_name || create_service_attachment_name(addon_definition, addon_plan);
  const service_attachment = await insert_service_attachment(pg_pool, [
    service_attachment_uuid,
    addon_attachment_name,
    service.service,
    target_app.id,
  ...
  if (!addon_plan.primary) {
    const map_ids = await addon_definition.demote(pg_pool, target_app, addon_plan, service_attachment[0]);
  ...
  service_attachment[0].config_vars = await config_vars(
    addon_definition, pg_pool, service, target_app.space, target_app.name,
  );
...
async function detach(addon_definition, pg_pool, app, addon_plan, service) {
  ...
  await addon_definition.detach(pg_pool, app, addon_plan, service);
  const service_attachment = await delete_service_attachment(pg_pool, [service.service, app.id]);
...
async function load_plans(addon_definition, pg_pool) {
  ...
  let loaded_plans = await get_plans(addon_definition.alamo_name, pg_pool);
  if (addon_definition.transform_alamo_plans) {
    loaded_plans = addon_definition.transform_alamo_plans(loaded_plans);
  }
  addon_definition.plans = loaded_plans;
}
