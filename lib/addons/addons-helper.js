...
async function provision(addon_definition, pg_pool, app, addon_plan, addon_attachment_name) {
  ...
  let service = await addon_definition.provision(pg_pool, addon_definition.alamo_name, app.name, app.space, app.org, addon_plan, service_uuid)
...
async function remap(addon_definition, pg_pool, app, addon, new_name) {
  ...
  assert.ok(addon.service_attachment, 'Unablet to find service attachment id for addon.')
  ...
  await update_service_attachment_name(pg_pool, [app.app_uuid, addon.service, new_name.toLowerCase()])
}
