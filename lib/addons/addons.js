...
async function get_plans(type, pg_pool) {
  console.assert(pg_pool, 'get_plans called without pg_pool')
...
function transform_service(app, addon_definition, addon_plan, service) {
  return {
    ...
    "web_url": "https://ui.appkit.sh/#/apps/" +  app.name + '-' + app.space
  };
}
...
async function detach(addon_definition, pg_pool, app, addon_plan, service, cb) {
  let attachments = await select_service_attachments(pg_pool, [service.service])
...
function begin_timers(addon_definition, pg_pool) {
  console.assert(pg_pool, 'Begin timers called without pg_pool connector.')
