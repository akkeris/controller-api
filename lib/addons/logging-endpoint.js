...
module.exports = function(drain, name, human_name, description, website) {
  ...
  async function provision(pg_pool, app, addon_plan) {
    ...
    let payload = {'url':drain};
    let data = await httph.request('post', config.alamo_app_controller_url + '/apps/' + app.name + '-' + app.space + '/log-drains', {"Authorization":config.simple_key}, payload)
  ...
  async function unprovision(pg_pool, app, addon_plan, service) {
    let data = await httph.request('get', config.alamo_app_controller_url + '/apps/' + app.name + '-' + app.space + '/log-drains', {"Authorization":config.simple_key}, null)
    data = JSON.parse(data);
    data = data.filter((x) => { return x.url === drain});
    if(data.length !== 1) {
      throw new common.NotFoundError("The specified addon could not be found.")
    }
    await httph.request('delete', config.alamo_app_controller_url + '/apps/' + app.name + '-' + app.space + '/log-drains/' + data[0].id, {"Authorization":config.simple_key}, null)
