...
function info(service) {
  return {
    ...
    supports_sharing: !!service.bindable,
    updated_at: '2016-08-09T12:00:00Z',
  };
}
...
async function get_state(pg_pool, addon_definition, plan_id, addon_id, app_uuid) {
  ...
  const res = await common.alamo.get_osb_status(
    pg_pool, addon_definition.id, plan_id, addon_id, app.space_name, app.app_name,
  );
...
async function provision(pg_pool, type, app_uuid, app_name, space_name, org_name, addon_plan, addon_id) {
  ...
  await common.alamo.create_osb_service(
    pg_pool, type, addon_plan.addon_service.id, addon_plan.id, addon_id, org_name, space_name, app_name,
  );
  try {
    await common.alamo.create_osb_bindings(
      pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, space_name, app_name, addon_id,
    );
  } catch (e) {
  ...
  const results = await Promise.all(
    formations.map(async (form) => common.alamo.bind_service(
      pg_pool, space_name, common.alamo.app_name(app_name, form.type), type, addon_id,
    )),
  );
  const app = await common.app_exists(pg_pool, `${app_name}-${space_name}`);
  const status = await get_state(pg_pool, addon_plan.addon_service, addon_plan.id, addon_id, app.app_uuid);
  return Object.assign(status, {
    foreign_key: `${type}:${addon_id}`, config_vars: {}, reply: results, created: new Date(),
  });
}
...
async function unprovision(pg_pool, type, app_uuid, app_name, space_name, org, addon_plan, service) {
  ...
  const [, addon_id] = service.foreign_key.split(':');
  try {
    await common.alamo.delete_osb_bindings(
      pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, space_name, app_name,
    );
  } catch (e) {
    // we don't need this to succeed.
  }
  ...
  await common.alamo.delete_osb_service(pg_pool, type, addon_id, addon_plan.id, org, space_name, app_name);
  return service.foreign_key;
}

async function update(pg_pool, app, addon_plan, addon) {
  const [addon_service_name, addon_id] = addon.foreign_key.split(':');
...
async function attach(pg_pool, app, addon_plan, service) {
  const [addon_service_name, addon_id] = service.foreign_key.split(':');
  ...
  await Promise.all(
    formations.map(async (form) => common.alamo.bind_service(
      pg_pool, app.space, common.alamo.app_name(app.name, form.type), addon_service_name, addon_id,
    )),
  );
  try {
    await common.alamo.create_osb_bindings(
      pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, app.space, app.name, app.id, service.service_attachment
    );
  } catch (e) {
    console.error(`Warning: Unable to create osb binding during attach for ${app.id} and addon ${JSON.stringify(service)}: Error: ${JSON.stringify(e)}`);
...
async function detach(pg_pool, app, addon_plan, service) {
  ...
  try {
    await common.alamo.delete_osb_bindings(
      pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, app.space, app.name,
    );
  } catch (e) {
    console.error(`Warning: Unable to remove osb binding during detach for ${app.id} and addon ${JSON.stringify(service)}: Error: ${JSON.stringify(e)}`);
    console.error('         this may not be a serious issue as some brokers do not support binding operations.');
    console.error(e);
  }
  await Promise.all(
...
module.exports = async function (pg_pool) {
  const services = await common.alamo.osb_catalog(pg_pool);
