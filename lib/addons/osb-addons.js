...
async function provision(pg_pool, type, app_uuid, app_name, space_name, org_name, addon_plan, service_uuid) {
  ...
  let status = await get_state(pg_pool, addon_plan.addon_service, addon_plan.id, service_uuid, app.app_uuid)
  return Object.assign(status, {foreign_key:type + ":" + service_uuid, config_vars:{}, reply:results, created:new Date()})
}
