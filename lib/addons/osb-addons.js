...
async function provision(pg_pool, type, app_uuid, app_name, space_name, org_name, addon_plan, addon_id) {
  ...
  try {
    ...
    console.error(`Warning: Unable to create osb binding during provision for ${app_uuid} and addon_id ${addon_id}: Error: ${JSON.stringify(e)}`);
...
async function unprovision(pg_pool, type, app_uuid, app_name, space_name, org, addon_plan, service) {
  ...
  try {
    await common.alamo.delete_osb_bindings(pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, space_name, app_name);
  } catch (e) {
    console.error(`Warning: Unable to remove osb binding during deprovision for ${app_uuid} and addon ${JSON.stringify(service)}: Error: ${JSON.stringify(e)}`);
    console.error(`         this may not be a serious issue as some brokers do not support binding operations.`);
    console.error(e);
  }
...
async function attach(pg_pool, app, addon_plan, service) {
  ...
  try {
    await common.alamo.create_osb_bindings(pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, app.space, app.name);
  } catch (e) {
...
async function detach(pg_pool, app, addon_plan, service) {
  ...
  try {
    await common.alamo.delete_osb_bindings(pg_pool, addon_plan.addon_service.id, addon_plan.id, addon_id, app.space, app.name);
  } catch (e) {
