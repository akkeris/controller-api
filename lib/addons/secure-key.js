...
async function create_securekey(pg_pool, app_name, space_name, foreign_key) {
  ...
  console.assert(primary.length === 64, 'The primary secure key length was not 64 characters long.')
  let secondary = await random_hex()
  console.assert(secondary.length === 64, 'The secondary secure key length was not 64 characters long.')
  let value = `${primary},${secondary}`
  console.assert(value.length === 129, 'The length of the secure key was not 129 characters after rotating.')
...
async function rotate(pg_pool, app_name, space_name, service) {
  ...
  console.assert(config_set.length === 1, 'The config set for secure key contained more than one key val pair.')
  console.assert(config_set[0].varvalue.indexOf(',') !== -1, 'The value for SECURE_KEY was invalid it did not contain a comma.')
  // Save off the last value
  console.assert(config_set[0].varvalue.length === 129, 'The length of the secure key was not 129 characters before rotating.')
  let primary = config_set[0].varvalue.split(',')[1]
  console.assert(primary.length === 64, 'The primary secure key length was not 64 characters long.')
  let secondary = await random_hex()
  console.assert(secondary.length === 64, 'The secondary secure key length was not 64 characters long.')
  config_set[0].varvalue = `${primary},${secondary}`
  console.assert(config_set[0].varvalue.length === 129, 'The length of the secure key was not 129 characters after rotating.')
  ...
  attachments.forEach((attachment) => {
    lifecycle.restart_and_redeploy_app(pg_pool, attachment.app, attachment.app_name, attachment.space, attachment.org, 'Rotated SECURE_KEYS').catch((err) => { /* do nothing */ });
  })
}
...
async function attach(pg_pool, app, addon_plan, service, owner) {
  ...
  await Promise.all(formations.map(async (form) => {
    return await common.alamo.config.bind(pg_pool, app.name, app.space, common.alamo.app_name(app.name, form.type), app.space, service.foreign_key)
  }))
  ...
  await insert_service_attachment(pg_pool, [service_attachment_uuid, service_attachment_name, service.service, app.id, true, created_updated, created_updated]);
...
async function detach(pg_pool, app, addon_plan, service) {
  ...
  await Promise.all(formations.map(async (form) => {
    return await common.alamo.config.unbind(pg_pool, app.name, app.space, common.alamo.app_name(app.name, form.type), app.space, service.foreign_key)
  }))
...
async function get_config_vars(pg_pool, service, space_name, app_name) {
  let config_set = await alamo.config.set.request(pg_pool, app_name, space_name, service.foreign_key)
  console.assert(config_set.length === 1, 'The config set for secure key contained more than one key val pair.')
  console.assert(config_set[0].varvalue.indexOf(',') !== -1, `The value for ${env_name} was invalid it did not contain a comma.`)
  let config_vars = {}
  config_vars[env_name] = config_set[0].varvalue
  return config_vars;
}
