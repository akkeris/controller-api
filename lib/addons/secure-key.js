...
async function create_securekey(pg_pool, app_name, space_name, foreign_key) {
  await alamo.config.set.create(pg_pool, app_name, space_name, foreign_key);
  const primary = await random_hex();
  assert.ok(primary.length === 64, 'The primary secure key length was not 64 characters long.');
  const secondary = await random_hex();
...
async function rotate(pg_pool, app_name, space_name, service) {
  const config_set = await alamo.config.set.request(pg_pool, app_name, space_name, service.foreign_key);
  ...
  const primary = config_set[0].varvalue.split(',')[0];
  assert.ok(primary.length === 64, 'The primary secure key length was not 64 characters long.');
  const secondary = await random_hex();
  assert.ok(secondary.length === 64, 'The secondary secure key length was not 64 characters long.');
  ...
  attachments.forEach((attachment) => {
    lifecycle.restart_and_redeploy_app(pg_pool, attachment.app, attachment.app_name, attachment.space, attachment.org, 'Rotated SECURE_KEY').catch(() => { /* do nothing */ });
  });
}
...
const actions_exec = {
  rotate: {
    async exec(pg_pool, plan, service, app /* action_id, req_url */) {
