...
async function attach(pg_pool, app, addon_plan, service, owner) {
  ...
  await Promise.all(formations.map(async (form) => {
    return await common.alamo.config.include(pg_pool, app.space, 'config', `${common.alamo.app_name(app.name, form.type)}-${app.space}`, 'service', service.foreign_key)
  }))
...
async function detach(pg_pool, app, addon_plan, service) {
  ...
  await Promise.all(formations.map(async (form) => {
    return await common.alamo.config.exclude(pg_pool, app.space, 'config', `${common.alamo.app_name(app.name, form.type)}-${app.space}`, 'service', service.foreign_key)
  }))
