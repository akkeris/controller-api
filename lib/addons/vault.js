...
function vault_plans_to_services(vault_plans) {
  ...
  vault_plans = Object.keys(grouping).map((key) => { return grouping[key]; });
  return vault_plans;
}
...
async function provision(pg_pool, type, app_name, space_name, org, addon_plan) {
...
function timer_begin() {
  // do nothing, eventually poll for vault services?
}

let exempt_list = {'p42':true}
function create_service(plan, regions, cb) {
  ...
  let service = {
    ...
    get_state:function() { return "provisioned" },
    ...
    timers:{begin:timer_begin.bind(null, plan)}
  }
  cb(service);
}

module.exports = async function(cb, pg_pool) {
  let plans = await common.alamo.vault_plans(pg_pool, 'vault')
  let regions = await common.alamo.regions(pg_pool)
  try {
    plans = vault_plans_to_services(plans);
    plans.forEach((plan) => {
      create_service(plan, regions, cb);
    });
  } catch (e) {
    console.error(e)
  }
