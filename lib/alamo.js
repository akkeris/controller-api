 ...
 * Topics
 */
async function create_topic(region, cluster, name, description, cleanup_policy, partitions, retention_ms, replicas) {
  let payload = {
    name,
    description,
    cleanup_policy,
  ...
  return await alamo_fetch('post', `${get_api_by_region_name(region)}/v1/kafka/cluster/${cluster}/topic`, payload);
}

async function get_topic(region, name) {
  return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/topics/${name}`);
}

async function list_topics(region) {
  return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/topics`);
}
...
async function create_topic_acl(region, cluster, topic, app, role) {
  let payload = {
    topic,
    app,
    role
  ...
  return await alamo_fetch('post', `${get_api_by_region_name(region)}/v1/kafka/cluster/${cluster}/acls`, payload);
}

async function delete_topic_acl(region, acl_id) {
  return await alamo_fetch('delete', `${get_api_by_region_name(region)}/v1/kafka/acls/${acl_id}`);
}

async function get_topic_acls(region, cluster, topic) {
  return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/kafka/cluster/${cluster}/acls?topic=${topic}`);
}
