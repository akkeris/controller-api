...
async function create_schema_mapping(region, cluster, topic, schema, role){
  
  if (role == 'key'){
    let payload;
    if (schema == 'string' || schema == 'none'){
      payload = { topic, 
        keyType: schema 
      };
    }
    ...
    throw 'Role must be "key" or "value"';
  }
}
