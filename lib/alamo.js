...
async function create_custom_config_set(pg_pool, space_name, name, type) {
  return await alamo_fetch('post', `${await get_region_api_by_space(pg_pool, space_name)}/v1/config/set`,
...
async function unbind_config_set(pg_pool, app_name, space_name, to_app_name, to_space_name, service_name, bind_type) {
  bind_type = bind_type || 'config'
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
...
async function create_osb_service(pg_pool, type, service_id, addon_service_id, plan_id, org_id, space_name, app_name) {
  return await alamo_fetch('put', `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${service_id}?accepts_incomplete=true`, {service_id:addon_service_id, plan_id, content:{platform:"akkeris", organization:{id:org_id}, space:{name:space_name}}, organization_guid:org_id})
}

async function update_osb_service(pg_pool, space_name, service_id, addon_service_id, plan_id) {
  return await alamo_fetch('patch', `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${service_id}?accepts_incomplete=true`, {service_id:addon_service_id, plan_id, previous_values:null})
}
...
async function deploy(pg_pool, space_name, app_name, formation_type, image, command, port, healthcheck, features, labels) {
  ...
  if(process.env.DEBUG) {
    console.log(`[debug] Deploying`, JSON.stringify({appname:alamo_app_name, appimage:image, space:space_name, command, port, features, labels}, null, 2));
  }
  await alamo_fetch('post', `${await get_region_api_by_space(pg_pool, space_name)}/v1/app/deploy`, {appname:alamo_app_name, appimage:image, space:space_name, command, port, features, labels})
}
...
async function delete_topic(region, cluster, name) { 
  try {
    return await alamo_fetch('delete', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/topics/${name}`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function get_topic(region, name) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/topics/${name}`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function get_topic_preview(region, cluster, name) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/topics/${name}/preview`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_topics(region) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/topics`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_consumer_groups(region, cluster) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/consumer-groups`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_consumer_group_offsets(region, cluster, consumer_group_name) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/consumer-groups/${consumer_group_name}/offsets`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_consumer_group_members(region, cluster, consumer_group_name) {
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/consumer-groups/${consumer_group_name}/members`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function seek_consumer_group(region, cluster, consumer_group_name, payload) {
  try {
    return await alamo_fetch('post', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/consumer-groups/${consumer_group_name}/seek`, payload);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_schemas(region, cluster){
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/schemas`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_schema_mappings(region, cluster, topic){
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region)}/v1/kafka/cluster/${cluster}/topics/${topic}/topic-schema-mappings`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function create_key_mapping(region, cluster, topic, keytype, schema){
  ...
  try {
    return await alamo_fetch('post', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/topic-key-mapping`, payload)
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function create_value_mapping(region, cluster, topic, schema){
  ...
  try {
    return await alamo_fetch('post', `${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/topic-schema-mapping`, payload)
  } catch(e) {
    handleKafkaHttpError(e)
  }
}

async function delete_schema_mapping(region, cluster, topic, schema){
  try {
    return await alamo_fetch('delete', `${get_api_by_region_name(region, cluster)}/v1/service/kafka/acls/${id}`)
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function list_topic_configs(region, cluster) {
  ...
  try {
    return await alamo_fetch('get', `${get_api_by_region_name(region, true)}/v1/service/kafka/cluster/${cluster}/configs`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
}
...
async function get_topic_config(region, cluster, name){
  var configs 
  try {
    configs = await alamo_fetch('get', `${get_api_by_region_name(region, true)}/v1/service/kafka/cluster/${cluster}/configs`);
  } catch(e) {
    handleKafkaHttpError(e)
  }
...
function handleKafkaHttpError(e) {
...
module.exports = {
  ...
  config:{
    set:{
      ...
      create_custom:create_custom_config_set,
    },
