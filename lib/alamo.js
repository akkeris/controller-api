...
async function alamo_fetch(method, uri, payload, headers) {
  ...
  if(method.toLowerCase() !== 'get' && (typeof payload !== 'string')) {
...
async function deploy(pg_pool, space_name, app_name, formation_type, image, command, port) {
  ...
  await alamo_fetch('post', `${await get_region_api_by_space(pg_pool, space_name)}/v1/app/deploy`, {appname:alamo_app_name, appimage:image, space:space_name, command, port})
}
