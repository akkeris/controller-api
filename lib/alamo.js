...
async function create_osb_bindings(pg_pool, service_id, plan_id, addon_id, space_name, app_name, service_attachment) {
  try {
    return alamo_fetch(
      'put',
      `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${addon_id}/service_bindings/${service_attachment}`,
      { service_id, plan_id },
      { 'x-silent-error': true },
    );
  } catch (e) {
...
async function delete_osb_bindings(pg_pool, service_id, plan_id, addon_id, space_name, app_name, service_attachment) {
  try {
    return alamo_fetch(
      'delete',
      `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${addon_id}/service_bindings/${service_attachment}`,
