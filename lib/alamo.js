...
async function mapped_service_config_vars(pg_pool, space_name, app_name, bindname, bindtype) {
  try {
    let config_vars = await alamo_fetch('get', `${await get_region_api_by_space(pg_pool, space_name)}/v1/space/${space_name}/app/${app_name}/configvars/${bindtype}/${bindname}`, null, {"x-silent-error":"true"})
    let obj = {}
...
async function dyno_service_exists(pg_pool, app_name, space_name) {
  try {
    let service = await alamo_fetch('get', `${await get_region_api_by_space(pg_pool, space_name)}/v1/utils/service/space/${space_name}/app/${app_name}`, null, {"x-silent-error":true});
...
async function create_osb_bindings(pg_pool, service_id, plan_id, addon_id, space_name, app_name) {
  try {
    return await alamo_fetch('put', `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${addon_id}/service_bindings/${app_name}-${space_name}`, {service_id, plan_id}, {"x-silent-error":true});
  } catch (e) {
    if (e.code === 422) {
      return; // indicates the broker is still creating, but acknowledges it.
    } else {
      throw e
    }
  }
...
async function delete_osb_bindings(pg_pool, service_id, plan_id, addon_id, space_name, app_name) {
  try {
    return await alamo_fetch('delete', `${await get_region_api_by_space(pg_pool, space_name)}/v2/service_instances/${addon_id}/service_bindings/${app_name}-${space_name}`, null, {"x-silent-error":true})
  } catch (e) {
    if (e.code === 422) {
      return // indicates the broker is still creating, but acknowledges it.
    } else {
...
async function dyno_attach(pg_pool, space, name, type, instance, command, stdin) {
  let alamo_name = get_alamo_app_name(name, type)
  return await alamo_fetch('post', `${await get_region_api_by_space(pg_pool, space)}/v1/space/${space}/app/${alamo_name}/instance/${instance}/exec`, JSON.stringify({command, stdin}))
}
