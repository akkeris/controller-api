...
async function bind_config_set(pg_pool, app_name, space_name, to_app_name, to_space_name, service_name) {
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
...
async function unbind_config_set(pg_pool, app_name, space_name, to_app_name, to_space_name, service_name) {
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
  await alamo_fetch('delete',`${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${to_space_name}/app/${to_app_name}/bind/config:${bind_name}`, null)
}
