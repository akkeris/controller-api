...
async function app_describe(pg_pool, app_name) {
  ...
  let appd = await alamo_fetch('get', `${await get_stack_api_by_space(pg_pool, space)}/v1/space/${space}/app/${alamoapp}`, null)
  return appd;
}
...
async function request_config_set(pg_pool, app_name, space_name, service_name) {
  ...
  return await alamo_fetch('get', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/config/set/${bind_name}`, null)
}
...
async function bind_config_set(pg_pool, app_name, space_name, to_app_name, to_space_name, service_name, bind_type) {
  ...
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
  await alamo_fetch('post',`${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${to_space_name}/app/${to_app_name}/bind`, 
      JSON.stringify({
        appname: to_app_name,
        space: to_space_name,
        bindtype: bind_type,
...
async function unbind_config_set(pg_pool, app_name, space_name, to_app_name, to_space_name, service_name) {
  bind_type = bind_type || 'config'
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
  await alamo_fetch('delete',`${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${to_space_name}/app/${to_app_name}/bind/${bind_type}:${bind_name}`, null)
}

async function include_config_set(pg_pool, parent_space_name, parent_type, parent_name, child_type, child_name) {
  await alamo_fetch('post', `${await get_stack_api_by_space(pg_pool, parent_space_name)}/v1/config/set/${parent_name}/include/${child_name}`, null)
}

async function exclude_config_set(pg_pool, parent_space_name, parent_type, parent_name, child_type, child_name) {
  await alamo_fetch('delete', `${await get_stack_api_by_space(pg_pool, parent_space_name)}/v1/config/set/${parent_name}/include/${child_name}`, null)
}

async function delete_config_set(pg_pool, app_name, space_name, service_name) {
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
  return await alamo_fetch('delete', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/config/set/${bind_name}`, null)
}
...
async function delete_config_var(pg_pool, app_name, space_name, name, service_name) {
  let bind_name = service_name ? service_name : `${app_name}-${space_name}`
  return await alamo_fetch('delete', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/config/set/${bind_name}/configvar/${name}`, null)
}
...
async function dyno_service_exists(pg_pool, app_name, space_name) {
  let service = await alamo_fetch('get', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/utils/service/space/${space_name}/app/${app_name}`, null);
...
async function create_log_drain(pg_pool, space_name, app_name, destination_url) {
...
async function list_log_drains(pg_pool, space_name, app_name) {
  return await alamo_fetch('get', `${await get_region_api_by_space(pg_pool, space_name)}/apps/${app_name}-${space_name}/log-drains`, null)
}

async function get_log_drains(pg_pool, space_name, app_name, drain_id) {
...
async function delete_log_drain(pg_pool, space_name, app_name, drain_id) {
...
async function create_log_session(pg_pool, space_name, app_name, lines, tail) {
...
async function maintenance_page(pg_pool, space_name, alamo_app_name, torf) {
    if (torf === true) {
        return await alamo_fetch('post', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${space_name}/app/${alamo_app_name}/maintenance`, null);
    }
    if (torf === false) {
        return await alamo_fetch('delete', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${space_name}/app/${alamo_app_name}/maintenance`, null);
    }
}
...
async function dyno_status(pg_pool, space_name, alamo_app_name) {
  return await alamo_fetch('get', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/kube/podstatus/${space_name}/${alamo_app_name}`, null)
}
...
async function dyno_restart(pg_pool, space_name, alamo_app_name) {
  return await alamo_fetch('post', `${await get_stack_api_by_space(pg_pool, space_name)}/v1/space/${space_name}/app/${alamo_app_name}/restart`, null, {'x-ignore-errors':'true'})
}
