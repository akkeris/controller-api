...
async function osb_catalog(pg_pool) {
  return (await Promise.all((await get_all_regions(pg_pool))
    .map(async (region) => {
      try {
        return (await alamo_fetch('get', `${region.url}/v2/catalog`, null, {'x-silent-error':true})).services.map((x) => Object.assign({"regions":[region.name]}, x))
      } catch (e) {
        return []
      }
    })))
    .reduce((services, reg_services) => {
      return services
        // if any existing services already have a service in reg_services add its regions.
        .map((service) => {
          service = Object.assign(service, {"regions":service.regions.concat(reg_services.filter((reg_service) => service.id === reg_service.id).reduce((sum, x) => sum.concat(x.regions), []))})
          // concat plans and reduce?...
          return service
        })
        // concat all regional_services which are not in the current services.
        .concat(reg_services.filter((reg_service) => !services.some((service) => reg_service.id === service.id)))
    }, [])
}
...
async function delete_topic(region, name) {
  return await alamo_fetch('delete', `${get_api_by_region_name(region)}/v1/service/kafka/topics/${name}`);
}
...
console.log(`post : ${get_api_by_region_name(region)}/v1/service/kafka/cluster/${cluster}/acls`)
console.log(payload)
...
async function get_topic_config(region, cluster, name){
  let configs = await alamo_fetch('get', `${get_api_by_region_name(region, true)}/v1/service/kafka/cluster/${cluster}/configs`);
