...
async function oneoff_deploy(
  ...
  try {
    ...
    // region-api expects [{ name: "key", value: "value" }]
    const newenv = [];
    if (env) {
    ...
    if (process.env.DEBUG) {
      console.log('[debug] Deploying One-off', JSON.stringify({
        appname: alamo_app_name, appimage: image, space: space_name, command, labels, env,
      }, null, 2));
    }

    await alamo_fetch('post', `${await get_region_api_by_space(pg_pool, space_name)}/v1/app/deploy/oneoff`, {
      podname: alamo_app_name,
      appimage: image,
      space: space_name,
      command,
      labels,
      ...(newenv.length > 0 && { env: newenv }),
    }, { 'x-silent-error': 'true' });
  } finally {
    deploying = false;
  }
