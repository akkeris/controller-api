...
async function delete_topic(region, name) {
  return await alamo_fetch('delete', `${get_api_by_region_name(region)}/v1/service/topics/${name}`);
}
...
async function list_schema_mappings(cluster){
  return await alamo_fetch('get', `${get_api_by_cluster_name(cluster)}/v1/service/kafka/cluster/${cluster}/schemas`)
}

async function create_schema_mapping(cluster, topic, schema, role){
  let payload = { topic, schema, role };

  if (role == 'key')
    return await alamo_fetch('post', `${get_api_by_cluster_name(cluster)}/v1/service/kafka/cluster/${cluster}/topic-key-mapping`, payload)
  else if (role == 'value')
    return await alamo_fetch('post', `${get_api_by_cluster_name(cluster)}/v1/service/kafka/cluster/${cluster}/topic-schema-mapping`, payload)
  else
    throw 'Role must be "key" or "value"';
}

async function delete_schema_mapping(cluster, topic, schema){
  return await alamo_fetch('delete', `${get_api_by_cluster_name(cluster)}/v1/service/kafka/acls/${id}`)
}
