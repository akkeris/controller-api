...
function format_filters(filts) {
  return filts.map((x) => { return {"filter":{"id":x.filter.filter},  "options":x.attachment_options} });
}
...
async function get_app_definition(pg_pool, app_key, ignore_build) {
  ...
  let formation_result  = await formation.list(pg_pool, app.app_uuid)
  ...
  let filters_result    = await common.filter_attachments_exists(pg_pool, app.app_uuid)
  let config_var_notes  = await config_vars.notes.get(pg_pool, app.app_uuid, app.space_name, app.app_name)
...
async function setup(pg_pool, payload) {
  ...
  try {
    ...
    setup_payload.filters.forEach((filter_instance) => {
      processing.push({"name":`Adding http filter ${filter_instance.filter.id}`, "item":filters.create_attachment.bind(filters.create_attachment, 
        pg_pool,
        app_uuid, 
    ...
    if(Object.keys(setup_payload.config_var_notes).length > 0) {
      processing.push({"name":"configuration variables notes", "item":config_vars.notes.update.bind(config_vars.notes.update,
        pg_pool, 
        app_uuid, 
        app_name, 
        space_name, 
