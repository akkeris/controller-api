...
function check_setup_features(payload) {
  if(payload.features) {
    for(let i=0; i < payload.features.length; i++) {
      console.assert(payload.features[i].id, 'The specified feature did not have an id.')
      console.assert(payload.features[i].name, 'The specified feature did not have a name.')
      console.assert(payload.features[i].enabled === true || payload.features[i].enabled === false, 'The specified feature did not have a value for enabled.')
    }
