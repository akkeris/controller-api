...
async function get_setup_status(pg_pool, req, res, regex) {
...
async function get_app_definition(pg_pool, req, res, regex) {
  ...
  httph.ok_response(res, JSON.stringify({
    ...
    , "source_blob":await format_source_blob_from_build(pg_pool, app.app_uuid, app.app_name, app.space_name, build)
    , "log-drains":format_log_drains(JSON.parse(logs_result))
    , "pipeline-couplings":format_pipeline_couplings(pipeline_result)
    // until we can figure out a way of managing this: , "sites":format_sites(routes_result)
  }));
}
...
async function setup(pg_pool, req, res, regex) {
  ...
  try {
    ...
    // create app
    //let app_manifest    = await manifests.create(pg_pool, payload.name, payload.description, payload.keywords, payload.success_url, payload.default_url, payload.admin_url, payload.logo || payload.logo_url, payload.scripts ? payload.scripts.postdeploy || null)
    let app_info        = await apps.create(pg_pool, payload.app.org, payload.app.space, payload.app.name) //, app_manifest.manifest_uuid)
    ...
    setup_payload.addons.forEach((plan) => {
      processing.push({"name":`Creating addon ${plan}`, "item":addon_services.addons.create.bind(addon_services.addons.create,
        pg_pool, 
        app_uuid, 
        app_name, 
        space_name, 
    ...
    // create auto release/build entry
    processing.push({"name":"Assigning auto build and deploy",  "item":auto_builds.create.bind(null, 
      pg_pool, 
      app_uuid, 
      null, 
      null, 
    ...
    ((async () => {
      ...
      for(let i=0; i < processing.length ; i++) {
        try {
          ...
          await update_app_setup(pg_pool, [app_setup_uuid, ((i + 1)/processing.length), errors.join(', '), state])
        }
      }
    })()).catch((e) => {
