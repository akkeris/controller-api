...
async function get_app_definition(pg_pool, app_key) {
...
async function http_get_app_definition(pg_pool, req, res, regex) {
  ...
  httph.ok_response(res, JSON.stringify(await get_app_definition(pg_pool, app_key)));
}
  ...
  try {
    ...
    ((async () => {
      ...
      for(let i=0; i < processing.length ; i++) {
        try {
          console.log(`app-setups (${i}/${processing.length}): ${processing[i].name}`)
