...
async function update(pg_pool, app_key, name, maintenance, description, labels) {
  ...
  if (description !== app.description || labels !== app.labels) {
    app.description = description === undefined ? app.description : description;
    app.labels = labels === undefined ? app.labels : labels;
  ...
  let errors = [];
  let dyno_types = await common.formations_exists(pg_pool, app.app_uuid)
