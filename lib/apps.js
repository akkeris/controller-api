...
function check_payload(payload) {
  assert.ok(payload.name && /(^[A-z0-9]+$)/.exec(payload.name) !== null, "The application name was invalid or missing, it cannot contain dashes or other special characters." );
...
// public
async function update_info(pg_pool, app_key, description, labels) {
  let app = await common.app_exists(pg_pool, app_key);
...
// public
const select_app_setup_by_app = query.bind(query, fs.readFileSync('./sql/select_app_setup_by_app.sql').toString('utf8'), (r) => { return r; });
async function del(pg_pool, app_key, elevated_access, user) {
  ...
  await Promise.all(preview_apps.map(async (preview_app) => {
    if(app.app_uuid === preview_app.source) {
      try {
        ...
        console.warn(`Error: Unable to remove preview app: ${preview_app.target} while trying to remove source app ${app_key}`)
  ...
  try {
    ...
    console.warn("Warning: unable to remove routes: ", e.message)
    console.warn(e.stack)
  }

  // if we happen to be a preview app, and we have sites which were created for our preview, remove them.
  try {
    await sites.remove_previews(pg_pool, app.app_uuid)
  } catch (e) {
    console.warn(`Warning: unable to remove preview sites for app uuid ${app.app_uuid}`)
    console.warn(e.stack)
  }
  // remove job if it exists.
  try {
    await builds.delete_job_if_exists(pg_pool, app.app_name, app.app_uuid)
  } catch (e) {
    console.warn(`Warning: unable to remove build job for app ${app.app_name}-${app.app_uuid}`)
    console.warn(e.stack)
  }
  // remove config set
...
// public
async function http_update_info(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let payload = await httph.buffer_json(req)
  let app = await update_info(pg_pool, app_key, payload.description, payload.labels)
  return httph.ok_response(res, JSON.stringify(app_postgres_to_response(app)))
}
