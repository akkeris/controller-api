...
function app_payload_to_response(payload) {
  return {
    ...
    formation: (payload.formation ? payload.formation : payload.size ? [{
      size: payload.size,
...
function check_payload(payload) {
  assert.ok(
    payload.name && /(^[A-Za-z0-9]+$)/.exec(payload.name) !== null,
    'The application name was invalid or missing, it cannot contain dashes or other special characters.',
  );
  assert.ok(
    payload.space && /(^[A-z0-9-]+$)/.exec(payload.space) !== null,
    'The application space field was not specified',
  );
  assert.ok(payload.org, 'The application org field was not specified');
  assert.ok(
    (`${payload.name}-${payload.space}`).length < 25,
    'The application name was too long, the app space and name must be less than 24 characters.',
  );
}

// private
const insert_app = query.bind(query, fs.readFileSync('./sql/insert_app.sql').toString('utf8'), null);
...
// const select_app = query.bind(query, select_app_query, app_postgres_to_response);

// private
const update_app = query.bind(query, fs.readFileSync('./sql/update_app.sql').toString('utf8'), (r) => r);
const update_app_info = query.bind(query, fs.readFileSync('./sql/update_app_info.sql').toString('utf8'), (r) => r);

const delete_app_query = fs.readFileSync('./sql/delete_app.sql').toString('utf8');
const delete_app = query.bind(query, delete_app_query, () => {});

// const delete_services_by_app_query = fs.readFileSync('./sql/delete_services_by_app.sql').toString('utf8');
// const delete_services_by_app = query.bind(query, delete_services_by_app_query, () => {});
...
// public
async function update(pg_pool, app_key, name, maintenance, description, labels, user) {
  ...
  if (description !== app.description || labels !== app.labels) {
    if (description === '') {
      hook_updated.app.description = app.description = '';
    } else if (description) {
      hook_updated.app.description = app.description = description;
    }
    hook_updated.app.labels = app.labels = labels || app.labels;
  ...
  await Promise.all((await common.formations_exists(pg_pool, app.app_uuid)).map(async (dyno_type) => {
    try {
      await common.alamo.dyno.scale(
        pg_pool,
        app.app_name,
        app.space_name,
        dyno_type.type,
        app.maintenance === true ? 0 : dyno_type.quantity,
      );
    } catch (err) {
      errors = true;
  ...
  const maintresult = await common.alamo.apps.maintenance_page(pg_pool, app.space_name, app.app_name, app.maintenance);
...
// public
async function create(pg_pool, org_key, space_key, app_name, description, labels, user) {
  app_name = app_name.toLowerCase().trim();
  space_key = space_key.toLowerCase().trim();

  // TODO: Include space configuration set into the app.
  const organization = await common.org_exists(pg_pool, org_key);
  const space = await common.space_exists(pg_pool, space_key);
  const app_key = `${app_name}-${space.name}`;
  let app_found = false;
  ...
  await insert_app(pg_pool, app_payload_to_postgres({
    id,
    created,
    modified: created,
    name: app_name,
  ...
  await features.update(pg_pool, id, app_name, space.name, 'http2', true);
  ...
  logs.event(pg_pool, app_name, space.name, 'Creating app');

  setTimeout(() => {
    common.notify_hooks(pg_pool, id, 'created', JSON.stringify({
      action: 'created',
      app: {
        name: app_name,
        id,
...
async function check_app_delete(pg_pool, app, space, elevated_access) {
  ...
  const app_setups = await select_app_setup_by_app(pg_pool, [app.app_uuid]);
...
async function remove_dependent_preview_apps(pg_pool, app, space, elevated_access) {
  const preview_apps = await common.preview_apps(pg_pool, app.app_uuid);
...
async function del(pg_pool, app_key, elevated_access, user) {
  ...
  // remove config set
  await common.alamo.config.set.delete(pg_pool, app.app_name, app.space_name);

  // remove actions
  await actions.delete_actions(pg_pool, app.app_uuid);
...
async function http_get(pg_pool, req, res, regex) {
  ...
  const app = await common.app_exists(pg_pool, app_key);
  const appresp = app_postgres_to_response(app);
  try {
    const appd = await common.alamo.app_describe(pg_pool, `${app.app_name}-${app.space_name}`);
...
async function http_create(pg_pool, req, res /* regex */) {
  ...
  return httph.created_response(
    res,
    await create(pg_pool, payload.org, payload.space, payload.name, payload.description, payload.labels),
  );
}
...
async function http_delete(pg_pool, req, res, regex) {
  const app_key = httph.first_match(req.url, regex);
  const app = await del(pg_pool, app_key, req.headers['x-elevated-access'] === 'true' && req.headers['x-username'], req.headers['x-username']);
...
async function http_update(pg_pool, req, res, regex) {
  const app_key = httph.first_match(req.url, regex);
  const payload = await httph.buffer_json(req);
  const app = await update(pg_pool, app_key, payload.name, payload.maintenance, payload.description, payload.labels, req.headers['x-username']);
