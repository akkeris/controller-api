...
function check_payload(payload) {
  assert.ok(/(^[A-Za-z0-9]+$)/.exec(payload.name) !== null, "The application name was invalid or missing, it cannot contain dashes or other special characters." );
...
async function update(pg_pool, app_key, name, maintenance, description, labels) {
  ...
  let errors = false;
  (await common.formations_exists(pg_pool, app.app_uuid)).forEach((dyno_type) => {
    try {
...
async function remove_preview_sites(pg_pool, app_uuid) {
  try {
    ...
    console.warn(`Warning: unable to remove preview sites for app uuid ${app_uuid}`)
    console.warn(e.stack)
  }
}
...
async function remove_pending_build_jobs(pg_pool, app_name, app_uuid) {
  try {
    ...
    console.warn(`Warning: unable to remove build job for app ${app_name}-${app_uuid}`)
    console.warn(e.stack)
  }
}
...
async function del(pg_pool, app_key, elevated_access, user) {
  ...
  // async remove routes
  await remote_routes(pg_pool, app.app_uuid);

  // if we happen to be a preview app, and we have sites which were created for our preview, remove them.
  await remote_preview_sites(pg_pool, app.app_uuid);

  // remove job if it exists.
  await remote_pending_build_jobs(pg_pool, app.app_name, app.app_uuid);
