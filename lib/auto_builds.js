...
const insert_auto_build = query.bind(query, fs.readFileSync('./sql/insert_auto_build.sql').toString('utf8'), (d) => d);
const select_auto_build = query.bind(query, fs.readFileSync('./sql/select_auto_build.sql').toString('utf8'), (d) => d);
const delete_auto_build = query.bind(query, fs.readFileSync('./sql/delete_auto_build.sql').toString('utf8'), (d) => d);
const update_auto_build = query.bind(query, fs.readFileSync('./sql/update_auto_build.sql').toString('utf8'), (d) => d);

async function del(pg_pool, app_uuid) {
  return delete_auto_build(pg_pool, [app_uuid]);
}

async function get(pg_pool, app_uuid) {
  const auto_builds = await select_auto_build(pg_pool, [app_uuid]);
...
async function create(
  ...
  return insert_auto_build(pg_pool, auto_build_params);
}

async function copy(pg_pool, src_app_uuid, dst_app_uuid) {
  const auto_build = await get(pg_pool, src_app_uuid);
  return create(pg_pool, dst_app_uuid, auto_build.repo, auto_build.branch, auto_build.authorization, true, true, 'auto-build-copy', auto_build.validation_token);
}
