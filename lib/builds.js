...
async function build_status_change(pg_pool, build_uuid, foreign_key, foreign_type, building, status) {
  ...
  if(status !== 'pending' && status !== 'queued') {
    logs.event(item.name, item.space, "Slug compilation " + (status === 'succeeded' ? 'finished' : 'failed'));
    if(item.token) {
      github.add_status( status === 'succeeded' ?  'success' : 'failure', item.name, item.space, item.token, item.repo, item.sha, 'Image build ' + status);
    }
  }
