...
function validate_callback_auth(token, build_uuid) {
  let stamp = Math.round((new Date()).getTime() / 1000000)
  let hmac = crypto.createHmac('sha256', (config.simple_key + stamp) )
  hmac.update(build_uuid)
  if(hmac.digest("hex") === token) {
    return true;
  } else {
    stamp = Math.round((new Date()).getTime() / 1000000) - 1
    hmac = crypto.createHmac('sha256', (config.simple_key + stamp) )
    hmac.update(build_uuid)
    if(hmac.digest("hex") === token) {
      return true
    }
...
async function build_status_change(pg_pool, build_uuid, foreign_key, foreign_type, building, status) {
  let item = await query_build(pg_pool, build_uuid)
  await mark_build_status(pg_pool, [build_uuid, status, foreign_type, foreign_key]);
  if(status !== 'pending' && status !== 'queued') {
    logs.event(item.app_name, item.space_name, "Slug compilation " + (status === 'succeeded' ? 'finished' : 'failed'));
    if(item.token) {
      github.add_status( status === 'succeeded' ?  'success' : 'failure', item.app_name, item.space_name, item.token, item.repo, item.sha, 'Image build ' + status);
    }
  }
  common.notify_hooks(pg_pool, item.app, 'build', JSON.stringify({
    'action':'build',
...
async function http_build_status_change(pg_pool, req, res, regex) {
  ...
  httph.ok_response(res, JSON.stringify(
    await build_status_change(pg_pool, build_key, payload.id, payload.type, payload.building, payload.status)))
}
...
async function request_build(pg_pool, app_name, app_uuid, space_name, sha, branch, repo, org, build_uuid, sources) {
  ...
  if(process.env.TEST_MODE && process.env.TEST_CALLBACK) {
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, auto_build, repo, branch, version, checksum, sha, url, author, message) {
  ...
  await request_build(pg_pool, app_name, app_uuid, space_name, build_obj.sha, build_obj.branch, build_obj.repo, org, build_obj.id, build_obj.url);
...
async function rebuild(pg_pool, req, res, regex) {
  ...
  //let foreign_build = 
  await request_build(pg_pool, app.app_name, app.app_uuid, app.space_name, build_obj.sha, build_obj.branch, build_obj.repo, app.org_name, build_uuid, build_obj.url)
  //build_obj.foreign_build_key = foreign_build.id;
...
async function result(pg_pool, req, res, regex) {
  ...
  let content = await httph.request('get', `${config.build_shuttle_url}/${app.app_name}-${app.app_uuid}/${build.foreign_build_key}/logs`, {'content-type':'application/json'}, null)
  if(content) {
