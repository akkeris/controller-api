...
const jenkins = jenkins_api.init(config.jenkins_url);

// private
const mark_build_status = query.bind(query, fs.readFileSync("./sql/update_build_status.sql").toString("utf8"), () => {})
const pull_building_items = query.bind(query, fs.readFileSync("./sql/select_building_builds.sql").toString('utf8'), (r) => { return r })
async function jenkins_builds(pg_pool) {
  try {
    await Promise.all((await pull_building_items(pg_pool, [])).map(async (item) => {
      let build = await jenkins.build_info(item.app_name + '-' + item.app_uuid, item.foreign_build_key)
      console.assert(build, 'Unable to process response from jenkins.')
      console.assert(!build.statusCode || build.statusCode < 405, 'Invalid status code response from jenkins.')
      if(build.building === false) {
        let status = build.result === 'SUCCESS' ? 'succeeded' : 'failed';
        await mark_build_status(pg_pool, [item.build, status, 'jenkins', item.foreign_build_key]);
        logs.event(item.app_name, item.space_name, "Slug compilation " + (status === 'succeeded' ? 'finished' : 'failed'));
        common.notify_hooks(pg_pool, item.app_uuid, 'build', JSON.stringify({
          'action':'build',
          'app':{ 'name':item.app_name, 'id':item.app_uuid },
          'space':{ 'name':item.space_name },
          'build':{
            'id':item.build,
            'result':status,
        ...
        if(item.token) {
          github.add_status( status === 'succeded' ?  'success' : 'failure', item.app_name, item.space_name, item.token, item.repo, item.sha, 'Image build ' + status);
        }
      }
    }))
  } catch (e) {
    console.error("Cannot pull build from jenkins:", e)
  } finally {
    setTimeout(() => { jenkins_builds(pg_pool).catch((e) => { console.error("Error processing builds:", e) }) }, 5000);
  }
}
...
function build_payload_to_obj(req_body, app_uuid, contents, userAgent, app_key, org, author, message) {
  ...
  build_obj.status = 'pending';
  build_obj.user_agent = userAgent;
  build_obj.description = 'foreign key is jenkins queue';
...
function build_obj_to_response(build_obj) {
  let resp = {
    ...
    status:build_obj.status === "queued" ? "pending" : build_obj.status,
...
async function check_build_succeeded(pg_pool, build_uuid) {
  ...
  result.docker_registry_url = common.registry_image(result.org, result.name, result.app, result.foreign_build_key);
  return result
}

// private
async function request_build(pg_pool, app_name, app_uuid, space_name, sha, branch, repo, org, build_uuid, sources) {
  ...
  let build_params = { 
    ...
    sources, 
    build_options,
    gm_registry_host:config.docker_registry_host,
    gm_registry_repo:config.gm_docker_repo,
    docker_registry:'', 
    docker_login:'', 
    docker_password:''
  };
  if(sources.toLowerCase().startsWith("docker://")) {
    ...
    try {
      console.assert(docker_uri.path.indexOf(':') > -1, 'No tag for the specified docker image was used. Please add a unique tag name.');
      console.assert(!docker_uri.auth || docker_uri.auth.indexOf(":") > -1, 'A username and password must be specified for a docker uri.');
      let docker_login = '';
      let docker_password = '';
      if(docker_uri.auth) {
  ...
  if(config.build_shuttle_url) {
    let data = await httph.request('post', config.build_shuttle_url, {'content-type':'application/json'}, JSON.stringify(build_params));
    if(data) {
      data = JSON.parse(data);
    }
    common.notify_hooks(pg_pool, app_uuid, 'build', JSON.stringify({
      'action':'build',
      'app':{
        'name':app_name,
        'id':app_uuid
    ...
    return jenkins.build(`${app_name}-${app_uuid}`, build_params);
  }
}
...
async function check_job_exists(app_name, app_uuid) {
  try {
    ...
    if(e.code === 404) {
      throw new common.NotFoundError("The specified job could not be found.")
    }
    console.error(e)
    if(e.stack) console.error(e.stack)
    throw new common.ServiceUnavailableError("Jenkins did not respond with the applications job info.")
  }
}
  ...
  try { 
    ...
    if(e instanceof common.NotFoundError) {
      try {
        ...
        console.error(e)
        if(e.stack) console.error(e.stack)
        throw new common.ServiceUnavailableError("Jenkins did not create the application job correctly.")
      }
    }
  }
}
...
async function delete_job_if_exists(app_name, app_uuid) {
  try {
    ...
    jenkins.delete_job(`${app_name}-${app_uuid}`);
    return build_info
  } catch (e) {
    if(e.code !== 404) {
      console.error(e)
      if(e.message) console.error(e.message)
      throw new common.ServiceUnavailableError("Jenkins did not delete the application job correctly.")
    }
    return null
  }
}
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, auto_build, repo, branch, version, checksum, sha, url, author, message) {
  ...
  let job_info = await create_job_if_needed(app_name, app_uuid);
  build_obj.id = uuid.v4();
  let build = await request_build(pg_pool, app_name, app_uuid, space_name, build_obj.sha, build_obj.branch, build_obj.repo, org, build_obj.id, build_obj.url);
  build_obj.foreign_build_key = build.id;
  if(!build_obj.url || build_obj.url.length > 1024) {
    build_obj.url = "";
  }
  await create_build_record(pg_pool, build_obj_to_postgres(build_obj))
  logs.event(app_name, space_name, "Slug compilation started");
...
async function http_create(pg_pool, req, res, regex) {
  ...
  httph.created_response(res, JSON.stringify(
    await create(pg_pool, app.app_uuid, app.app_name, app.space_name, '', app.org_name, payload.auto_build, payload.repo, payload.branch, payload.version, payload.checksum, payload.sha, payload.url, payload.author, payload.message)));
}
...
async function http_get(pg_pool, req, res, regex) {
  ...
  return httph.ok_response(res, JSON.stringify(build_obj_to_response(build)));
}
...
async function rebuild(pg_pool, req, res, regex) {
  ...
  let job_info = await create_job_if_needed(app.app_name, app.app_uuid);
  let build_uuid = uuid.v4();
  let jenkins_build = await request_build(pg_pool, app.app_name, app.app_uuid, app.space_name, build_obj.sha, build_obj.branch, build_obj.repo, app.org_name, build_uuid, build_obj.url)
  build_obj.foreign_build_key = jenkins_build.id;
  build_obj.id = build_uuid;
  if(!build_obj.url || build_obj.url.length > 1024) {
    build_obj.url = "";
  }
  await create_build_record(pg_pool, build_obj_to_postgres(build_obj))
  logs.event(app.app_name, app.space_name, "Slug compilation started");
...
// public
async function result(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let build_uuid = httph.second_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  let build = await get(pg_pool, app.app_uuid, build_uuid)
  let content = await jenkins.job_output(`${app.app_name}-${app.app_uuid}`, build.foreign_build_key)
  ...
  return httph.ok_response(res, JSON.stringify({
    ...
    "lines":content && content.output ? content.output.split('\n') : ""
  }));
}

// public
//return httph.err_request(409, res, "The build could not be stopped, or is already finished.", ["Cannot stop build:", content_err, content]);
async function stop(pg_pool, req, res, regex) {
  ...
  let content = await jenkins.stop_build(app.app_name + '-' + app.app_uuid, build.foreign_build_key);
  await mark_build_status(pg_pool, [build.id, 'failed', 'The specified build was stopped.', build.foreign_build_key]);
...
function begin_timers(pg_pool) {
  jenkins_builds(pg_pool).catch((e) => { console.error("Error processing jenkins builds:", e)});
}
...
module.exports = {
  ...
  delete_job_if_exists:delete_job_if_exists,
  strip_build_results
};
