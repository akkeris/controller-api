...
async function build_status_change(pg_pool, build_uuid, foreign_key, foreign_type, building, status, req) {
...
async function http_build_status_change(pg_pool, req, res, regex) {
  ...
  httph.ok_response(res, JSON.stringify(
    await build_status_change(pg_pool, build_key, payload.id, payload.type, payload.building, payload.status, req)))
}
