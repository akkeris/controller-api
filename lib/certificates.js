...
const update_query = query.bind(query, fs.readFileSync('./sql/update_certificate.sql').toString('utf8'), (r) => r);
const select_query = query.bind(query, fs.readFileSync('./sql/select_certificate.sql').toString('utf8'), (r) => db_to_response(r));
async function info_endpoints(pg_pool, req, res, regex) {
  const id = httph.first_match(req.url, regex);
  const items = await select_query(pg_pool, [id]);
...
const select_all_query = query.bind(query, fs.readFileSync('./sql/select_certificates.sql').toString('utf8'), (r) => db_to_response(r));
async function list(filter_orders, pg_pool, req, res /* regex */) {
  const items = await select_all_query(pg_pool, []);
  return httph.ok_response(res, JSON.stringify(items.filter((item) => (!filter_orders && item.status === 'issued' && item.installed)
...
function check_create(payload) {
  ...
  assert.ok(
    payload.common_name && /(^[A-z0-9.\-*]+$)/.exec(payload.common_name) !== null,
    'The domain name was invalid, it must be an alpha numeric string.',
  );
  assert.ok(payload.common_name.length < 65, 'The common domain provided was too long.');
  assert.ok(payload.domain_names
    && !payload.domain_names.some((domain) => domain.length > 64),
  'One or more of the alternative domains was too long.');
  assert.ok(payload.domain_names
    && !payload.domain_names.some((domain_name) => /(^[A-z0-9.\-*]+$)/.exec(domain_name) === null),
...
const insert_query = query.bind(query, fs.readFileSync('./sql/insert_certificate.sql').toString('utf8'), (r) => r);
const insert_names_query = query.bind(query, fs.readFileSync('./sql/insert_certificate_names.sql').toString('utf8'), (r) => r);
async function create(pg_pool, req, res /* regex */) {
  const payload = await httph.buffer_json(req);
  const org = await common.org_exists(pg_pool, payload.org);
  try {
    check_create(payload);
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  const items = await select_query(pg_pool, [payload.name]);
  if (items.length !== 0) {
  ...
  if (!process.env.TEST_MODE) {
    const requested_cert = await common.alamo.certificate.create(
      region.name, payload.comments, payload.common_name, payload.domain_names, payload.requester.name,
    );
  ...
  await Promise.all(payload.domain_names.map(async (domain_name) => (
    insert_names_query(pg_pool, [uuid.v4(), payload.id, domain_name, domain_name === payload.common_name])
  )));
  payload.region = {
    id: region.region,
    name: region.name,
