...
async function info_orders(pg_pool, req, res, regex) {
  ...
  if(!process.env.TEST_MODE) {
    ...
    requested_cert = {"orderstatus":"issued", validfrom:"2016-08-25T18:51:09.371Z", validto:"2016-08-25T18:51:09.371Z"}
  } else {
    requested_cert = {"orderstatus":"pending", validfrom:null, validto:null}
  }
  let dirty = false;
  if(items[0].status !== requested_cert.orderstatus) {
    dirty = true;
    items[0].status = requested_cert.orderstatus;
    items[0].updated_at = (new Date()).toISOString();
  }
  if(items[0].status === "issued" && (!items[0].issued || !items[0].expires)) {
    dirty = true;
    items[0].issued = (new Date(requested_cert.validfrom)).toISOString()
    items[0].expires = (new Date(requested_cert.validto)).toISOString()
  }
  if(dirty) {
...
function check_create(payload) {
  ...
  console.assert(payload.common_name.length < 65, 'The common domain provided was too long.')
...
async function create(pg_pool, req, res, regex) {
  ...
  payload.region = region.region_uuid
  ...
  let item_created = await insert_query(pg_pool, request_to_db(payload))
  payload.domain_names.forEach((domain_name) => {
    insert_names_query(pg_pool, [uuid.v4(), payload.id, domain_name, domain_name === payload.common_name]).catch((err) => {
      console.error('ERROR: An error occurred inserting the domain name:', domain_name, 'on payload', payload);
      console.error('ERROR: This will require manually inserting the domain name for the above record.');
    });
  });
  payload.region = {
