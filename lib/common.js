...
let select_pipeline = query.bind(query, fs.readFileSync('./sql/select_pipeline.sql').toString('utf8'), null);
async function check_pipeline_exists(pg_pool, pipeline_key) {
  let result = await select_pipeline(pg_pool, [pipeline_key])
  if(result.length !== 1) {
    throw new http_help.NotFoundError(`The specified pipeline ${pipeline_key} does not exist.`)
  }
  return result[0]
}
...
module.exports = {
  ...
  pipeline_exists:check_pipeline_exists,
  registry_image,
  encrypt_token,
  decrypt_token,
  notify_hooks,
