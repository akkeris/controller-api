...
const hook_types = [
  ...
  require('./hook-types/sms.js'),
  require('./hook-types/telephone.js'),
  ...
  try {
    ...
    hooks.forEach(async (hook) => {
      if(hook.active === true && hook.url && hook.events.split(',').map((x) => { return x.toLowerCase().trim(); }).indexOf(type.toLowerCase()) !== -1) 
      {
        ...
        let recorded_url = socs({URL:hook.url}).URL;
        ...
        try {
          ...
          insert_hook_result(pg_pool, [id, hook.hook, hook.events, recorded_url, 0, null, JSON.stringify(e), JSON.stringify({}), payload])
          .catch((err) => {
            console.error('Error recording hook result:', err)
            console.error('Data not recorded (during hook error): ', [id, hook.hook, hook.events, recorded_url, 0, JSON.stringify(e), '', JSON.stringify({}), payload]);
          });
        } 
      }
    })
