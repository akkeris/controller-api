...
async function check_deployment_filters(pg_pool, app_uuid, dyno_type, features) {
  ...
  const filters = (await check_filter_attachments(pg_pool, app_uuid))
    .map((x) => {
...
async function trigger_action(pg_pool, app_key, action_key, triggered_by, source) {
  ...
  if (action.formation.options && action.formation.options.image && action.formation.options.image !== '') {
    ...
    if (!latest_image) {
      throw new httph.BadRequestError('The specified app does not have an image. Either create a new release or set an image override on the action.');
    }
  ...
  // If an action is already running, cancel it
  const action_runs = await select_action_runs(pg_pool, [action.action]);
  if (action_runs && action_runs.length > 0) {
    const [run] = action_runs;
    if (run.status === 'running' || run.status === 'starting') {
      update_action_run_status(pg_pool, action.action, run.action_run, 'cancelled', 999, run.started_at, (new Date(Date.now()).toISOString()));
    }
  }
