...
function add_service(pg_pool, services, existing_plans) {
  return services.map((service) => {
    let found_service = service.info()
    let service_exists = false
    try {
      service_by_id_or_name(found_service.id)
      service_exists = true
    } catch (e) {
      // do nothing
    }
    if(service_exists) {
      throw new Error(`Error: the service ${found_service.id} was already registered.`)
    }
    console.log('Registering service ' + service.info().name)
    try { 
      service.timers.begin(pg_pool)
    } catch (e) {
      throw new Error("Unable to start timer for addon: " + e.message);
    }
    ...
    let plans = service.plans().reduce((acc, x) => acc.concat(x.id), [])
    if(existing_plans.filter(plan => plans.includes(plan)).length > 0) {
      throw new Error(`Error: the service ${service.info().id} ${service.info().name} includes one or more plans with an id that already exists ${plans.join(',')}`)
    }
    return service
  })
}
...
async function refresh_services(pg_pool) {
  ...
  try {
    ...
    global.addon_services = services.map((service) => add_service(pg_pool, service, services)).reduce((acc, val) => acc.concat(val), []);
  } catch (e) {
