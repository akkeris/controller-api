...
async function check_topic_exists(pg_pool, topic_key) {
  let result = await select_topic(pg_pool, [topic_key]);
  if (result.length !== 1) {
    throw new http_help.NotFoundError(`The specified topic ${topic_key} does not exist.`);
  }

  return result[0];
}

let select_topic_config = query.bind(query, fs.readFileSync("./sql/select_topic_config.sql").toString('utf8'), null);
async function check_topic_config_exists(pg_pool, topic_key) {
  let result = await select_topic_config(pg_pool, [topic_key]);
  if (result.length !== 1) {
    throw new http_help.NotFoundError(`The specified topic config ${topic_key} does not exist.`);
  }
...
module.exports = {
  ...
  topic_config_exists:check_topic_config_exists,
