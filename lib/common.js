...
function x5cFromPem(pem) {
  return pem.split('\n').filter((x) => x !== "" && !x.startsWith("---")).join('')
}

function signToToken(signature) {
  return signature.signatures[0].protected + 
    "." + signature.payload + 
    "." + signature.signatures[0].signature;
}

function verifyToToken(verify) {
...
async function jwks_verify(pem, intended_issuer, intended_audience, data, signature) {
  ...
  pem = pem.trim();
  let cert = await jose.JWK.asKey(pem, 'pem');
  let full_signature = {
  ...
  try {
    ...
    throw new Error('Invalid format: payload is not a json object.');
  }
  // check standard JWT token parameters
  if(payload.iss && intended_issuer && payload.iss !== intended_issuer) {
    throw new Error('Unauthorized: issuer is invalid');
  }
  if(!payload.exp || payload.exp < (new Date()).getTime()) {
    throw new Error('Unauthorized: token is expired, or has no \"exp\" field.');
  }
  if(payload.aud && intended_audience && payload.aud !== intended_audience) {
    throw new Error('Unauthorized: audience is invalid');
  }
  if(payload.nbf && payload.nbf > (new Date()).getTime()) {
    throw new Error('Unauthorized: token cannot be used yet, now < \"nbf\" field.');
  }
  return payload;
}
...
async function http_jwks_uri(jwt_public_cert, pg_pool, req, res) {
  ...
  let result = {"keys":[await Promise.all(
    pems.split('-----END PUBLIC KEY-----\n')
      .filter((x) => x && x !== null && x !== '')
      .map((x) => x + '-----END PUBLIC KEY-----\n')
      .map(async (pem) => {
        try {
          let jwk = await jose.JWK.asKey(pem, 'pem');
          return {"use":"sig", "x5c":[x5cFromPem(pem)], "alg":"RS256", ...jwk.toJSON()};
        } catch (e) {
          console.error(e);
          return null;
        }
...
const TTL_TEMP_TOKEN = 60/*sec*/ * 60/*min*/; 
async function create_temp_jwt_token(pem, username, audience, issuer, elevated_access, metadata = {}) {
  ...
  let payload = {
    ...
    "exp":(new Date()).getTime() + TTL_TEMP_TOKEN,          // expiration date (https://tools.ietf.org/html/rfc7519#section-4.1.4)
    "nbf":(new Date()).getTime(),                           // token is not valid before (https://tools.ietf.org/html/rfc7519#section-4.1.5)
  ...
  return signToToken(await jwks_sign(pem, payload));
}
...
function decrypt_token(key, enc_token) {
  ...
  let tokenParts = enc_token.replace('salted:', '').split(':');
...
async function notify_hooks(pg_pool, app_uuid, type, payload, username) {
  try {
    ...
    await Promise.all(hooks.map(async (hook) => {
      ...
      try {
        let token = await create_temp_jwt_token(config.jwt_private_key, username, config.akkeris_api_url, config.akkeris_api_url, false, {"hook_id":id, "hook_type":type, "app_uuid":app_uuid}); // never permit elevated access.
        let hook_type = hook_types.filter((x) => x.test(hook.url))[0];
        assert.ok(hook_type, `No valid hook was found for the url ${hook.url}`);
        let {sent_metadata, sent_data, received_code, received_metadata, received_data} = await hook_type.fire(id, hook.url, type, JSON.parse(payload), hmac.update(payload).digest('hex'), token);
