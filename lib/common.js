...
async function jwks_verify(pem, intended_issuer, intended_audience, data, signature) {
  ...
  assert.ok((payload.iss && intended_issuer && payload.iss === intended_issuer) || !intended_issuer || !payload.iss, 'Unauthorized: issuer is invalid');
  assert.ok(payload.exp && payload.exp > (new Date()).getTime(), 'Unauthorized: token is expired, or has no \"exp\" field.');
  assert.ok((payload.aud && intended_audience && payload.aud === intended_audience) || !intended_audience || !payload.aud, 'Unauthorized: audience is invalid');
  assert.ok(payload.nbf && payload.nbf < (new Date()).getTime() || !payload.nbf, 'Unauthorized: token cannot be used yet, now < \"nbf\" field.');
  return payload;
}
    ...
    pems.split('-----END PUBLIC KEY-----\n')
      ...
      .map(async (pem) => {
        try {
          ...
          return {"use":"sig", "x5c":[x5c_from_pem(pem)], "alg":"RS256", ...jwk.toJSON()};
        } catch (e) {
          console.error(e);
          return null;
        }
      ...
      if(x.filter.type === "jwt") {
        ...
        return {
          ...
          "data": {
            ...
            "allow_methods":(x.filter.options.allow_methods || []).join(','),
            "allow_headers":(x.filter.options.allow_headers || []).join(','),
            "expose_headers":(x.filter.options.expose_headers || []).join(','),
            "max_age":x.filter.options.max_age,
            "allow_credentials":x.filter.options.allow_credentials,
          }
        }
      } else {
    ...
    await Promise.all(hooks.map(async (hook) => {
      ...
      try {
        ...
        if (type !== 'destroy' && type !== 'crashed' && type !== 'preview-released') { 
          token = await create_temp_jwt_token(config.jwt_private_key, username, config.akkeris_api_url, config.akkeris_api_url, false, {"hook_id":id, "hook_type":type, app_uuid}); // never permit elevated access.
        }
