    ...
    await Promise.all(hooks.map(async (hook) => {
      ...
      try {
        ...
        if (type !== 'destroy' && type !== 'crashed' && type !== 'preview-released') { 
          token = await create_temp_jwt_token(config.jwt_private_key, username, config.akkeris_api_url, config.akkeris_api_url, false, {"hook_id":id, "hook_type":type, "app_uuid":app_uuid}); // never permit elevated access.
        }
