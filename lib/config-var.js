...
function from_alamo_to_config_var(config_vars_tmp) {
  const config_vars = {};
  if (config_vars_tmp) {
    config_vars_tmp.forEach((x) => {
      config_vars[x.varname] = x.varvalue;
    });
  }
  return config_vars;
}
...
async function get_app_only_config(pg_pool, app_name, space_name) {
  const data = await common.alamo.config.set.request(pg_pool, app_name, space_name);
  return from_alamo_to_config_var(data);
}

async function get_app_config(pg_pool, app_name, space_name, app_uuid) {
  const config_var_set = await get_app_only_config(pg_pool, app_name, space_name);
  const service_data = await addons.service_config_vars(pg_pool, app_uuid, space_name, app_name);
...
function validate_config_var(config_var_name) {
  assert.ok((/^[A-z_0-9]+$/g).exec(config_var_name) !== null,
...
const delete_config_var_notes = query.bind(query, fs.readFileSync('./sql/delete_config_var_notes.sql').toString('utf8'), null);
async function update(pg_pool, app_uuid, app_name, space_name, space_tags, org, config_vars, user) {
  ...
  const existing_config_vars = await common.alamo.config.set.request(pg_pool, app_name, space_name);
  const service_data = await addons.service_config_vars(pg_pool, app_uuid, space_name, app_name);
  const reserved_config_vars = Object.keys(system_config_vars);
  ...
  await Promise.all(Object.keys(config_vars).filter((x) => x !== '').map(async (key) => {
    ...
    } if (config_out[key] || config_out[key] === '') {
      changed = true;
  ...
  if (changed) {
    setTimeout(() => {
      ...
      lifecycle.restart_and_redeploy_app(
        pg_pool, app_uuid, app_name, space_name, org, 'Config Vars Changed',
...
const select_config_var_notes = query.bind(query, fs.readFileSync('./sql/select_config_var_notes.sql').toString('utf8'), null);
const upsert_config_var_notes = query.bind(query, fs.readFileSync('./sql/upsert_config_var_notes.sql').toString('utf8'), null);
async function get_config_var_notes(pg_pool, app_uuid, space_name, app_name) {
  const service_config_info = await addons.service_config_info(pg_pool, app_uuid, space_name, app_name);
  const user_config_vars = await get_app_only_config(pg_pool, app_name, space_name);
  const notes_config_vars = await select_config_var_notes(pg_pool, [app_uuid]);
  const response = JSON.parse(JSON.stringify(system_config_vars));

  service_config_info.forEach((service_info) => {
    const keys = Object.keys(service_info.config_vars);
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      const notes = notes_config_vars.filter((x) => x.key === key)[0];
      if (!response[key]) {
        response[key] = {
          type: 'addon',
          addon: {
  ...
  const keys = Object.keys(user_config_vars);
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    const notes = notes_config_vars.filter((x) => x.key === key)[0];
    if (!response[key]) {
...
async function update_config_var_notes(pg_pool, app_uuid, app_name, space_name, notes) {
  const config_var_notes = await get_config_var_notes(pg_pool, app_uuid, space_name, app_name);
...
async function http_notes_get(pg_pool, req, res, regex) {
  ...
  await common.space_exists(pg_pool, app.space_name);
...
async function http_get(pg_pool, req, res, regex) {
  ...
  const config_var_set = await get_app_config(pg_pool, app.app_name, app.space_name, app.app_uuid);
...
async function http_update(pg_pool, req, res, regex) {
  ...
  const app = await common.app_exists(pg_pool, app_key);
  const space = await common.space_exists(pg_pool, app.space_name);
  const config_vars = await httph.buffer(req);
  return httph.ok_response(res,
    JSON.stringify(await update(
      pg_pool,
      app.app_uuid,
      app.app_name,
      app.space_name,
