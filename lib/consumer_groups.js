...
async function list_consumer_groups(pg_pool, req, res, regex) {
  ...
  let {cluster: cluster_uuid, name: cluster_name, region_name} = await common.cluster_exists(pg_pool, cluster_key);

  let data = await common.alamo.consumer_groups.list(region_name, cluster_name);
...
async function list_consumer_group_offsets(pg_pool, req, res, regex) {
  ...
  let consumer_group_name = httph.second_match(req.url, regex);

  let data = await common.alamo.consumer_groups.offsets(region_name, cluster_name, consumer_group_name);
  return httph.ok_response(res, data);
}
...
async function list_consumer_group_members(pg_pool, req, res, regex) {
  ...
  let consumer_group_name = httph.second_match(req.url, regex);
  let data = await common.alamo.consumer_groups.members(region_name, cluster_name, consumer_group_name);
  return httph.ok_response(res, data);
}
...
async function seek(pg_pool, req, res, regex) {
  ...
  let consumer_group_name = httph.second_match(req.url, regex);
  let data = await common.alamo.consumer_groups.seek(region_name, cluster_name, consumer_group_name, payload);
  return httph.ok_response(res, data);
}
