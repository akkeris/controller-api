...
async function createDiagnostic(pg_pool, req, res, regex) {
  ...
  if (!payload.command || payload.command === "") {
  ...
  const db_record = (await insert_diagnostic(pg_pool, diagnostic))[0];
...
async function deleteDiagnostic(pg_pool, req, res, regex) {
  ...
  await delete_diagnostic(pg_pool, [diagnostic.diagnostic_uuid]);
  ...
  return httph.ok_response(res, JSON.stringify({
    "id": diagnostic.diagnostic_uuid,
    "name": `${diagnostic.name}-${diagnostic.space}`, 
    "result": "successful"
  }));
}
...
async function updateDiagnostic(pg_pool, req, res, regex) {
  ...
  const [ rawDiagnostic ] = await select_raw_diagnostic(pg_pool, [diagnostic.diagnostic_uuid]);
  ...
  payload.app = payload.app ? (await common.app_exists(pg_pool, payload.app)).app_uuid : null;
  const pipeline = payload.pipeline ? (await common.pipeline_exists(pg_pool, payload.pipeline)).pipeline : null;

  if (payload.pipeline && payload.pipeline !== '' && ( !diagnostic.transitionfrom && !payload.transitionfrom) ) {
    throw new httph.UnprocessibleEntityError("transitionfrom is required when specifying a pipeline");
  }

  if (payload.pipeline && payload.pipeline !== '' && ( !diagnostic.transitionto && !payload.transitionto) ) {
  ...
  if (typeof payload.startdelay === 'undefined' && payload.startdelay === '') {
    payload.startdelay = null;
  } else {
    payload.startdelay = payload.startdelay || diagnostic.startdelay;
  }

  if (typeof payload.command === 'undefined' && payload.command === '') {
    payload.command = null;
  } else {
    payload.command = payload.command || diagnostic.command;
  }

  const updatedDiagnostic = [
    diagnostic.diagnostic_uuid,
    payload.app || diagnostic.app_uuid,
