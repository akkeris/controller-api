...
async function restart_dyno_type(pg_pool, req, res, regex) {
  ...
  if(formation.length === 0) {
    ...
    if(dyno) {
      response.dyno = dyno;
      if(why) {
        logs.event(pg_pool, app.app_name, app.space_name, 'Restarting ' + formation[0].type + '.' + dyno + ' (User Requested)')
      }
      await common.alamo.dyno.stop(pg_pool, app.space_name, common.alamo.app_name(app.app_name, formation[0].type), dyno)
    } else {
      if(why) {
        logs.event(pg_pool, app.app_name, app.space_name, 'Restarting ' + formation[0].type + ' (User Requested)')
      }
      await common.alamo.dyno.restart(pg_pool, app.space_name, common.alamo.app_name(app.app_name, formation[0].type))
    }
...
async function restart_app(pg_pool, req, res, regex) {
  ...
  if(why) {
    logs.event(pg_pool, app_name, space_name, 'Restarting (User Requested)')
  }
