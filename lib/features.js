...
async function update_feature(pg_pool, app_uuid, app_name, space_name, feature_key, enabled, req) {
...
async function http_update(pg_pool, req, res, regex) {
  ...
  let feature = await update_feature(pg_pool, app.app_uuid, app.app_name, app.space_name, feature_key, payload.enabled, req)
