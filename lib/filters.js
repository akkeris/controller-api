...
function to_filter_data(payload) {
  if(payload.type === "jwt") {
    ...
    assert.ok(payload.options.allow_methods.every((x) => /[A-Za-z0-9\:]+/.test(x)), 'One or more allowed methods has an invalid value. Regular expressions are not allowed.');
    assert.ok(payload.options.allow_headers.every((x) => /[A-Za-z0-9\:\-]+/.test(x)), 'One or more allowed headers has an invalid value. Regular expressions are not allowed.');
    assert.ok(payload.options.expose_headers.every((x) => /[A-Za-z0-9\:\-]+/.test(x)), 'One or more exposed headers has an invalid value. Regular expressions are not allowed.');
    assert.ok(payload.options.allow_origin.every((x) => /[A-Za-z0-9\:\*]+/.test(x)), 'One or more allowed origins has an invalid value. Regular expressions are not allowed.');
    assert.ok(!payload.options.max_age || (typeof payload.options.max_age === 'number' && payload.options.max_age > 0), 'The max_age parameter must be a positive number')
...
async function http_filters_update(pg_pool, req, res, regex) {
  ...
  let options = {}
  try {
    assert.ok(payload.type && (payload.type === "jwt" || payload.type === "cors") || !payload.type, 'The filters type was not provided or was an invalid value.')
    options = to_filter_data(payload);
  } catch (e) {
    throw new common.BadRequestError(e.message)
  }
...
async function http_filter_create_attachment(pg_pool, req, res, regex) {
  ...
  try {
    ...
    filter = filter[0];
    let attachments = await common.filter_attachments_exists(pg_pool, app.app_uuid);
