...
async function update_dyno(pg_pool, app_uuid, app_name, space_name, form) {
  ...
  if(typeof (form.quantity) !== 'undefined' && form.quantity !== null) {
    logs.event(app_name, space_name, "Changing dynos on " + formation[0].type + " to " + form.quantity + " dynos of " + formation[0].size)
  ...
  if(form.port && formation[0].type === 'web') {
    logs.event(app_name, space_name, "Changing active port on " + formation[0].type + " to " + form.port)
  ...
  if(form.healthcheck && formation[0].type === 'web') {
    logs.event(app_name, space_name, "Changing dyno healthcheck  on " + formation[0].type + " to " + form.healthcheck)
...
async function formation_batch_update(pg_pool, req, res, regex) {
  ...
  let results = [];
  for(let i=0; i < inbound.length; i++) {
    results.push(await update_dyno(pg_pool, app.app_uuid, app.app_name, app.space_name, inbound[i]));
  }
  setTimeout(() => {
