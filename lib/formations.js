...
function formation_payload_check(payload, sizes) {
  ...
  if (payload.oneoff && payload.options && payload.options instanceof Object && Object.keys(payload.options).length > 0) {
    if (payload.options.image || payload.options.image === '') {
      assert.ok(
        typeof payload.options.image === 'string' && payload.options.image.trim().length > 0,
      ...
      const tagMatch = /^(.*):([a-zA-Z0-9._-]+)$/.exec(payload.options.image.trim());
    ...
    if (payload.options.env || payload.options.env === '') {
      ...
      assert.ok(payload.options.env instanceof Object, 'The env option must be an object consisting of at least one key-value pair.');
      assert.ok(Object.keys(payload.options.env).length > 0, 'The env option must be an object consisting of at least one key-value pair.');
      Object.keys(payload.options.env).forEach((key) => {
        assert.ok(
          typeof payload.options.env[key] === 'string',
