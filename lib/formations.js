...
async function restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org_name, why) {
  ...
  for(let i=0; i < formations.length; i++) {
    ...
    await common.alamo.deploy(pg_pool, space_name, app_name, form.type, image, form.command, form.port, form.healthcheck, await features.deployment_features(pg_pool, app_uuid, form.type), labels)
  }
  if (why) {
...
async function formation_delete(pg_pool, req, res, regex) {
  ...
  let app = await common.app_exists(pg_pool, app_key)
  if(app.disabled) {
    throw new common.ConflictError("An apps formation can't be changed while its been placed in maintenance mode.")
  }
...
async function web_formation_create(pg_pool, req, res, regex) {
  ...
  if(app.disabled) {
    throw new common.ConflictError("An apps formation can't be changed or created while its been placed in maintenance mode.")
  }
