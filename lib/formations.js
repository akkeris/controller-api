...
async function create_dyno(pg_pool, app_uuid, app_name, space_name, type, command, quantity, size, port, healthcheck, user, oneoff = false, oneoff_options) {
...
function formation_payload_check(payload, sizes) {
  ...
  assert.ok(payload.type === 'web' || (payload.type !== 'web' && payload.command !== '' && payload.command),
    'A run command must be specified for any type outside of a web process.');
  assert.ok(payload.oneoff || (!payload.oneoff && typeof (payload.oneoff_options) !== 'undefined'),
