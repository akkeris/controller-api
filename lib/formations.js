...
function postgres_to_payload(result) {
  return {
    ...
    // Only add options for oneoff dynos
    ...(result.oneoff && result.oneoff === true && { oneoff: result.oneoff, options: result.options }),
...
function check_image(image) {
  ...
  const tagMatch = /^(.*):([a-zA-Z0-9._-]+)$/.exec(image.trim());
  assert.ok(tagMatch && !!tagMatch[1], 'A tag must be specified for the one-off image option, but no tag was found.');
}

function check_env(env) {
  // Make sure that env follows this forrmat: { "key": "value" }
  assert.ok(env instanceof Object, 'The env option must be an object consisting of at least one key-value pair.');
  assert.ok(Object.keys(env).length > 0, 'The env option must be an object consisting of at least one key-value pair.');
  Object.keys(env).forEach((key) => {
    assert.ok(
      typeof env[key] === 'string',
      'Invalid env value: env option must be an object consisting of at least one key-value pair.',
    );
  });
}
...
function formation_payload_check(payload, sizes) {
  assert.ok(typeof (payload.size) === 'undefined' || sizes.indexOf(payload.size) !== -1,
    'The formation size was not recognized.');
  ...
  assert.ok(!payload.oneoff || !payload.instances || payload.instances === 0,
    'Invalid number of instances for one-off formation - must be null or 0');
  assert.ok(
    !payload.options
    || (payload.options && payload.options instanceof Object),
    'The options specified were not recognized',
  );
  if (payload.oneoff && payload.options && payload.options instanceof Object && Object.keys(payload.options).length > 0) {
    if (payload.options.image || payload.options.image === '') {
      check_image(payload.options.image);
    }
    if (payload.options.env || payload.options.env === '') {
      check_env(payload.options.env);
    }
  }
}
...
async function create(
  ...
  try {
    ...
    payload = {
      type, command, quantity, port, size, healthcheck, removeHealthcheck: remove_health_check, oneoff, options,
    };
  ...
  return create_dyno(
    pg_pool, app_uuid, app_name, space_name, type, command, quantity, size, port, healthcheck, user, oneoff, options,
  );
}

const update_oneoff_formation = query.bind(query, fs.readFileSync('./sql/update_oneoff_formation.sql').toString('utf8'), (r) => r);
async function oneoff_update(pg_pool, app_uuid, space_name, formation_uuid, name, size, command, options) {
  const formation = (await select_formation(pg_pool, [app_uuid, formation_uuid]))[0];
  assert(formation, 'The specified formation does not exist');

  const payload = {};

  // If given update value exists, check and add to payload.
  ...
  if (size && size !== '') {
    const sizes = await sizes_to_enum(pg_pool, space_name);
    assert.ok(sizes.indexOf(size) !== -1, 'The formation size was not recognized.');
    payload.size = size;
  }
  ...
  payload.options = formation.options || {};

  if (options && options.image !== null && typeof options.image === 'string') {
    // If empty string, skip check and clear image override
    if (options.image !== '') {
      check_image(options.image);
      payload.options.image = options.image;
    } else {
      payload.options.image = undefined;
    }
  }

  // The environment will be replaced with what's in the payload, if provided.
  // This is a full update, not an individual patch per env var.
  // Might take another look at this in the future

  if (options && options.env && typeof options.env === 'object') {
    // If empty object, skip check and clear environment variables
    if (Object.keys(options.env).length !== 0) {
      check_env(options.env);
      payload.options.env = options.env;
    } else {
      payload.options.env = undefined;
    }
  }

  // Null/undefined values won't be updated (handled in query)
  await update_oneoff_formation(pg_pool, [formation_uuid, payload.name, payload.size, payload.command, payload.options]);
}
