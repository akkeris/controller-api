...
// private - DOES NOT RECORD CHANGES, THIS MUST RECORD THE PORT CHANGE
//         - PRIOR TO ISSUING A CHANGE INTO THE CONFIG.
async function randomly_change_env(pg_pool, name, space) {
  return await common.alamo.config.update(pg_pool, name, space, 'RESTART', 'restart' + Math.floor(Math.random() * 10000000))
}

// public
const select_formations = query.bind(query, fs.readFileSync("./sql/select_formations.sql").toString('utf8'), (x) => { return x; });
async function list(pg_pool, app_name, space_name) {
  return await select_formations(pg_pool, [app_name + '-' + space_name]);
}
...
async function restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org_name, why) {
  let formations = await list(pg_pool, app_name, space_name);
  let image = await select_latest_image(pg_pool, [app_uuid]);
  if (image.length === 0) {
    return;
  }
  image = image[0];
  image = common.registry_image(image.build_org_name, image.build_app_name, image.build_app, image.foreign_build_key)
  await randomly_change_env(pg_pool, app_name, space_name);
  for(let i=0; i < formations.length; i++) {
    let form = formations[i]
    await common.alamo.deploy(pg_pool, space_name, app_name, form.type, image, form.command, form.port, form.healthcheck)
  }
  if (why) {
...
async function delete_dynos(pg_pool, app_uuid, app_name, space_name) {
  let forms = await select_formations(pg_pool, [`${app_name}-${space_name}`]);
  let errors = [];
...
async function update_dyno(pg_pool, app_uuid, app_name, space_name, form) {
  ...
  if(formation[0].type === 'web' && existing_formation[0].command && form['remove-command']) {
    logs.event(app_name, space_name, "Changing startup command on web to default")
  ...
  if(form.command && form.command !== existing_formation[0].command) {
    logs.event(app_name, space_name, "Changing startup command on " + formation[0].type + " to " + form.command)
  }
  if(typeof (form.quantity) !== 'undefined' && form.quantity !== null) {
    logs.event(app_name, space_name, "Scaling dynos on " + formation[0].type + " to " + form.quantity)
  ...
  if(form.port && formation[0].type === 'web') {
    logs.event(app_name, space_name, "Changing application port to " + form.port)
    await common.alamo.dyno.change_port(pg_pool, app_name, space_name, form.port)
  }
  if(form.size) {
    logs.event(app_name, space_name, "Changing dyno plan on " + formation[0].type + " to " + formation[0].size)
  ...
  if(form.healthcheck && formation[0].type === 'web') {
    logs.event(app_name, space_name, "Changing application health check to " + form.healthcheck)
    ...
    logs.event(app_name, space_name, "Removing application health check")
...
async function formation_info(pg_pool, req, res, regex) {
...
// public
async function formation_list(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  let formations = await select_formations(pg_pool, [`${app.app_name}-${app.space_name}`])
  return httph.ok_response(res, JSON.stringify(formations.map(postgres_to_payload)))
}

// public
async function formation_delete(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let formation_key = httph.second_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  if(app.disabled) {
    throw new common.ConflictError("An apps formation can be changed while its been placed in maintenance mode.")
  }
...
// public
async function formation_size_list(pg_pool, req, res) {
  return httph.ok_response(res, await common.alamo.sizes(pg_pool));
}

// public
function formation_payload_check(payload, sizes) {
  console.assert(typeof (payload.size) === 'undefined' || sizes.indexOf(payload.size) !== -1,
    'The payload size was not recognized.');
  console.assert(typeof (payload.type) === 'undefined' || payload.type && /(^[A-z0-9]+$)/.exec(payload.type) !== null,
    'The type specified was invalid, it cannot contain spaces or special characters (alpha numeric only)');
  console.assert(typeof (payload.quantity) === 'undefined' || Number.isInteger(payload.quantity) && payload.quantity > -1 && payload.quantity < 33,
    'The number of instances must be between 0 and 32.');
  console.assert(typeof (payload.port) === 'undefined' || payload.port === null || (payload.port && Number.isInteger(payload.port) && payload.port > 1023 && payload.port < 65536),
    'The specified port is invalid, it must be a number between 1024 and 65535.');
  console.assert(payload.type === 'web' || (payload.type !== 'web' && !payload.port),
    'A port was specified for a non-web based application, the port should not be set.');
  console.assert(payload.type === 'web' || (payload.type !== 'web' && !payload.healthcheck),
    'A healthcheck was specified for a non-web based application, the path should not be set.');
  console.assert(payload.type === 'web' || (payload.type !== 'web' && !payload.removeHealthcheck),
    'Cannot remove healthcheck on non web based dyno.');
}

// public
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org_name, type, size, quantity, command, port, healthcheck, remove_health_check) {
  ...
  try {
    ...
    console.assert(type === 'web' || type !== 'web' && command !== '' && command,
  ...
  if (formation.length !== 0) {
    throw new common.UnprocessibleEntityError(`The process of type ${type} already exists.`);
  }
  ...
  if(type !== 'web') {
    if((await select_formation(pg_pool, [app_uuid, "web"])).length === 0) {
       await create_dyno(pg_pool, app_uuid, app_name, space_name, "web", null, 0, config.dyno_default_size, config.default_port, null);
    }
  }

  formation = await create_dyno(pg_pool, app_uuid, app_name, space_name, type, command, quantity, size, port, healthcheck);
  if (type !== 'web') {
    setTimeout(() => {
      restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org_name, 'Formation Changed').catch(console.error.bind(console));
      common.notify_hooks(pg_pool, app_uuid, 'formation_change', JSON.stringify({
        'action': 'formation_change',
        'app': {
          'name': app_name,
          'id': app_uuid
...
// public
async function web_formation_create(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  ...
  let formation = await create(pg_pool, app.app_uuid, app.app_name, app.space_name, space.tags, app.org_name, payload.type, payload.size, payload.quantity, payload.command, payload.port, payload.healthcheck, payload.removeHealthcheck)
  return httph.created_response(res, JSON.stringify(formation))
}

// public
async function formation_batch_update(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  let space = await common.space_exists(pg_pool, app.space_name)
  ...
  try {
    console.assert(Array.isArray(inbound), 'The payload was not an array of formation changes.');
    ...
    for (let i = 0; i < inbound.length; i++) {
      ...
      console.assert(inbound[i].type, 'The type specified was invalid, it cannot contain spaces or special characters (alpha numeric only)');
      formation_payload_check(inbound[i], size_names);
    }
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message)
  }
  let results = await Promise.all(inbound.map(async (form) => await update_dyno(pg_pool, app.app_uuid, app.app_name, app.space_name, form)));
  setTimeout(() => {
    restart_and_redeploy_app(pg_pool, app.app_uuid, app.app_name, app.space_name, app.org_name, 'Formation Changed').catch(console.error.bind(console))
    common.notify_hooks(pg_pool, app.app_uuid, 'formation_change', JSON.stringify({
      'action': 'formation_change',
      'app': {
        'name': app.app_name,
...
// public
async function formation_update(pg_pool, req, res, regex) {
  let app_key = httph.first_match(req.url, regex)
  let formation_key = httph.second_match(req.url, regex)
  let app = await common.app_exists(pg_pool, app_key)
  let space = await common.space_exists(pg_pool, app.space_name)
...
module.exports = {
  ...
  update: formation_update,
  delete: formation_delete,
  batch_update: formation_batch_update,
  create,
  http:{
    create:web_formation_create
  },
  create_dyno,
  delete_dynos,
  restart_and_redeploy_app,
  list_types: list,
  sizes: formation_size_list,
}