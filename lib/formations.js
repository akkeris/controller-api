...
async function create(
  ...
  try {
    ...
    payload = {
      type, command, quantity, port, size, healthcheck, removeHealthcheck: remove_health_check,
    };
  ...
  return create_dyno(pg_pool, app_uuid, app_name, space_name, type, command, quantity, size, port, healthcheck, user);
}
...
async function http_update(pg_pool, req, res, regex) {
  ...
  const form = await update_dyno(pg_pool, app.app_uuid, app.app_name, app.space_name, payload);
  await update_app_updated_at(pg_pool, [app.app_uuid]);
  setTimeout(() => {
    lifecycle.restart_and_redeploy_app(pg_pool, app.app_uuid, app.app_name, app.space_name, app.org_name, 'Formation Changed')
      .catch(console.error.bind(console));
