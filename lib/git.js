...
async function remove_webhook_if_needed(app_key, repo, token) {
  ...
  let repo_url = new url.URL(http_help.clean_forward_slash(repo))
  let github_hooks_api = 'https://api.github.com/repos' + repo_url.pathname + '/hooks';
  ...
  let hook = hooks.filter((hook) => { return hook.config.url === `${config.akkeris_app_controller_url}/apps/${app_key}/builds/auto/github`; }); 
  if(hook.length !== 0) {
    try {
...
async function create_webhook_if_needed(app_key, repo, token, user) {
  ...
  let repo_url = new url.URL(http_help.clean_forward_slash(repo))
  // list webhooks
  let alamo_hook_url = `${config.akkeris_app_controller_url}/apps/${app_key}/builds/auto/github`;
  let github_hooks_api = 'https://api.github.com/repos' + repo_url.pathname + '/hooks';
  let github_headers = {'user-agent':'akkeris-controller-api', 'authorization':'token ' + token, 'x-silent-error':true};
  let hooks = null
  try {
...
async function http_github_delete_auto_build(pg_pool, req, res, regex) {
  ...
  try {
    ...
    if (err.code === 401) {       
      ...
      throw new http_help.InternalServerError(`The cached Github token was invalid: please manually remove any Akkeris hooks from the ${query_data[0].repo} repository`)
    } else { 
      // Something else weird happened
      console.error("error removing auto build webhook from Github:", err)
...
async function create_build_from_scm(pg_pool, app_uuid, repo, branch, token) { 
  ...
  let headers = {'user-agent':'akkeris-controller-api', 'x-silent-error':true, 'authorization':`token ${token}`};

  let repo_org_and_name = new url.URL(http_help.clean_forward_slash(repo))

  let branch_response = await http_help.request('get', `https://api.github.com/repos${repo_org_and_name.pathname}/branches/${branch}`, headers, null);
  let {commit:{sha, commit:{url: link_url, message, author:{name:author}}}} = JSON.parse(branch_response);

  let content_url = `https://api.github.com/repos${repo_org_and_name.pathname}/zipball/${branch}`;
  let content_response = await http_help.request('get', content_url, {'x-response':true, ...headers}, null);
  content_url = content_response.headers['location'] || content_url;
  return await builds.create(pg_pool, 
...
async function http_github_create_auto_build(pg_pool, req, res, regex) {
  ...
  let created = await create_webhook_if_needed(`${app.app_name}-${app.space_name}`, data.repo, data.token, data.username)
...
async function should_create_preview_app(pg_pool, app, payload) {
  ...
  return existing_apps.length === 0 && (await common.feature_enabled(pg_pool, app.app_uuid, "preview"))
}
...
async function http_github_webhook_received(pg_pool, req, res, regex) {
  ...
  authorizations = authorizations.filter((record) => {
    return format_git_repo_url(record.repo) === format_git_repo_url(payload.repo) &&
...
async function create_deployment_status(token, org_repo, deployment_id, state, environment, environment_url, description, log_url) {
  ...
  try {
    await http_help.request('post', 
      ...
      {'user-agent':'akkeris-controller-api', 'authorization':`token ${token}`, 'accept': 'application/vnd.github.flash-preview+json', 'x-silent-error':true}, 
      JSON.stringify({
        state,
        log_url,
        description,
...
async function create_deployment(token, org_repo, ref, environment, payload, description, transient_environment, production_environment) {
  ...
  return JSON.parse(await http_help.request('post', 
    ...
    {'user-agent':'akkeris-controller-api', 'authorization':`token ${token}`, 'accept': 'application/vnd.github.ant-man-preview+json', 'x-silent-error':true}, 
    JSON.stringify({
      ref,
      "task":"deploy",
      "auto_merge":false, 
...
function map_github_url_to_org_repo(github_url) {
  return format_git_repo_url(github_url).replace(/https\:\/\//, '').replace(/http\:\/\//, '').split('/').slice(1,3).join('/')
}
...
const select_release_scm_metadata = query.bind(query, fs.readFileSync('./sql/select_release_scm_metadata.sql').toString('utf8'), (d) => { return d; });
const update_release_scm_metadata = query.bind(query, fs.readFileSync('./sql/update_release_scm_metadata.sql').toString('utf8'), (d) => { return d; });

async function get_deployments(pg_pool, release_id) {
  let deployments = await select_release_scm_metadata(pg_pool, [release_id])
  assert.ok(deployments.length === 1, `The release id provided to get_deployments was invalid ${release_id}`)
...
async function add_deployment(pg_pool, release_id, environment, environment_url, org_repo, description, log_url, deployment) {
  assert.ok(release_id, 'The release id was not provided to add_deployment')
  ...
  let deployments = (await get_deployments(pg_pool, release_id)).concat([Object.assign({environment, environment_url, org_repo, description, log_url}, deployment)])
  return await update_release_scm_metadata(pg_pool, [release_id, JSON.stringify(deployments, null, 2)])
}
...
async function get_token(pg_pool, app_uuid) {
  ...
  let source_app = await previews.source_app(pg_pool, app_uuid)
  if (source_app !== null) {
...
async function update_commit_status_for_build(pg_pool, payload) {
  try {
    if(payload.build && payload.build.commit && payload.build.branch) {
      ...
      let org_repo = map_github_url_to_org_repo(payload.build.repo)
      let token = await get_token(pg_pool, payload.app.id)
      if(token === null) {
        // No authorization
        return
      }
...
// private
async function create_deployments_for_release(pg_pool, payload) {
  try {
    if(payload.build && payload.build.commit) {
      ...
      let org_repo = map_github_url_to_org_repo(payload.build.repo)
      let app = await common.app_exists(pg_pool, payload.app.id)
      let space = await common.space_exists(pg_pool, payload.space.name)
      let description = `Deployment pending`
      let environment = `${payload.app.name}-${payload.space.name}`
      let production_environment = space.tags.indexOf('compliance=prod') > -1 ? true : false
      let transient_environment = (await previews.source_app(pg_pool, payload.app.id)) === null ? false : true // This effectively means if we're a preview app.
      let environment_url = app.url
      let log_url = `${config.akkeris_ui_url}/apps/${payload.app.name}-${payload.space.name}`
      let token = await get_token(pg_pool, payload.app.id)
      if(token === null) {
        // No authorization
        return
      ...
      await add_deployment(pg_pool, payload.release.id, environment, environment_url, org_repo, description, log_url,
        await create_deployment(token, org_repo, payload.build.commit, environment, payload, description, transient_environment, production_environment))

      // Create deployments for all the sites we're in.
      let unique_site_names = (await routes.list(pg_pool, [payload.app.id]))
        .filter((s) => s.site.preview === null && !transient_environment || s.site.preview !== null && transient_environment)
      ...
      await Promise.all(unique_site_names.map(async (site) => {
        return await add_deployment(pg_pool, payload.release.id, site, `https://${site}`, org_repo, description, log_url,
          await create_deployment(token, org_repo, payload.build.commit, site, payload, description, transient_environment, production_environment))
      }))
    }
  } catch (e) {
    console.error(e.message)
...
async function update_deployments_for_release(pg_pool, payload) {
  try {
    ...
    if(payload.release && payload.release.id) {
      ...
      let deployments = await get_deployments(pg_pool, payload.release.id)
      ...
      let description = `Deployment ${state}`
      let token = await get_token(pg_pool, payload.app.id)
      if(token === null) {
        // No authorization
        return
      ...
      await Promise.all(deployments.map(async (deployment) => {
        return await create_deployment_status(token, 
          deployment.org_repo, deployment.id, state, deployment.environment, deployment.environment_url, description, deployment.log_url)
      }))
    }
...
function init(pg_pool) {
  ...
  common.lifecycle.on('release', create_deployments_for_release.bind(null, pg_pool))
