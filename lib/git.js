...
async function create_build_from_scm(pg_pool, app_uuid, repo, branch, token) { 
  ...
  let repo_org_and_name = url.parse(http_help.clean_forward_slash(repo));

  let branch_response = await http_help.request('get', `https://api.github.com/repos${repo_org_and_name.path}/branches/${branch}`, headers, null);
  let {commit:{sha, commit:{url: link_url, message, author:{name:author}}}} = JSON.parse(branch_response);

  let content_url = `https://api.github.com/repos${repo_org_and_name.path}/zipball/${branch}`;
