...
const select_release_scm_metadata = query.bind(query, fs.readFileSync('./sql/select_release_scm_metadata.sql').toString('utf8'), (d) => d);
const update_release_scm_metadata = query.bind(query, fs.readFileSync('./sql/update_release_scm_metadata.sql').toString('utf8'), (d) => d);

async function get_deployments(pg_pool, release_id) {
  const deployments = await select_release_scm_metadata(pg_pool, [release_id]);
...
async function add_deployment(
  ...
  const deployments = (await get_deployments(pg_pool, release_id)).concat([{
    environment, environment_url, org_repo, description, log_url, ...deployment,
  }]);
  return update_release_scm_metadata(pg_pool, [release_id, JSON.stringify(deployments, null, 2)]);
}
...
async function create_deployments_for_release(pg_pool, payload) {
  try {
    if (payload.build && payload.build.commit) {
      ...
      await Promise.all(unique_site_names.map(async (site) => add_deployment(
        pg_pool,
        payload.release.id,
        site,
        `https://${site}`,
