...
async function create_deployment(app_name, space_name, token, org_repo, ref) {
  assert.ok(app_name, 'The app name was not specified in create_deployment.')
  assert.ok(space_name, 'The space name was not specified in create_deployment.')
...
async function create_deployment_status(app_name, space_name, token, org_repo, deployment_id, state, web_url) {
  ...
  assert.ok(token, 'The token was not specified in create_deployment.')
  assert.ok(org_repo, 'The organization and repo was not specified in create_deployment.')
  assert.ok(deployment_id, 'The deployment_id was not specified in create_deployment.')
  assert.ok(state, 'The state was not specified in create_deployment.')
  assert.ok(web_url, 'The web_url was not specified in create_deployment.')
  if(process.env.TEST_MODE) {
