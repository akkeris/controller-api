...
async function http_github_delete_auto_build(pg_pool, req, res, regex) {
  ...
  let response_message = '{"status":"successful"}';
  try {
    ...
    if (err.code === 401) {
      ...
      response_message = `{"status":"error", "message":"Auto build removed, but the cached Github token was invalid- please manually remove any Akkeris hooks from the ${query_data[0].repo} repository"}`
    } else { 
