...
async function create_deployment(app_name, space_name, token, org_repo, ref) {
  ...
  let github_headers = {'user-agent':'akkeris-controller-api', 'authorization':`token ${token}`, 'accept': 'application/vnd.github.ant-man-preview+json'}
  let payload = {
    "ref":ref,
    "task":"deploy",
    "auto_merge":false, 
...
async function should_kill_preview_app(pg_pool, app, payload) {
  ...
  let existing_apps = (await previews.list(pg_pool, app.app_uuid)).filter((papp) => { 
    return app.foreign_key.toString() === payload.pull_request.head.ref.toString() && app.app_uuid === papp.source
  })
  ...
  if(await should_kill_preview_app(pg_pool, app, payload)) {
    try {
      ...
      if(process.env.CANARY_MODE) {
        return console.log(`We would have deleted ${preview_apps[0].target} based on a PR closing.`);
      }
      // DO NOT DELETE app, DELETE THE PREVIEW APP app CREATED.
      await previews.delete_by_target(pg_pool, preview_apps[0].target)
      await apps.delete(pg_pool, existing_apps[0].target)
    } catch (e) {
      console.error("Error: failed to remove preview app after PR closed:")
      console.error(app)
    }
    ...
    console.assert(payload.pull_request.head.sha, 'The sha identifier was not presnet on the incoming payload.')
    try {
      let rec = await previews.create(pg_pool, app.app_uuid, payload.source_branch, payload.pull_request.head.sha.substring(0,5), `Created from ${payload.repo} PR #${payload.pull_request.number}`)
      console.log(`Created preview app from ${app.app_uuid} ${app.app_name}-${app.space_name} -> ${payload.pull_request.head.sha.substring(0,5)}`)
    } catch (e) {
...
async function notify_release(pg_pool, key, description, auto_release) {
  if(auto_release.token && auto_release.repo && auto_release.sha) {
    ...
    try {
      Promise.all((await previews.list(pg_pool, auto_release.app)).map(async (preview) => {
        return create_deployment_status(auto_release.app_name, auto_release.space_name, token, repo_url.path, preview.foreign_status_key, key, auto_release.url)
      }))
    } catch (e) {
