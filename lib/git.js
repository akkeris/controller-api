...
async function get_deployments_from_build(pg_pool, build_id) {
  const found_builds = await select_build_scm_metadata(pg_pool, [build_id]);
  assert.ok(found_builds.length === 1, `The build id provided to get_deployments_from_build was invalid ${build_id}`);
  const { scm_metadata } = found_builds[0];
  if (!scm_metadata) {
    return [];
  }
  return JSON.parse(scm_metadata);
}

async function add_deployment_to_build(
  pg_pool,
  build_id,
  environment,
  environment_url,
  ...
  assert.ok(build_id, 'The build id was not provided to add_deployment_to_build');
  assert.ok(environment, 'The environment was not provided to add_deployment_to_build');
  assert.ok(environment_url, 'The environment_url was not provided to add_deployment_to_build');
  assert.ok(org_repo, 'The org_repo was not provided to add_deployment_to_build');
  assert.ok(deployment, 'The deployment was not provided to add_deployment_to_build');
  const deployments = (await get_deployments_from_build(pg_pool, build_id)).concat([{
...
async function create_deployments_for_build(pg_pool, payload) {
  try {
    if (payload.build && payload.build.commit && payload.build.result === 'pending') {
      ...
      await Promise.all(unique_site_names.map(async (site) => {
        ...
        return add_deployment_to_build(
          pg_pool,
          payload.build.id,
          site,
          `https://${site}`,
...
// private
async function update_deployments_for_build(pg_pool, payload) {
  try {
    if (payload.build && payload.build.result !== 'succeeded' && payload.build.result !== 'failed') {
      // No change in status to report
      return;
    }
    assert.ok(payload.build, 'No build was found');
    assert.ok(payload.build.result, 'No build result was found');
    assert.ok(payload.build.id, 'No build id was found');
    assert.ok(payload.app, 'No app object was found');
    assert.ok(payload.app.id, 'No app id was found');
    const deployments = await get_deployments_from_build(pg_pool, payload.build.id);
    const state = payload.build.result === 'succeeded' ? 'success' : 'failure';
...
async function update_deployments_for_release(pg_pool, payload) {
  try {
    ...
    if (payload.release && payload.release.id) {
      ...
      assert.ok(payload.app, 'No app object was found');
      assert.ok(payload.app.id, 'No app id was found');
      const deployments = await get_deployments_from_build(pg_pool, payload.release.id);
...
function init(pg_pool) {
  ...
  common.lifecycle.on('build', update_deployments_for_build.bind(null, pg_pool));
