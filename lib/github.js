...
async function github_remove_webhook_if_needed(app_key, repo, token) {
  ...
  // list webhooks
  let alamo_base = process.env.TEST_MODE ? 'http://alamo.trevor.ultrahook.com' : config.alamo_app_controller_url;
...
async function github_auto_build_check(pg_pool, req, regex) {
  ...
  try {
    ...
    console.assert(data.repo.startsWith("http"), 'The "repo" field was not a http or https url, note ssh is not supported.')
...
async function github_auto_build(pg_pool, req, res, regex) {
  ...
  let auto_build_params = [auto_build_uuid, app.app_uuid, new Date(), new Date(), data.repo, data.branch, authorization_uuid, data.auto_deploy, data.status_check, 'aka', false, validation_token]
  await insert_auto_build(pg_pool, auto_build_params)
...
function add_status(status, app_name, space_name, enc_token, repo, sha, description) {
  ...
  let payload = {
    'state':status, 
    'target_url':'https://ui.appkit.sh/#/apps/' + app,
    'description':description,
    'context':'alamo/appkit'
  };
