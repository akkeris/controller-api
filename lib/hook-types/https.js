...
function test(hook_url) {
  if (hook_url.toLowerCase().startsWith('https://')) {
    return true;
  } if (hook_url.toLowerCase().startsWith('http://')) {
    return true;
  }
  return false;
}
...
function fire(id, hook_url, type, payload, hmac, headers = {}, token = null) {
  return new Promise((response, reject) => {
    if (!test(hook_url)) {
      reject(new Error('unable to fire webhook:', hook_url, 'as http or https'));
      return;
    }
    const options = {};
    ...
    const req = (hook_url.startsWith('https://') ? https : http).request((new url.URL(hook_url)), options, (res) => {
      ...
      res.on('data', (chunk) => {
        if (!recorded_result) {
          if (data.length > halfMb || chunk > halfMb) {
            ...
            response({
              sent_metadata: options.headers,
              sent_data: payload,
              received_code: res.statusCode,
              received_metadata: res.headers,
              received_data: data,
            });
            return;
          }
