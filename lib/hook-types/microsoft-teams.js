...
function test(hook_url) {
  if(hook_url.toLowerCase().startsWith('https://outlook.office365.com/webhook')) {
    return true;
  } else {
    return false;
  }
...
function fire(id, hook_url, type, incoming, hmac, headers = {}) {
  ...
  let payload = {
    ...
    "@type": "MessageCard",
    "themeColor": "0072C6",
    "title": `${payload.action} event fired on ${payload.app.name}-${payload.space.name}`,
  };
...
module.exports = {
  test,
  fire,
  enabled:() => process.env.WEBHOOK_DISABLE_MICROSOFT_TEAMS ? false : true,
