...
function fire(id, hook_url, type, incoming, hmac, headers = {}, token = null) {
  ...
  if(incoming.action === 'released') {
    ...
    const from = (incoming.slug.source_blog && incoming.slug.source_blob.commit) ? `${incoming.slug.source_blob.repo} ${incoming.slug.source_blob.branch} ``${incoming.slug.source_blob.commit.substring(0,7)}``` : incoming.slug.image;
    payload = {
      type: 'markdwn',
      text: `*${incoming.app.name}-${incoming.space.name}* ${incoming.dyno.type} were released! ``${version}``\n${message}From ${from}`,
    }
  }
  if(incoming.action === 'crashed') {
