...
function formatter(incoming) {
  ...
  if (incoming.action === 'released') {
    ...
    payload = {
      type: 'markdwn',
      text: `*${incoming.app.name}-${incoming.space.name}* ${incoming.dyno.type} was released! \`${version}\`\n${message}From ${from}`,
    };
  ...
  } else if (incoming.action === 'config_change') {
    const config_changes = incoming.changes.map((change) => {
      const verbs = { create: 'Created', delete: 'Deleted', update: 'Updated' };
      return `    • ${verbs[change.type]} \`${change.name}\``;
    }).join('\n');
    payload = {
      type: 'markdwn',
      text: `Config var change(s) on app \`${incoming.app.name}-${incoming.space.name}\`:\n${config_changes}`,
    };
  }
  return payload;
}
