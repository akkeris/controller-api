...
function calc_prorated_amount(id, item) {
  const base = new Date(id);
  const from = Date.UTC(base.getFullYear(), base.getUTCMonth(), 1);
  const to = Date.UTC(base.getFullYear(), base.getUTCMonth() + 1, 0, 23, 59, 59);
  ...
  const prorate = (end - start) / (to - from);
...
function to_response(id, items) {
  ...
  if (
    ...
    || (from.getFullYear() === now.getFullYear() && from.getMonth() > now.getMonth())
  ) {
    state = 2;
  }
  ...
  return {
    addons_total: items.reduce((a, c) => a + (c.type === 'addon' ? c.billed_price : 0), 0),
    database_total: items.reduce((a, c) => a + (c.description.indexOf('postgresql') > -1 ? c.billed_price : 0), 0),
    charges_total: items.reduce((a, c) => a + c.billed_price, 0),
    created_at: from.toISOString(),
    credits_total: 0,
    dyno_units: items.reduce((a, c) => a + (c.type === 'dyno' ? c.quantity : 0), 0),
    id,
    number: from.getTime().toString(),
    payment_status: state === 1 ? 'Paid' : 'Pending',
    period_end: to.toISOString(),
    period_start: from.toISOString(),
    platform_total: items.reduce((a, c) => a + (c.type === 'dyno' ? c.billed_price : 0), 0),
    state,
    total: items.reduce((a, c) => a + c.billed_price, 0),
    updated_at: from.toISOString(),
    weighted_dyno_hours: items.reduce((a, c) => a + (c.type === 'dyno' ? c.quantity : 0), 0) * 24 * to.getDate(),
    items,
  };
}
...
const insert_invoice_cache = query.bind(query, fs.readFileSync('./sql/insert_invoice_cache.sql').toString('utf8'), (r) => r);
const get_invoice_cache = query.bind(query, fs.readFileSync('./sql/get_invoice_cache.sql').toString('utf8'), (r) => r);

const list_invoices = query.bind(query, fs.readFileSync('./sql/invoices.sql').toString('utf8'), (r) => r);
const get_invoice = query.bind(query, fs.readFileSync('./sql/invoice.sql').toString('utf8'), to_response_items);
const list_invoices_by_org = query.bind(query, fs.readFileSync('./sql/invoices_by_org.sql').toString('utf8'), (r) => r);
const get_invoice_by_org = query.bind(query, fs.readFileSync('./sql/invoice_by_org.sql').toString('utf8'), to_response_items);
const list_invoices_by_space = query.bind(query, fs.readFileSync('./sql/invoices_by_space.sql').toString('utf8'), (r) => r);
const get_invoice_by_space = query.bind(query, fs.readFileSync('./sql/invoice_by_space.sql').toString('utf8'), to_response_items);

async function get(pg_pool, req, res, regex) {
  const invoice_id = httph.first_match(req.url, regex);

  let invoice = await get_invoice_cache(pg_pool, [invoice_id, null, null]);
  if (!invoice || invoice.length === 0) {
    invoice = await get_invoice(pg_pool, [invoice_id]);
    const resp = to_response(invoice_id, invoice);
    if (resp.state === 1) {
...
async function list(pg_pool, req, res /* regex */) {
  const invoices = await list_invoices(pg_pool, []);
  const start = new Date(invoices[0].start);
  const end = new Date();
  const dates = [invoice_format('/account/invoices/', new Date(start.toISOString()))];
...
async function list_by_org(pg_pool, req, res, regex) {
  const org = httph.first_match(req.url, regex);
  const invoices = await list_invoices_by_org(pg_pool, [org]);
  if (!invoices || invoices.length === 0) {
    throw new common.NotFoundError('The specified invoice was not found.');
  }
  const start = new Date(invoices[0].start);
  const end = new Date();
  const dates = [invoice_format(`/organizations/${org}/invoices/`, new Date(start.toISOString()))];
  while (start.getTime() < end.getTime()) {
    start.setMonth(start.getMonth() + 1);
...
async function get_by_org(pg_pool, req, res, regex) {
  const org = httph.first_match(req.url, regex);
  const invoice_id = httph.second_match(req.url, regex);
...
async function list_by_space(pg_pool, req, res, regex) {
  const space = httph.first_match(req.url, regex);
  const invoices = await list_invoices_by_space(pg_pool, [space]);
  if (!invoices || invoices.length === 0) {
    throw new common.NotFoundError('The specified invoice was not found.');
  }
  const start = new Date(invoices[0].start);
  const end = new Date();
  const dates = [invoice_format(`/spaces/${space}/invoices/`, new Date(start.toISOString()))];
  while (start.getTime() < end.getTime()) {
    start.setMonth(start.getMonth() + 1);
