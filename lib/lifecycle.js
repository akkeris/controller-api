...
async function restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org_name, why) {
  ...
  for(let i=0; i < formations.length; i++) {
    ...
    await common.alamo.deploy(pg_pool, space_name, app_name, form.type, image, form.command, form.port, form.healthcheck, await common.deployment_features(pg_pool, app_uuid, form.type), labels, await common.deployment_filters(pg_pool, app_uuid, form.type))
  }
}
// See below.
module.exports = {
