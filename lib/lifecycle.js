...
async function restart_and_redeploy_app(pg_pool, app_uuid, app_name, space_name, org_name, why) {
  ...
  for (let i = 0; i < formations.length; i++) {
    ...
    if (form.oneoff) {
      /* Oneoff Deployment */
      let env = null;
      let oneoffimage = image;
      // Make sure the database driver saved the oneoff_options column as an object rather than a string
      // This might not be neccesary but is a safety check that might help if we ever change db drivers
      if (typeof form.oneoff_options === 'string') {
        try {
          const obj = JSON.parse(form.oneoff_options);
          form.oneoff_options = obj;
        } catch (err) {
          // Ignore, form.oneoff_options was not modified
        }
      }
      if (form.oneoff_options && form.oneoff_options.image) {
        oneoffimage = form.oneoff_options.image;
      }
      if (form.oneoff_options && form.oneoff_options.env) {
        env = form.oneoff_options.env;
      }
      await common.alamo.oneoff_deploy(
        pg_pool, space_name, app_name, form.type, oneoffimage, form.command, labels, env, form.size,
      );
    } else {
      /* Standard Deployment */
      const features = await common.deployment_features(pg_pool, app_uuid, form.type);
      const filters = await common.deployment_filters(pg_pool, app_uuid, form.type, features);
