...
async function restart_app(pg_pool, app_name, space_name, why) {
  ...
  try {
    console.assert(formations.length === 0, 'No formations were found.')
    let release = await releases.latest_release(pg_pool, app.app_uuid)
  } catch (e) {
    // dont actually bother restarting anything, since theres either no releases
    // or there's no formations on the app
    if(why) {
