...
// const curl = config.log_shuttle_url ? (new url.URL(config.log_shuttle_url)) : '';
// const log_shuttle_url = curl !== '' ? (`${curl.protocol}//${curl.host}`) : '';
// const log_shuttle_token = curl !== '' ? (curl.username ? (curl.password ? `${curl.username}:${curl.password}` : curl.username) : '') : '';
// const log_shuttle_headers = { 'content-type': 'application/json', authorization: log_shuttle_token };
...
async function create_app_logdrain(pg_pool, app_uuid, app_name, space_name, url, user) {
  ...
  const data = await common.alamo.drains.create(pg_pool, 'app', `${app_name}-${space_name}`, url);
...
async function create_site_logdrain(pg_pool, site_uuid, site_name, url /* user */) {
  ...
  assert.ok(url, 'Cannot create log-drain, url is undefined.');
  return common.alamo.drains.create(pg_pool, 'site', site_name, url);
}

async function list_app_logdrain(pg_pool, app_uuid, app_name, space_name) {
  return common.alamo.drains.list(pg_pool, 'app', `${app_name}-${space_name}`);
}

async function list_site_logdrain(pg_pool, site_uuid, site_name) {
  return common.alamo.drains.list(pg_pool, 'site', site_name);
}

async function get_app_logdrain(pg_pool, app_uuid, app_name, space_name, drain_id) {
  return common.alamo.drains.get(pg_pool, 'app', `${app_name}-${space_name}`, drain_id);
}

async function get_site_logdrain(pg_pool, site_uuid, site_name, drain_id) {
  return common.alamo.drains.get(pg_pool, 'site', site_name, drain_id);
}

async function del_app_logdrain(pg_pool, app_uuid, app_name, space_name, drain_id, user) {
  const data = await common.alamo.drains.delete(pg_pool, 'app', `${app_name}-${space_name}`, drain_id);
  common.notify_hooks(pg_pool, app_uuid, 'logdrain_change', JSON.stringify({
    action: 'logdrain_change',
    app: {
      name: app_name,
...
async function del_site_logdrain(pg_pool, site_uuid, site_name, drain_id /* user */) {
  return common.alamo.drains.delete(pg_pool, 'site', site_name, drain_id);
}

async function delete_all_app_drains(pg_pool, app_uuid, app_name, space_name /* org_name */) {
  return Promise.all((await list_app_logdrain(pg_pool, app_uuid, app_name, space_name))
...
async function http_create(pg_pool, req, res, regex) {
  if (req.url.indexOf('/sites/') === -1) {
    ...
    const body = await httph.buffer_json(req, res);
    if (!body || !body.url) {
      throw new common.BadRequestError('The request did not include a "url" parameter.');
    }
    const existing_drains = await list_app_logdrain(pg_pool, app.app_uuid, app.app_name, app.space_name);
  ...
  const existing_drains = await list_site_logdrain(pg_pool, site.site, site.domain);
  if (existing_drains.some((x) => x.url.toLowerCase().trim() === body.url.toLowerCase().trim())) {
...
async function http_get(pg_pool, req, res, regex) {
  if (req.url.indexOf('/sites/') === -1) {
    const app_key = httph.first_match(req.url, regex);
    const drain_id = httph.second_match(req.url, regex);
    const app = await common.app_exists(pg_pool, app_key);
...
async function event(pg_pool, app, space, data) {
  return common.log_event(pg_pool, app, space, data);
}
