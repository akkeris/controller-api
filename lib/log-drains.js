...
async function create(pg_pool, app_uuid, app_name, space_name, url, user) {
  ...
  let data = null
  if(curl === '') {
    data = await common.alamo.drains.create(pg_pool, space_name, app_name, url)
  } else {
    // DEPRECATED: Remove once multi-region support is deployed to all environments.
    data = JSON.parse((await httph.request('post', `${log_shuttle_url}/apps/${app_name}-${space_name}/log-drains`, log_shuttle_headers, JSON.stringify({url}))).toString())
  }
...
async function del(pg_pool, app_uuid, app_name, space_name, drain_id, user) {
  let data = null
  if (curl === '') {
    data = await common.alamo.drains.delete(pg_pool, space_name, app_name, drain_id)
  } else {
    // DEPRECATED: Remove once multi-region support is deployed to all environments.
    data = JSON.parse((await httph.request('delete', `${log_shuttle_url}/apps/${app_name}-${space_name}/log-drains/${drain_id}`, log_shuttle_headers, null)).toString())
  }
  common.notify_hooks(pg_pool, app_uuid, 'logdrain_change', JSON.stringify({
    'action':'logdrain_change',
    'app':{
...
async function http_create(pg_pool, req, res, regex) {
  ...
  let drain = await create(pg_pool, app.app_uuid, app.app_name, app.space_name, body.url, req.headers['x-username'])
...
async function event(pg_pool, app, space, data) {
  ...
  try {
    if(curl === '') {
      ...
      // DEPRECATED: Remove once multi-region support is deployed to all environments.
      await httph.request('post', `${log_shuttle_url}/log-events`, log_shuttle_headers, JSON.stringify({
        "log":data,
        "stream":"stdout",
