...
let curl = config.log_session_url ? (new url.URL(config.log_session_url)) : '';
let log_session_url = curl ? (curl.protocol + "//" + curl.host) : '';
let log_session_token = curl !== '' ? (curl.username ? (curl.password ? curl.username + ':' + curl.password : curl.username) : '') : '';
...
async function create_app_logsession(pg_pool, app_name, space_name, lines, tail) {
  ...
  let data = common.alamo.drains.session(pg_pool, 'app', app_name + '-' + space_name, lines, tail)
...
async function create_site_logsession(pg_pool, site_uuid, site_name, lines, tail) {
  ...
  let data = common.alamo.drains.session(pg_pool, 'site', site_name, lines, tail)
  data.created_date = (new Date()).toISOString();
  return data
}
...
async function http_create(pg_pool, req, res, regex) {
  if(req.url.indexOf('/sites/') === -1) {
    ...
    let data = await create_app_logsession(pg_pool, app.app_name, app.space_name, payload.lines, payload.tail)
    ...
    let payload = await httph.buffer_json(req, res)
    let data = await create_site_logsession(pg_pool, site.site, site.domain, payload.lines, payload.tail)
    return httph.created_response(res, JSON.stringify({
      "created_at":data.created_date,
      "id":data.id,
      "logplex_url":data.logplex_url ? data.logplex_url : (log_session_url + '/log-sessions/' + data.id),
