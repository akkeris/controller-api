...
async function create_app_logsession(pg_pool, app_name, space_name, lines, tail) {
  ...
  let data = null
  if (curl === '') {
    data = common.alamo.drains.session(pg_pool, 'app', app_name + '-' + space_name, lines, tail)
  } else {
...
async function create_site_logsession(pg_pool, site_uuid, site_name, lines, tail) {
  ...
  let data = null
  if (curl === '') {
    data = common.alamo.drains.session(pg_pool, 'site', site_name, lines, tail)
  } else {
    // DEPRECATED: Remove once multi-region support is deployed to all environments.
    data = JSON.parse((await httph.request('post',  log_session_url + '/log-sessions', log_session_headers, {
      site:site_name,
...
async function http_create(pg_pool, req, res, regex) {
  if(req.url.indexOf('/sites/') === -1) {
    ...
    let site = await common.app_exists(pg_pool, site_key)
    let payload = await httph.buffer_json(req, res)
