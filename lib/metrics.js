...
const query_template = prometheus_version === 'v1'
  ? 'avg(%metric%{namespace="%space%",container_name="%app%",kubernetes_role="node"})'
  : 'avg(%metric%{namespace="%space%",container_name="%app%",job="kubernetes-nodes-cadvisor"})';
...
async function query_prometheus_metrics(
  pg_pool,
  fqdn,
  app_name,
  space_name,
  ...
  const received = { dyno: formation_type };
  const alamo_app_name = common.alamo.app_name(app_name, formation_type);
  let prometheus_url = config.prometheus_metrics_url;
  if (!prometheus_url) {
    prometheus_url = await common.alamo.region_api_by_space(pg_pool, space_name);
  }
  await Promise.all(prometheus_metrics.map(async (metric) => {
    const q = metric.indexOf('cpu_') > -1
      ? query_rate_template
      : (metric.indexOf('network_') > -1
        ? query_named_pod_template
        : query_template);
    const query_url = `${prometheus_url
    ...
    const data = await httph.request('get', query_url, {}, null);
    const { result } = JSON.parse(data).data;
    const name = metric.replace(/container_/, '');
    received[name] = {};
    result.forEach((x) => x.values.forEach((y) => {
      [, received[name][Math.floor(y[0])]] = y;
    }));
  }));
  return received;
}
...
async function query_available_influxdb_metrics(pg_pool, fqdn, app_name, space_name, formation_type /* callback */) {
  ...
  const data = await httph.request('get', available_metrics, { 'X-JSON': true }, null);
  if (data.results && data.results[0] && data.results[0].series && data.results[0].series[0]) {
    return data.results[0].series[0].values.map((x) => x[0]).slice(0, 100); // only support 100 metrics to be returned
  }
  return [];
}
...
function influxdb_array_to_object(data) {
  const obj = {};
  data.forEach((x) => {
    obj[((new Date(x[0])).getTime() / 1000).toString()] = (x[1] === null || typeof (x[1]) === 'undefined') ? '0' : x[1].toString();
  });
  return obj;
}
...
function normalize_influxdb(dyno, data) {
  const result = { dyno };
  if (data.results && data.results[0] && data.results[0].series && data.results[0].series[0]) {
    let { name } = data.results[0].series[0];
...
function has_data_influxdb(dyno, data) {
  if (data.results && data.results[0] && data.results[0].series && data.results[0].series[0]) {
    return !data.results[0].series[0].values.every((x) => x[1] === 0 || x[1] === null);
  }
  return false;
}
...
async function query_influxdb_network_metrics(pg_pool, space_name, fqdn, from, to, resolution) {
  ...
  fqdn = fqdn.replace('https://', '').replace('http://', '').replace('/', '');
  const metrics = await query_available_influxdb_metrics(pg_pool, fqdn, null, space_name, null);
  const metrics_data = await Promise.all(metrics.map((metric) => {
    const query = `select ${determine_aggregate(metric)}(value) from "${metric}" where fqdn='${fqdn.replace(/'/g, '')}' and time > '${from.toISOString().replace(/'/g, '')}' and time < '${to.toISOString().replace(/'/g, '')}' group by time(${resolution})`;
  ...
  metrics_data
    ...
    .forEach((x) => Object.keys(x).forEach((y) => {
      result[y] = x[y];
    }));
  return result;
}
...
async function query_influxdb_app_metrics(pg_pool, fqdn, app_name, space_name, from, to, resolution, formation_type) {
  ...
  const metrics = await query_available_influxdb_metrics(pg_pool, null, app_name, space_name, formation_type);
  const metrics_data = await Promise.all(metrics.map((metric) => {
    const query = `select ${determine_aggregate(metric)}(value) from "${metric}" where app = '${common.alamo.app_name(app_name, formation_type)}-${space_name}' and time > '${from.toISOString().replace(/'/g, '')}' and time < '${to.toISOString().replace(/'/g, '')}' group by time(${resolution})`;
    return httph.request('get', `${influx_url}/query?db=opentsdb&q=${encodeURIComponent(query)}`, { 'X-JSON': true }, null);
  }));
  const result = {};
  metrics_data
...
async function http_metrics(pg_pool, req, res, regex) {
  ...
  const params = (new url.URL(req.url.toLowerCase(), `http://${req.headers.host}/`)).searchParams;

  const to = params.get('to') ? new Date(params.get('to')) : new Date();
  const from = params.get('from') ? new Date(params.get('from')) : new Date(to);
  if (to.getTime() === from.getTime()) {
    from.setDate(from.getDate() - 1);
  }
  const resolution = params.resolution || '30m';
