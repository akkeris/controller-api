...
const query_template = prometheus_version === 'v1' ? 'avg(%metric%{namespace="%space%",container_name="%app%",kubernetes_role="node"})' : 'avg(%metric%{namespace="%space%",container_name="%app%",job="kubernetes-cadvisors"})';
...
async function query_prometheus_metrics(pg_pool, fqdn, app_name, space_name, from, to, resolution, formation_type, callback) {
  ...
  if(!prometheus_url) {
    prometheus_url = await common.region_api_by_space(pg_pool, space_name)
  }
...
async function query_available_influxdb_metrics(pg_pool, fqdn, app_name, space_name, formation_type, callback) {
  ...
  if(!influx_url) {
    influx_url = await common.region_api_by_space(pg_pool, space_name)
  }
