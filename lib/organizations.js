...
// private
let select_org_query = fs.readFileSync("./sql/select_org.sql").toString('utf8');
let select_org = query.bind(query, select_org_query, (n) => { return n; });

let select_orgs_query = fs.readFileSync("./sql/select_orgs.sql").toString('utf8');
let select_orgs = query.bind(query, select_orgs_query, (n) => { return n; });
...
async function list(pg_pool, req, res, regex) {
  let org = await select_orgs(pg_pool, [])
...
async function get(pg_pool, req, res, regex) {
  let org = http_help.first_match(req.url, regex)
  let org_obj = await select_org(pg_pool, [org])
  if(org_obj.length === 0) {
...
async function create(pg_pool, req, res, regex) {
  ...
  try {
    console.assert(payload.name, "Organization requires the field name.")
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message)
  }
  let org = await select_org(pg_pool, [payload.name])
  if(org.length !== 0) {
...
async function remove(pg_pool, req, res, regex) {
  let org_obj = await select_org(pg_pool, [http_help.first_match(req.url, regex)])
