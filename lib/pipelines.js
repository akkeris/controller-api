...
const select_pipeline = query.bind(query, fs.readFileSync('./sql/select_pipeline.sql').toString('utf8'), (d) => d);
const insert_pipeline = query.bind(query, fs.readFileSync('./sql/insert_pipeline.sql').toString('utf8'), (d) => d);
const update_pipeline_updated_at = query.bind(query, fs.readFileSync('./sql/update_pipeline_updated_at.sql').toString('utf8'), (d) => d);
async function http_create(pg_pool, req, res /* regex */) {
  ...
  const pipeline = await select_pipeline(pg_pool, [payload.name]);
  if (pipeline.length !== 0) {
    throw new common.UnprocessibleEntityError('The specified pipeline already exists.');
  }
  const pipeline_uuid = uuid.v4();
...
async function http_get(pg_pool, req, res, regex) {
  const pipeline_key = httph.first_match(req.url, regex);
  const pipeline = await select_pipeline(pg_pool, [pipeline_key.toLowerCase()]);
  if (pipeline.length === 0) {
...
const list_pipelines_query = query.bind(query, fs.readFileSync('./sql/select_pipelines.sql').toString('utf8'), (d) => d);
async function http_list(pg_pool, req, res /* regex */) {
  const data = await list_pipelines_query(pg_pool, []);
  return httph.ok_response(res, JSON.stringify(data.map(pipeline_postgres_to_response)));
}

const delete_pipeline_query = query.bind(query, fs.readFileSync('./sql/delete_pipeline.sql').toString('utf8'), (d) => d);
const delete_pipeline_couplings_by_pipeline_query = query.bind(
  query,
  fs.readFileSync('./sql/delete_pipeline_couplings_by_pipeline.sql').toString('utf8'), (d) => d,
);
...
const select_pipeline_coupling_by_app = query.bind(
  query,
  fs.readFileSync('./sql/select_pipeline_coupling_by_app.sql').toString('utf8'), (d) => d,
);
async function http_get_pipeline_coupling_by_app(pg_pool, req, res, regex) {
  ...
  const pipeline_coupling = await select_pipeline_coupling_by_app(pg_pool, [app.app_uuid]);
...
const insert_pipeline_coupling = query.bind(query, fs.readFileSync('./sql/insert_pipeline_coupling.sql').toString('utf8'), (d) => d);
async function create_pipeline_coupling(
  pg_pool,
  app_uuid,
  app_name,
  space_name,
  ...
  try {
    ...
    assert.ok(
      Array.isArray(required_status_checks.contexts)
      && required_status_checks.contexts.every((x) => typeof x === 'string'),
      'The required status checks must be an array of strings!',
    );
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  ...
  const pipeline_coupling = await select_pipeline_coupling_by_app(pg_pool, [app_uuid]);
  if (pipeline_coupling.length !== 0) {
    throw new common.ConflictError('The application already is pipelined.');
  }
  const pipelines = await select_pipeline(pg_pool, [pipeline]);
  if (pipelines.length === 0) {
    throw new common.NotFoundError('The specified pipeline was not found.');
  }
  const pipeline_coupling_uuid = uuid.v4();
  const created = new Date();
  await insert_pipeline_coupling(
    pg_pool,
    [
      pipeline_coupling_uuid,
      created,
      created,
...
const get_pipeline_coupling_query = query.bind(query, fs.readFileSync('./sql/select_pipeline_coupling.sql').toString('utf8'), (d) => d);
const update_pipeline_coupling = query.bind(query, fs.readFileSync('./sql/update_pipeline_coupling.sql').toString('utf8'), (d) => d);
async function http_update_pipeline_coupling(pg_pool, req, res, regex) {
  const payload = await httph.buffer_json(req);
  const pipeline_coupling_uuid = httph.first_match(req.url, regex);
  let pipeline_coupling = await get_pipeline_coupling_query(pg_pool, [pipeline_coupling_uuid]);
  if (pipeline_coupling.length === 0) {
    throw new common.NotFoundError('The specified pipeline coupling was not found.');
  }
  await common.app_exists(pg_pool, pipeline_coupling[0].app_uuid);
  ...
  if (
    payload.required_status_checks
    && payload.required_status_checks.contexts
    && payload.required_status_checks.contexts.length > 0
    && pipeline_coupling[0].stage === 'review'
  ) {
    throw new common.UnprocessibleEntityError('Required status checks cannot be added to a pipelines review stage.');
  }

  const current_contexts = pipeline_coupling[0].required_status_checks;
  const proposed_contexts = payload.required_status_checks.contexts;
  const contexts_removed = !current_contexts.every((existing_context) => proposed_contexts.includes(existing_context));
  const is_elevated_access = req.headers['x-elevated-access'] === 'true' && req.headers['x-username'];
...
async function http_create_pipeline_coupling(pg_pool, req, res /* regex */) {
  ...
  const pipeline_coupling = await create_pipeline_coupling(
    pg_pool,
    app.app_uuid,
    app.app_name,
    app.space_name,
...
async function http_get_pipeline_coupling(pg_pool, req, res, regex) {
  const pipeline_coupling_uuid = httph.first_match(req.url, regex);
  const pipeline_coupling = await get_pipeline_coupling_query(pg_pool, [pipeline_coupling_uuid]);
  if (pipeline_coupling.length === 0) {
    throw new common.NotFoundError('The specified pipeline coupling was not found.');
  }
...
const list_pipeline_couplings_query = query.bind(
  query,
  fs.readFileSync('./sql/select_pipeline_couplings.sql').toString('utf8'), (d) => d,
);
async function http_list_pipeline_couplings(pg_pool, req, res /* regex */) {
  const pipeline_couplings = await list_pipeline_couplings_query(pg_pool, []);
  httph.ok_response(res,
    JSON.stringify(pipeline_couplings.map(pipeline_coupling_postgres_to_response)));
}

const list_pipeline_couplings_by_pipeline_query = query.bind(
  query,
  fs.readFileSync('./sql/select_pipeline_couplings_by_pipeline.sql').toString('utf8'), (d) => d,
);
async function http_list_pipeline_couplings_by_pipeline(pg_pool, req, res, regex) {
  ...
  const pipeline_couplings = await list_pipeline_couplings_by_pipeline_query(pg_pool, [pipeline[0].pipeline]);
  return httph.ok_response(res,
    JSON.stringify(pipeline_couplings.map(pipeline_coupling_postgres_to_response)));
}

const delete_pipeline_coupling_query = query.bind(
  query,
  fs.readFileSync('./sql/delete_pipeline_coupling.sql').toString('utf8'), (d) => d,
);
...
async function get_promotion_warnings(pg_pool, app_src, app_dst) {
  const warnings = [];
  const src_config = await config_var.get(pg_pool, app_src.name, app_src.space, app_src.app);
  const dst_config = await config_var.get(pg_pool, app_dst.name, app_dst.space, app_dst.app);
  const src_keys = Object.keys(src_config);
  const dst_keys = Object.keys(dst_config);

  const warnings_config_source = [];
  for (let i = 0; i < src_keys.length; i++) {
    const key = src_keys[i];
    if (!Object.prototype.hasOwnProperty.call(dst_config, key)) {
      warnings_config_source.push(key);
    }
  }
  const warnings_config_dest = [];
  for (let i = 0; i < dst_keys.length; i++) {
    const key = dst_keys[i];
    if (!Object.prototype.hasOwnProperty.call(src_config, key)) {
  ...
  const src_addons = await addons.list(pg_pool, app_src.app, app_src.name, app_src.space, app_src.org);
  const dst_addons = await addons.list(pg_pool, app_dst.app, app_dst.name, app_dst.space, app_dst.org);

  const warnings_addon_source = [];
...
const create_pipeline_promotion_query = query.bind(query, fs.readFileSync('./sql/insert_pipeline_promotion.sql').toString('utf8'), (d) => d);
const get_pipeline_couplings_by_stage = query.bind(query, fs.readFileSync('./sql/select_pipeline_couplings_by_stage.sql').toString('utf8'), (d) => d);
const create_pipeline_promotion_target_query = query.bind(query, fs.readFileSync('./sql/insert_pipeline_promotion_target.sql').toString('utf8'), (d) => d);
const select_release_statuses = query.bind(query, fs.readFileSync('./sql/select_release_statuses.sql').toString('utf8'), (d) => d);
async function http_create_pipeline_promotion(pg_pool, req, res /* regex */) {
  ...
  try {
    ...
    assert.ok(payload.targets.every((target) => (!!target.app.id)),
      'A specified target for deployment did not contain an app id, e.g., #/targets/[]/app/id did not exist.');
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  const app = await common.app_exists(pg_pool, payload.source.app.id);
  ...
  try {
    ...
    assert.ok(
      pipeline_coupling[0].pipeline === payload.pipeline.id
      || pipeline_coupling[0].name === payload.pipeline.name.toLowerCase(),
      'The specified app is not in the given pipeline.',
    );
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  ...
  const pipeline_couplings = await get_pipeline_couplings_by_stage(
    pg_pool,
    [
      payload.pipeline.id,
      stages[pipeline_coupling[0].stage],
    ],
  );
  // ensure all the apps in the payload.target are
  // in the list of apps that are coupled and in the next stage of the source app.
  const targets = pipeline_couplings.filter((pc) => payload.targets.some((target) => pc.app_uuid === target.app.id));
  ...
  const is_elevated_access = req.headers['x-elevated-access'] === 'true' && req.headers['x-username'];
  const successful_statuses = (await select_release_statuses(pg_pool, [latest_release.release]))
    .filter((x) => x.state === 'success')
    .map((x) => x.context);
  const failures = [];
  targets.forEach((target) => {
    const required_statuses = target.required_status_checks;
    const failed_statuses = required_statuses.filter((x) => !successful_statuses.includes(x));
  ...
  if (payload.safe) {
    for (let i = 0; i < targets.length; i++) {
      ...
      const app_src = {
        ...
        org: app.org_uuid,
        space_tags: app.space_tags,
      };
      const app_dst = {
        app: target.app_uuid,
        name: target.app_name,
        space: target.space_name,
        org: target.org,
        space_tags: target.space_tags,
      };
      // eslint-disable-next-line no-await-in-loop
      const warnings = await get_promotion_warnings(pg_pool, app_src, app_dst);
  ...
  const pipeline_promotion_uuid = uuid.v4();
  const created = new Date();
  await create_pipeline_promotion_query(
    pg_pool,
    [
      pipeline_promotion_uuid,
      created,
      created,
      pipeline_coupling[0].pipeline,
      latest_release.release,
      `Promotion of ${app.app_name}-${app.space_name}`,
      'aka',
    ],
  );
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    // TODO: don't use objects, pass these into destination as full arguments.
    const app_src = {
      app: app.app_uuid, name: app.app_name, space: app.space_name, org: app.org_uuid, space_tags: app.space_tags,
    };
    const app_dst = {
      app: target.app_uuid, name: target.app_name, space: target.space_name, org: target.org, space_tags: target.space_tags,
    };
    // eslint-disable-next-line no-await-in-loop
    const release = await releases.create_release(
      pg_pool,
      app_src,
      app_dst,
      latest_release.build,
      `Promotion from ${app.app_name}-${app.space_name}`,
      'promotion',
      `Promotion from ${app.app_uuid} of ${latest_release.build}`,
      'aka',
      req.headers['x-username'],
    );
    const query_created = new Date();
    const pipeline_promotion_target_id = uuid.v4();
    create_pipeline_promotion_target_query(
      pg_pool,
      [
        pipeline_promotion_target_id,
        query_created,
        query_created,
        pipeline_promotion_uuid,
        release.id,
...
const list_pipeline_promotion_query = query.bind(
  query,
  fs.readFileSync('./sql/select_pipeline_promotions.sql').toString('utf8'), (d) => d,
);
async function http_list_pipeline_promotions(pg_pool, req, res /* regex */) {
  const pipeline_promotions = await list_pipeline_promotion_query(pg_pool, []);
  return httph.ok_response(res, JSON.stringify(pipeline_promotions.map(pipeline_promotions_postgres_to_response)));
}

const get_pipeline_promotion_query = query.bind(
  query,
  fs.readFileSync('./sql/select_pipeline_promotion.sql').toString('utf8'), (d) => d,
);
async function http_get_pipeline_promotion(pg_pool, req, res, regex) {
  const pipeline_promotion_uuid = httph.first_match(req.url, regex);
  const pipeline_promotion = await get_pipeline_promotion_query(pg_pool, [pipeline_promotion_uuid]);
...
async function http_get_pipeline_promotion_target(pg_pool, req, res /* regex */) {
  const pipeline_promotion = await list_pipeline_promotion_query(pg_pool, []);
...
const select_release_statuses_by_pipeline = query.bind(
  query,
  fs.readFileSync('./sql/select_release_statuses_by_pipeline.sql').toString('utf8'), (d) => d,
);
async function http_list_pipeline_statuses(pg_pool, req, res, regex) {
  ...
  const pipeline_statuses = await select_release_statuses_by_pipeline(pg_pool, [pipeline[0].pipeline]);
  return httph.ok_response(res, JSON.stringify(pipeline_statuses.map((x) => ({ context: x.context, name: x.name }))));
}
