...
async function get_promotion_warnings(pg_pool, app_src, app_dst) {
  ...
  let src_addons = await addons.list(pg_pool, app_src.app_uuid, app_src.name, app_src.space, app_src.org);
  let dst_addons = await addons.list(pg_pool, app_dst.app_uuid, app_dst.name, app_dst.space, app_dst.org);
