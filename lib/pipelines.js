...
async function get_promotion_warnings(pg_pool, app_src, app_dst) {
  ...
  let dst_config = await config_var.get(pg_pool, app_dst.name, app_dst.space, app_dst.app_uuid)
...
async function http_create_pipeline_promotion(pg_pool, req, res, regex) {
  ...
  try {
    ...
    assert.ok(payload.targets.every((target) => { return target.app.id ? true : false; }), 'A specified target for deployment did not contain an app id, e.g., #/targets/[]/app/id did not exist.')
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message)
  }
