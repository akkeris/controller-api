...
async function http_update_pipeline_coupling(pg_pool, req, res, regex) {
  ...
  if(!payload.required_status_checks || !payload.required_status_checks.contexts || !Array.isArray(payload.required_status_checks.contexts) ||
     !payload.required_status_checks.every((x) => typeof x === 'string')) 
  {
  ...
  let proposed_contexts = payload.required_status_checks;
...
async function http_create_pipeline_promotion(pg_pool, req, res, regex) {
  ...
  let is_elevated_access = req.headers['x-elevated-access'] === 'true' && req.headers['x-username'];
  let successful_statuses = (await select_release_statuses(pg_pool, [latest_release.release])).filter((release) => release.state === 'success');
  let failures = [];
  targets.map((target) => {
  ...
  if(failures.length !== 0 && (payload.safe && is_elevated_access || !is_elevated_access)) {
    throw new common.ConflictError(`This promotion was stopped as it was deemed unsafe. Reasons:\n${failures.join("\n")}`);
  }
