...
async function http_create(pg_pool, req, res, regex) {
  ...
  try {
    console.assert(payload.name && payload.name.match(/^[a-z][a-z0-9-]{2,29}$/) !== null, 
...
async function create_pipeline_coupling(pg_pool, app_uuid, app_name, space_name, org_uuid, pipeline, stage) {
  try {
    console.assert(app_uuid, 'No application was provided.');
    console.assert(app_name, 'No application name was provided.')
    console.assert(pipeline, 'No pipeline was provided.');
    console.assert(typeof(pipeline) === 'string', 'The specified pipeline was not a uuid');
    pipeline = pipeline.toLowerCase();
    console.assert(stage, 'No stage was provided.');
    console.assert(stage === 'review' || stage === 'development' || stage === 'staging' || stage === 'production',
...
async function http_list_pipeline_couplings_by_pipeline(pg_pool, req, res, regex) {
  let pipeline_uuid = httph.first_match(req.url, regex);
  let pipeline_couplings = await list_pipeline_couplings_by_pipeline_query(pg_pool, [pipeline_uuid])
  return httph.ok_response(res,
    JSON.stringify(pipeline_couplings.map(pipeline_coupling_postgres_to_response)))
}
...
async function get_promotion_warnings(pg_pool, app_src, app_dst) {
  let warnings = [];
  let src_config = await config_var.get(pg_pool, app_src.name, app_src.space, app_src.app_uuid)
  let dst_config = await config_var.get(pg_pool, app_dst.name, app_src.space, app_dst.app_uuid)
  ...
  for (let i = 0; i < src_keys.length; i++){
    ...
    if (!dst_config.hasOwnProperty(key)) {
        warnings.push(`  The source app contains a key (${key}) that is not present in the destination app (${app_dst.name}-${app_dst.space}).`)
    }
  }
  for (let i = 0; i < dst_keys.length; i++){
    let key = dst_keys[i];
    if (!src_config.hasOwnProperty(key)) {
        warnings.push(`  The destination app (${app_dst.name}-${app_dst.space}) contains a key (${key}) that is not present in the source app`)
    }
  }
  return warnings
}
...
async function http_create_pipeline_promotion(pg_pool, req, res, regex) {
  let payload = await httph.buffer_json(req)
  try {
    console.assert(payload.pipeline, 'The request did not include a "pipeline" to promote.')
    console.assert(payload.source, 'The request did not include a "source" app to promote.')
    console.assert(payload.source.app, 'The request did not include a "source/app" id or name to promote.')
    console.assert(payload.source.app.id, 'The request did not include a "source/app/id" to promote.')
    console.assert(payload.targets, 'The request did not include a "target" array of targets to promote to.')
    console.assert(Array.isArray(payload.targets), 'The requested target field of apps to deploy to was not an array.')
    console.assert(payload.targets.every((target) => { return target.app.id ? true : false; }), 'A specified target for deployment did not contain an app id, e.g., #/targets/[]/app/id did not exist.')
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message)
  }
  ...
  let pipeline_coupling = await select_pipeline_coupling_by_app(pg_pool, [app.app_uuid])
  try {
    console.assert(stages[pipeline_coupling[0].stage], 'The specified app cannot be promoted as it has no next stage.')
    console.assert(pipeline_coupling[0].pipeline === payload.pipeline.id || pipeline_coupling[0].name === payload.pipeline.name.toLowerCase(), 'The specified app is not in the given pipeline.')
  } catch(e) {
    throw new common.UnprocessibleEntityError(e.message)
  }
