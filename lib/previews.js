...
async function create(pg_pool, source_app_uuid, foreign_key, suffix, additional_info) {
  ...
  try {
    ...
    definition.app.name = definition.app.name.substring(0, max_subdomain_length - suffix.length) + suffix
    ...
    let site_definitions = await Promise.all(unique_site_names.map(async (site) => {
      let site_to_create = site.split('.').map((x, i) => (i === 0) ? (x.substring(0, max_subdomain_length - suffix.length) + suffix) : x ).join('.')
    ...
    common.notify_hooks(pg_pool, source_app.app_uuid, 'preview', JSON.stringify(payload), req.headers['x-username']);
    return payload
  } catch (e) {
