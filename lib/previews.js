...
async function preview_app_released(pg_pool, payload) {
  ...
  if(target_app.preview) {
    ...
    let source_app = await source_app(pg_pool, target_app.app_uuid);
    p.action = 'preview-released';
    p.source_app = {id:source_app.app_uuid, name:source_app.app_name};
    common.lifecycle.emit('preview-released', p);
    common.notify_hooks(pg_pool, target_app.app_uuid, p.action, JSON.stringify(p), "System");
  }
}
