...
async function create(pg_pool, source_app_uuid, foreign_key, suffix, additional_info) {
  ...
  try {
    ...
    if(source_app.space_tags.indexOf('compliance=socs') > -1) {
    ...
    let required_env = Object.keys(definition.env).map((key) => { return {...definition.env[key], key}; }).filter((x) => x.required && !x.value);
    ...
    if (await common.feature_enabled(pg_pool, source_app_uuid, "preview-sites")) {
      ...
      let site_definitions = await Promise.all(unique_site_names.map(async (site) => {
        ...
        let site_routes = await routes.list_by_site(pg_pool, [site]);
        ...
        let non_pending_routes = site_routes.filter((x) => x.pending !== true)
...
async function preview_app_released(pg_pool, payload) {
  let target_app = await common.app_exists(pg_pool, payload.key || payload.app.id);
  if(target_app.preview) {
    let p = JSON.parse(JSON.stringify(payload));
    let sapp = await source_app(pg_pool, target_app.app_uuid);
