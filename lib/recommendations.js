...
async function http_get(pg_pool, req, res, regex) {
  ...
  const app = await common.app_exists(pg_pool, app_key);
  let payload;
  let recommendation_uuid;
  try {
    payload = await httph.buffer_json(req);
  } catch (err) {
    // do nothing, no payload was provided
    recommendation_uuid = httph.second_match(req.url, regex);
    if (!recommendation_uuid) {
      throw new httph.BadRequestError('Invalid payload and/or no recommendation UUID provided');
    }
  }

  // No recommendation UUID provided - Payload must have service, resource_type, action
  if (!recommendation_uuid) {
    if (
      (!payload.service || payload.service === '')
      || (!payload.resource_type || payload.resource_type === '')
      || (!payload.action || payload.action === '')
    ) {
      throw new httph.BadRequestError('Invalid payload supplied - must have service, resource_type, and action');
    }
    const resource_type = await check_resource_type_exists(pg_pool, payload.resource_type);
    return get_recommendation(pg_pool, null, app.app_uuid, payload.service, resource_type, payload.action);
  }

  return get_recommendation(pg_pool, recommendation_uuid);
}
...
async function http_list(pg_pool, req, res, regex) {
  const app_key = httph.first_match(req.url, regex);
  const app = await common.app_exists(pg_pool, app_key);
  return httph.ok_response(res, JSON.stringify(await list_recommendations(pg_pool, app.app_uuid)));
}
