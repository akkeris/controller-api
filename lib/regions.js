...
async function update(
  ...
  try {
    assert.ok(!country || country !== '', 'The specified country field does not exist and is required.');
    assert.ok(!description || description !== '', 'The description field for this region is required.');
    assert.ok(!locale || locale !== '', 'The specified locale must exist.');
    assert.ok(!provider_name || provider_name !== '', 'The specified providers name must exist.');
    assert.ok(!provider_region || provider_region !== '', 'The specified providers region must exist.');
    assert.ok(!provider_availability_zones || Array.isArray(provider_availability_zones), 'The specified providers availability_zones is required.');
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  const updated_region = await update_region(
    pg_pool,
    [
      name,
      country,
      ...
      provider_name,
      provider_region,
      provider_availability_zones ? provider_availability_zones.join(',') : null,
      high_availability,
      deprecated,
    ],
  );
...
async function create(
  ...
  try {
    assert.ok(name && name !== '', 'The regions name was not provided and is required.');
    assert.ok(
      /^[A-z0-9-]+$/.exec(name) !== null && name.length < 32,
      'The region name was invalid, it must be an alpha numeric and may contain a hyphen.',
    );
    assert.ok(country && country !== '', 'The specified country field does not exist and is required.');
    assert.ok(description && description !== '', 'The description field for this region is required.');
    assert.ok(locale && locale !== '', 'The specified locale must exist.');
    assert.ok(provider_name && provider_name !== '', 'The specified providers name must exist.');
    assert.ok(private_capable === false || private_capable === true, 'The specified value for private capable was neither true or false.');
    assert.ok(provider_region && provider_region !== '', 'The specified providers region must exist.');
    assert.ok(provider_availability_zones, 'The specified providers availability zones is required.');
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  const region = await common.region_exists(pg_pool, name, true);
  if (region) {
  ...
  const params = [
    ...
    provider_name,
    provider_region,
    provider_availability_zones.join(','),
    high_availability,
  ];
  const new_region = await insert_region(pg_pool, params);
...
async function http_update(pg_pool, req, res, exp) {
  ...
  await common.region_exists(pg_pool,
  ...
  return httph.ok_response(res,
    JSON.stringify(await update(
      ...
      payload.description,
      payload.locale,
      payload.private_capable,
      payload.provider ? payload.provider.name : null,
      payload.provider ? payload.provider.region : null,
      payload.provider ? payload.provider.availability_zones : null,
      payload.high_availability,
      payload.deprecated,
    )));
}
...
async function http_create(pg_pool, req, res /* exp */) {
  ...
  return httph.created_response(res,
    JSON.stringify(await create(
      ...
      payload.provider.availability_zones,
      payload.high_availability,
    )));
}
...
async function http_delete(pg_pool, req, res, exp) {
  ...
  const region = await common.region_exists(pg_pool, httph.first_match(req.url, exp));
  const deleted_region = await delete_region(pg_pool, [region.region]);
