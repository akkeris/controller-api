...
// private
// TODO: get github notifications out of here.
  ...
  try {
    ...
    for(let i=0; i < auto.length; i++) {
      ...
      if(auto_release.token && auto_release.repo && auto_release.sha) {
        github.add_status('pending', auto_release.app_name, auto_release.space_name, auto_release.token, auto_release.repo, auto_release.sha, 'Deployment started.');
      }
      try {
        ...
        if(auto_release.token && auto_release.repo && auto_release.sha) {
          github.add_status('success', auto_release.app_name, auto_release.space_name, auto_release.token, auto_release.repo, auto_release.sha, 'Deployment successful.');
        }
        console.log('release automatically created for ' + auto_release.app);
      } catch (err) {
        if(auto_release.token && auto_release.repo && auto_release.sha) {
          github.add_status('failure', auto_release.app_name, auto_release.space_name, auto_release.token, auto_release.repo, auto_release.sha, 'Deployment failed.');
        }
