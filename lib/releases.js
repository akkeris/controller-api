...
async function request_release(pg_pool, app_dest, image) {
  ...
  for (let i=0 ; i < formations.length; i++) {
    ...
    results.push(await common.alamo.deploy(pg_pool, app_dest.space, app_dest.name, form.type, image, form.command, form.port, form.healthcheck, await features.deployment_features(pg_pool, app_dest.app, form.type), labels))
  }
  return results;
}
