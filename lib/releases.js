...
async function request_release(pg_pool, app_dest, image, release_id) {
  let formations = await common.formations_exists(pg_pool, app_dest.name, app_dest.space)
  if(formations.length === 0) {
    ...
    let dyno_info = await formation.create(pg_pool, app_dest.app, app_dest.name, app_dest.space, app_dest.space_tags, app_dest.org_name, 'web', config.dyno_default_size, 1, null, config.default_port, null, false)
    let new_formations = await common.formations_exists(pg_pool, app_dest.name, app_dest.space)
  ...
  for (let i=0 ; i < formations.length; i++) {
    ...
    results.push(await common.alamo.deploy(pg_pool, app_dest.space, app_dest.name, form.type, image, form.command, form.port, form.healthcheck, await features.deployment_features(pg_pool, app_dest.app, form.type), labels, await filters.deployment_filters(pg_pool, app_dest.app, form.type)))
  }
  return results;
}
