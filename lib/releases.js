  ...
  for (let i = 0; i < formations.length; i++) {
    ...
    results.push(
      await common.alamo.deploy(// eslint-disable-line no-await-in-loop
        ...
        form.port,
        form.healthcheck,
        await common.deployment_features(pg_pool, app_dest.app, form.type), // eslint-disable-line no-await-in-loop
        labels,
        await common.deployment_filters(pg_pool, app_dest.app, form.type), // eslint-disable-line no-await-in-loop
      ),
    );
  }
  return results;
