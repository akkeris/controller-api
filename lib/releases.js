...
// public
async function create_release(pg_pool, app_src, app_dest, build_uuid, description, trigger, trigger_notes, agent, req) {
...
async function create(pg_pool, app_uuid, app_name, space_name, space_tags, org, description, slug, release_id, trigger_notes, req) {
  if(!slug && !release_id) {
  ...
  return await create_release(pg_pool, target_app, target_app, slug, description, desc_build, trigger_notes || '', 'aka', req);
}
...
async function http_create(pg_pool, req, res, regex) {
  ...
  if(data.slug) {
    ...
    if(!build || build.app !== app.app_uuid) {
      throw new common.ConflictError(`The slug ${data.slug} does not exist or is still in process of buildling`)
    }
  ...
  return http_help.created_response(res, 
    JSON.stringify(await create(pg_pool, app.app_uuid, app.app_name, app.space_name, space.tags, app.org_uuid, data.description, data.slug, data.release, data.trigger_notes, req)))
}
