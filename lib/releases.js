...
function combined_state(release) {
  let success = (release.success_statuses || []).filter((x) => !!x);
  let pending = (release.pending_statuses || []).filter((x) => !!x);
  let failure = (release.failure_statuses || []).filter((x) => !!x);
  let error = (release.error_statuses || []).filter((x) => !!x);
  if(error.length > 0) {
    return 'error';
  }
  if(failure.length > 0) {
