...
const select_release_statuses = query.bind(query, fs.readFileSync("./sql/select_release_status.sql").toString('utf8'), null);
const insert_release_statuses = query.bind(query, fs.readFileSync("./sql/select_release_status.sql").toString('utf8'), null);
const update_release_statuses = query.bind(query, fs.readFileSync("./sql/select_release_status.sql").toString('utf8'), null);

// public
async function http_get_release_status(pg_pool, req, res, regex) {
  let app_key = http_help.first_match(req.url, regex);
  let release_id = http_help.second_match(req.url, regex);
  let release_status_key = http_help.third_match(req.url, regex);
  common.check_uuid(release_id);
  let release = await common.release_exists(pg_pool, app.app_uuid, release_id);
...
async function http_list_release_statuses(pg_pool, req, res, regex) {
  let app_key = http_help.first_match(req.url, regex);
  let release_id = http_help.second_match(req.url, regex);
  let release_status_id = http_help.third_match(req.url, regex);
  common.check_uuid(release_id);
  let release = await common.release_exists(pg_pool, app.app_uuid, release_id);
...
async function http_create_release_status(pg_pool, req, res, regex) {
  ...
  try {
    ...
    assert.ok(release.release, 'The release id was not defined.');
  ...
  await insert_release_status(pg_pool, [release_status_id, release.release, payload.state, payload.name, payload.context, payload.description || '', payload.target_url || '', payload.image_url || '', created_updated_at, created_updated_at]);
...
async function http_update_release_status(pg_pool, req, res, regex) {
  ...
  await update_release_status(pg_pool, [release_status_id, release.release, payload.name, payload.image_url, payload.target_url, payload.description, payload.state]);
  return httph.ok_response(res, JSON.stringify({
    "id":release_status_id,
