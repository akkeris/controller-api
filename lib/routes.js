...
async function create(pg_pool, site_uuid, app_uuid, source_path, target_path) {
  try {
  ...
  let routes = await select_route_by_details(pg_pool, [app.app_uuid, site.site, source_path, target_path])
  if (routes.length !== 0) {
    throw new common.UnprocessibleEntityError(`A route with the specified details already exists. (${source_path} -> ${target_path}`)
  }
  let pending = false
  if(await common.alamo.dyno.service_exists(pg_pool, app.app_name, app.space_name)) {
    await create_alamo_path(pg_pool, site.domain, app.space_name, app.app_name, source_path, target_path)
  } else {
    pending = true
  }
...
async function http_create(pg_pool, req, res, regex) {
  ...
  let route = await create(pg_pool, payload.site, payload.app, payload.source_path, payload.target_path)
  ...
  common.lifecycle.on('released', async (payload) => {
    try {
      if(payload.app.name && payload.space.name && payload.app.id) {
        ...
        for(let i=0; i < routes.length; i++) {
          let route = routes[i];
          await create_alamo_path(pg_pool, route.site.domain, route.space.name, route.app.name, route.source_path, route.target_path)
          await update_route(pg_pool, [route.id, null, null, true])
        }
      }
    } catch (e) {
