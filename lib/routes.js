...
async function create(pg_pool, site_uuid, app_uuid, source_path, target_path) {
  ...
  let data = await create_alamo_path(pg_pool, site.domain, app.space_name, app.app_name, source_path, target_path)
  let route_id = uuid.v4();
  let created_updated = new Date();
  routes = await insert_route(pg_pool, [route_id, app.app_uuid, site.site, source_path, target_path, created_updated, created_updated])
  return routes[0]
}
