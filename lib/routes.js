...
async function create_alamo_path(pg_pool, domain, space_name, app_name, app_uuid, source_path, target_path, donotpush) {
  let data = await common.alamo.sites.create_route(pg_pool, space_name, app_name, domain, source_path, target_path, await common.deployment_filters(pg_pool, app_uuid, "web"))
  if (!donotpush) {
...
async function create(pg_pool, site_uuid, app_uuid, source_path, target_path, donotpush) {
  ...
  if(await common.alamo.dyno.service_exists(pg_pool, app.app_name, app.space_name)) {
    await create_alamo_path(pg_pool, site.domain, app.space_name, app.app_name, app.app_uuid, source_path, target_path, donotpush)
  } else {
    pending = true
  }
    ...
    try {
      if(payload.app.name && payload.space.name && payload.app.id) {
        ...
        for(let i=0; i < routes.length; i++) {
          ...
          try {
            await create_alamo_path(pg_pool, route.site.domain, route.space.name, route.app.name, payload.app.id, route.source_path, route.target_path, true)
          } catch (e) {
