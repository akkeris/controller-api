...
const select_authorizations = query.bind(query, select_auth_stmt, (d) => d);
const update_authorizations = query.bind(query, update_auth_stmt, (d) => d);

const select_hooks = query.bind(query, select_hooks_stmt, (d) => d);
const update_hooks = query.bind(query, update_hooks_stmt, (d) => d);

function old_decrypt(enc_token) {
  const deciph = crypto.createDecipher('aes192', old_key);
...
async function update_tokens() {
  ...
  const curl = new url.URL(database_url);
