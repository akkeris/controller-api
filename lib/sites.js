...
let update_site_info = query.bind(query, fs.readFileSync('./sql/update_site_info.sql').toString('utf8'), to_response);
...
async function http_update(pg_pool, req, res, regex) {
  ...
  let sites_updated = await update_site_info(pg_pool, [site_id, description, labels])
...
async function create(pg_pool, internal, region, domain, description, labels) {
  ...
  try {
    ...
    assert.ok(!domain || /(^[A-z0-9-.]+$)/.exec(domain) !== null, 'The domain name of a site must only use alphanumerics, hyphens and periods.');
  ...
  sites_obj = await insert_site(pg_pool, [site_id, domain, region_uuid, created_updated, created_updated, internal ? 'compliance=internal' : '', description, labels])
  sites_obj[0].region = {
    "name":region_name,
    "id":region_uuid
  }
...
async function del(pg_pool, site_key) {
  ...
  if(rs.length !== 0) {
    // Ignore if a push fails. If the ingress times out we dont want to let something linger.
    try {
...
async function http_create(pg_pool, req, res, regex) {
  let payload = await httph.buffer_json(req)
  return httph.created_response(res, JSON.stringify(await create(pg_pool, payload.internal, payload.region, payload.domain, payload.description, payload.labels)))
}
...
async function remove_previews(pg_pool, app_uuid) {
  ...
  await Promise.all(sites.map(async (site) => {
    if(site.preview_target !== null && site.preview_target === app_uuid) {
      return await del(pg_pool, site.site)
    }
  }))
  return sites
}
