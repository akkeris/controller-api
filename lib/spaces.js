...
function space_payload_to_response(payload) {
  return {
    compliance: payload.tags.split(',').filter((x) => x.startsWith('compliance=')).map((x) => x.replace('compliance=', '')),
...
// private
const select_space = query.bind(query, fs.readFileSync('./sql/select_space.sql').toString('utf8'), (n) => n);
const insert_space = query.bind(query, fs.readFileSync('./sql/insert_space.sql').toString('utf8'), (n) => n);
...
async function create(pg_pool, req, res /* regex */) {
  let reserved_spaces = ['kube-system', 'brokers', 'k2-poc', 'kube-public', 'akkeris-system', 'istio-system', 'cert-manager'];
  try {
    if (process.env.RESERVED_SPACES) {
      reserved_spaces = reserved_spaces.concat(process.env.RESERVED_SPACES.split(',').map((x) => x.trim().toLowerCase()));
    }
  } catch (e) {
    console.log(`Warning: unable to add spaces to protected list: ${e.message}\n${e.stack}`);
  }
  ...
  const data = await select_space(pg_pool, [payload.name]);
  if (data.length !== 0) {
  ...
  if (payload.compliance) {
    if (Array.isArray(payload.compliance)) {
      ...
      payload.tags = payload.compliance.map((x) => `compliance=${x}`).join(',');
    } else {
  ...
  const spaces = await common.alamo.spaces.list(pg_pool);
  if (spaces.some((space) => space === payload.name)) {
    throw new common.ConflictError('The specified space is already in use.');
  }
  ...
  await common.alamo.spaces.create(pg_pool, payload.name, internal, payload.compliance);
