...
async function list(pg_pool) {
  return select_stacks(pg_pool, []);
}

async function update(pg_pool, stack_uuid, beta, deprecated) {
  const updated_stack = await update_stack(
    pg_pool,
    [
      stack_uuid,
      (beta === true || beta === false) ? beta : null,
      (deprecated === true || deprecated === false) ? deprecated : null,
    ],
  );
...
async function create(pg_pool, region_uuid, name, beta, deprecated) {
  try {
    assert.ok(name && name !== '', 'The stacks name was not provided and is required.');
    assert.ok(
      /^[A-z0-9-]+$/.exec(name) !== null && name.length < 32,
      'The stack name was invalid, it must be an alpha numeric and may contain a hyphen.',
    );
    assert.ok(region_uuid || region_uuid === '', 'The specified region was not provided.');
  } catch (e) {
    throw new common.UnprocessibleEntityError(e.message);
  }
  const stack = await common.stack_exists(pg_pool, name, true);
  if (stack) {
    throw new common.ConflictError(`The stack ${name} already exists.`);
  }
  const region = await common.region_exists(pg_pool, region_uuid);
  const params = [uuid.v4(), region.region, name, beta, deprecated];
  const new_stack = await insert_stack(pg_pool, params);
...
async function http_update(pg_pool, req, res, exp) {
  ...
  const stack = await common.stack_exists(pg_pool,
    httph.first_match(req.url, exp));
  const payload = await httph.buffer_json(req);
  const stack_new = await update(pg_pool, stack.stack, payload.state === 'beta', payload.state === 'deprecated');
...
async function http_create(pg_pool, req, res /* exp */) {
  ...
  return httph.created_response(res,
    JSON.stringify(await create(
      pg_pool, payload.region ? payload.region.id : '',
      payload.name,
      payload.state === 'beta',
      payload.state === 'deprecated',
    )));
}
...
async function http_delete(pg_pool, req, res, exp) {
  ...
  const stack = await common.stack_exists(pg_pool, httph.first_match(req.url, exp));
  const deleted_stack = await delete_stack(pg_pool, [stack.stack]);
