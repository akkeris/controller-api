  ...
  try {
    ...
    if (task.length > 0) {
      ...
      switch(task.action) {
        case 'resync-addon-state':
          ...
          let app = await common.app_exists(app_uuid)
          ...
          if (addon.state === "provisioned" && !already_processed_resync_addons[addon_id + app_uuid]) {
            ...
            await update_task(pg_pool, ['finished', null, null, addon.state, null, new Date()]);
    ...
    if(task) {
      if (task.retries + 1 > 60) {
        ...
        await update_task(pg_pool, ['failed', task.retries + 1, "Error processing: " + err.toString(), null, null, null]);
      } else {
        await update_task(pg_pool, ['pending', task.retries + 1, "Error processing: " + err.toString(), null, null, null]);
      }
    }
