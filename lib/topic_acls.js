...
const select_acl_by_app_and_topic_and_role_and_consumergroup = query.bind(query, fs.readFileSync('./sql/select_topic_acl_by_app_and_topic_and_role_and_consumergroup.sql').toString('utf8'), n => n);
...
const delete_acls_by_app = query.bind(query, fs.readFileSync('./sql/delete_topic_acls_by_app.sql').toString('utf8'), n => n);
...
async function create(pg_pool, req, res, regex){
  ...
  let cg = payload.consumerGroupName;
  // Create in system
  let {id, consumerGroupName} = await common.alamo.topic_acls.create(region_name, cluster_name, topic_name, app_name, space_name, role, cg);

  // Create in DB
  let result = await insert_acl(pg_pool, [id, topic_uuid, app_uuid, role, consumerGroupName]);
...
async function  remove_consumer(pg_pool, req, res, regex){
  ...
  let {topic: topic_uuid} = await common.topic_exists(pg_pool, topic_key, cluster_key);
  let role = httph.fourth_match(req.url, regex);
  let consumer_group_name = httph.fifth_match(req.url, regex);
  let acl_id;
  ...
  if (/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/.test(acl_key)){
    ...
    let acls = await select_acl_by_app_and_topic_and_role_and_consumergroup(pg_pool, [app_uuid, topic_uuid, role, consumer_group_name]);
    if (!acls || acls.length === 0) {
      throw new httph.NotFoundError(`No ACL found for topic ${topic_key} and app ${acl_key} in role ${role}.`);
    }
    acl_id = acls[0].topic_acl;
...
async function handle_kafka_addon_delete(pg_pool, addon_change) {
  if(addon_change.change === 'delete' && addon_change.changes.length > 0) {
    for(var i in addon_change.changes) {
      let change = addon_change.changes[i]
