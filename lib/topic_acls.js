...
const select_acl_by_app_and_topic = query.bind(query, fs.readFileSync('./sql/select_topic_acl_by_app_and_topic.sql').toString('utf8'), n => n);       
...
async function remove(pg_pool, req, res, regex){
  ...
  if (/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/.test(acl_key)){
    ...
    let acls = await select_acl_by_app_and_topic(pg_pool, [app_uuid, topic_uuid]);
    if (!acls || acls.length == 0){
      throw new httph.NotFoundError('No such ACL');
    }
