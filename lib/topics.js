...
async function preview(pg_pool, req, res, regex) {
  ...
  //Check elevated access to preview topic  prod cluster or non qa,dev,test topic.
  if(cluster_name.toLowerCase().trim() === 'prod' || !/^(test\-|qa\-|dev\-)/.test(topic_key)) {
...
async function recreate(pg_pool, req, res, regex) {
  ...
  //check access
  if(cluster_name.toLowerCase().trim() === 'prod' || !/^(test\-|qa\-|dev\-)/.test(name)) {
    if(!(req.headers['x-elevated-access'] === 'true' && req.headers['x-username'])) {
      console.error(`User '${req.headers['x-username']}' Unauthorized to recreate topic '${name}' in cluster '${cluster_name}'`)
  ...
  console.log(`Topic '${name}' in cluster '${cluster_name}' recreate request by User '${req.headers['x-username']}'`)
