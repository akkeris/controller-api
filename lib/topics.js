...
async function create(pg_pool, req, res, regex) {
  ...
  let {cluster: cluster_id, topic_name_regex, name: cluster_name} = await common.cluster_exists(pg_pool, cluster_key);
  ...
  // TODO: change order so that region call goes first and then the config values (retention_ms, etc.) are taken from its result instead of from the config.
  let [inserted] = await insert_topic(pg_pool, [uuid.v4(), name, config_name, partitions, replicas, retention_ms, cleanup_policy, cluster_id, region_id, org, description]);
...
async function remove(pg_pool, req, res, regex){
  ...
  if(cluster_name == 'prod' || !/^(test\-|qa\-|dev\-)/.test(topic_key)) {
    if(!(req.headers['x-elevated-access'] === 'true' && req.headers['x-username'])) {
      console.error(`User '${req.headers['x-username']}' Unauthorized to delete topic '${topic_key}' in cluster '${cluster_name}'`)
  ...
  console.log(`Topic '${topic_key}' in cluster '${cluster_name}' deletion request by User '${req.headers['x-username']}'`)
