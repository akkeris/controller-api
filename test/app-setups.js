...
describe('app-setups:', function () {
  ...
  const appname = `alamotest${Math.floor(Math.random() * 100000)}`;
  let app_uuid = null; // eslint-disable-line
  ...
  it('ensure we can create an app from a definition', async () => {
    ...
    const data = JSON.parse(await httph.request(
      'post',
      'http://localhost:5000/app-setups',
      alamo_headers,
  ...
  it('ensure we can check the status of app creation', (done) => {
    const intv = setInterval(async () => {
      const data = JSON.parse(await httph.request(
        'get',
        `http://localhost:5000/app-setups/${app_setup_uuid}`,
        alamo_headers,
        null,
      ));
      expect(data).to.be.an('object');
  ...
  it('ensure the app setup starts the build', async () => {
    const builds = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${appname}-default/builds`,
      alamo_headers,
      null,
    ));
  ...
  it('Ensures we can place the application into maintenance mode.', async () => {
    const data = await httph.request(
      'patch',
      `http://localhost:5000/apps/${appname}-default`,
      alamo_headers,
  ...
  it('Ensures we can take the application out of maintenance mode.', async () => {
    const data = await httph.request(
      ...
      `http://localhost:5000/apps/${appname}-default`,
      alamo_headers,
      JSON.stringify({ maintenance: false }),
    );
    const appobj = JSON.parse(data);
    expect(appobj).to.be.an('object');
    expect(appobj.archived_at).to.be.a('string');
  ...
  it('ensure app setups created the config vars', async () => {
    const config_vars = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${appname}-default/config-vars`,
      alamo_headers,
      null,
    ));
  ...
  it('ensure app setups created the formation', async () => {
    const formation = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${appname}-default/formation`,
      alamo_headers,
      null,
    ));
  ...
  it('ensure we can get an app definition', async () => {
    const data = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${appname}-default/app-setups`,
      alamo_headers,
      null,
    ));
    expect(data).to.be.an('object');
