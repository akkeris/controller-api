...
describe("certificates: ensure we can pull and list certificates and orders", function() {  
  ...
  const alamo_headers = {"Authorization":process.env.AUTH_KEY, "User-Agent":"Hello", "x-username":"testuser@abcd.com", "x-elevated-access":"true"};
  ...
  it("covers not allowing names that are not alpha numeric", (done) => {
    httph.request('post', 'http://localhost:5000/ssl-orders', alamo_headers, JSON.stringify({name:"this is not valid.!"}), 
    (err, data) => {
  ...
  it("covers not allowing domain names that are not valid", (done) => {
    httph.request('post', 'http://localhost:5000/ssl-orders', alamo_headers, JSON.stringify({name:"fugazi", "common_name":"www.no t a domain.com"}), 
    (err, data) => {
        expect(err).to.be.an('object');
        expect(data).to.be.null;
        done();
  ...
  it("covers ensuring domain names contains common name.", (done) => {
    httph.request('post', 'http://localhost:5000/ssl-orders', 
      Object.assign({"x-username":"testuser@abcd.com"}, alamo_headers), 
      JSON.stringify({name:"fugazi", "common_name":"fugazi.abcd.io", "domain_names":[], "comments":"This is a test, if you find it, reject it.", "org":"test"}), 
    (err, data) => {
        expect(err).to.be.an('object');
  ...
  let short_name = 'fugazi' + Math.round(Math.random() * 10000000)
  it("partially covers creating certificates", (done) => {
    httph.request('post', 'http://localhost:5000/ssl-orders', 
      Object.assign({"x-username":"testuser@abcd.com"}, alamo_headers), 
      JSON.stringify({name:short_name, "common_name":"fugazi.abcd.io", "domain_names":["fugazi.abcd.io"], "comments":"This is a test, if you find it, reject it.", "org":"test"}), 
    (err, data) => {
      if(err) {
        console.error(err)
      }
  ...
  it("covers ensuring the list of ssl orders does not contain cert since its installed.", (done) => {
    ...
    (err, data) => {
      ...
      expect(obj).to.be.an('array');
      obj.forEach(validate_certificate)
      expect(obj.filter((d) => { return d.name === short_name}).length).to.equal(0)
      done();
    });
  });
  ...
  it("covers ensuring the list of ssl endpoints now contains this..", (done) => {
    ...
    (err, data) => {
      ...
      expect(obj).to.be.an('array');
      obj.forEach(validate_certificate)
      expect(obj.filter((d) => { return d.name === short_name}).length).to.equal(1)
      done();
    });
  });
});