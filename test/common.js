...
describe('base: Ensure helper tools work correctly.', () => {
  ...
  it('Ensures successful match.', (done) => {
    const match = httph.first_match('http://localhost:5000/apps/hello', new RegExp('/apps/([A-z0-9\\-\\_\\.]+)$'));
    expect(match).to.equal('hello');
    done();
  });
  it('Ensures spaces do not match.', (done) => {
    const match = httph.first_match('http://localhost:5000/apps/h ello', new RegExp('/apps/([A-z0-9\\-\\_\\.]+)$'));
  ...
  it('Ensure gzip compression works.', (done) => {
    httph.request('get', 'http://localhost:5000/apps', { 'accept-encoding': 'gzip', ...alamo_headers }, null,
      (err, data) => {
  ...
  it("Ensure we redact passwords in uri's.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com' });
    expect(res.SOME_URI).to.equal('https://foobar:[redacted]@host.com');
    done();
  });
  it("Ensure we redact passwords in uri's with ports.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com:1234' });
  ...
  it("Ensure we redact passwords in uri's with paths no ports.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo' });
  ...
  it("Ensure we redact passwords in uri's with paths no ports and args.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo?foo=bar' });
  ...
  it("Ensure we redact passwords in uri's where it may appear as a query param.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo?password=foobar' });
  ...
  it("Ensure we redact passwords in uri's where it may appear as a query param with multiple args, first.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo?password=foobar&justfine=true' });
  ...
  it("Ensure we redact passwords in uri's where it may appear as a query param with multiple args, not-first.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo?justfine=true&password=foobar' });
  ...
  it("Ensure we redact passwords in uri's where it may appear as a query param with multiple args, middle.", (done) => {
    const res = common.socs({ SOME_URI: 'https://foobar:fee@host.com/foo?justfine=true&password=foobar&alsofine=true' });
  ...
  it("Ensure we redact passwords in uri's with a wide variety of characters.", (done) => {
    const res = common.socs({
      SOME_URI: "https://foobar:1234567890QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm!#$%^&*()_+-={}|[]\\:\";'<>?,./@host.com/path?arg=arg3224&foo=bar",
    });
  ...
  it("Ensure we redact passwords in uri's with a wide variety of characters (with a port).", (done) => {
    const res = common.socs({
      SOME_URI: "https://foobar:1234567890QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm!#$%^&*()_+-={}|[]\\:\";'<>?,./@host.com:1235/path?arg=arg3224&foo=bar",
    });
  ...
  it("Ensure we redact passwords in uri's with a wide variety of characters (without a username).", (done) => {
    const res = common.socs({
      SOME_URI: "https://:1234567890QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm!#$%^&*()_+-={}|[]\\:\";'<>?,./@host.com/path?arg=arg3224&foo=bar",
    });
  ...
  it('Ensure we redact passwords where the key has a potentially sensitive name.', (done) => {
    const res = common.socs({
      ...
      SOME_KEY: 'should be also redacted',
      SOME_PASS: 'should be redacted?',
      SOME_PASSWORD: 'should be redacted',
      SOME_SECRET: 'should be redacted',
    });
