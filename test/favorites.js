...
describe('favorites: ensure we can create, list and delete favorites', function () {
  ...
  function headers(username) {
    const copy = JSON.parse(JSON.stringify(alamo_headers));
    copy['x-username'] = username;
    return copy;
  }

  it('covers listing favorites', (done) => {
    const u = `test${uuid.v4()}`;
    httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: app_id }, (err /* data */) => {
      expect(err).to.be.null;
      httph.request('get', 'http://localhost:5000/favorites', headers(u), null, (err2, data) => {
        ...
        httph.request('delete', `http://localhost:5000/favorites/${app_id}`, headers(u), null, (err3 /* data */) => {
  ...
  it('POST /favorites handles the case when the user adds the same favorite app twice', (done) => {
    ...
    httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: app_id }, (err /* data */) => {
      expect(err).to.be.null;
      httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: app_id }, (err2 /* data */) => {
        expect(err2).to.be.null;
        httph.request('get', 'http://localhost:5000/favorites', headers(u), null, (err3, data) => {
          expect(err3).to.be.null;
          expect(data).to.be.a('string');
          const obj = JSON.parse(data);
          expect(obj).to.be.an('array');
          expect(obj.length).equals(1);
          expect(obj[0].app).equals(app_id);
          httph.request('delete', `http://localhost:5000/favorites/${app_id}`, headers(u), null, (err4 /* data */) => {
  ...
  it('POST and DELETE to /favorites allow to pass the app name', (done) => {
    ...
    httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: 'api-default' }, (err /* data */) => {
      ...
      httph.request('get', 'http://localhost:5000/favorites', headers(u), null, (err2, data) => {
        ...
        expect(obj.length).equals(1);
        expect(obj[0].app).equals(app_id);
        httph.request('delete', 'http://localhost:5000/favorites/api-default', headers(u), null, (err3 /* data */) => {
          expect(err3).to.be.null;
          done();
        });
      });
  ...
  it('POST /favorites can add the same app after removal', (done) => {
    ...
    httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: app_id }, (err /* data */) => {
      expect(err).to.be.null;
      httph.request('delete', `http://localhost:5000/favorites/${app_id}`, headers(u), null, (err2 /* data */) => {
        expect(err2).to.be.null;
        httph.request('post', 'http://localhost:5000/favorites', headers(u), { app: app_id }, (err3, data) => {
          expect(err3).to.be.null;
          expect(data).to.be.a('string');
          const obj = JSON.parse(data);
