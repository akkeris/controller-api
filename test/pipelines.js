...
describe("pipelines", function() {
  ...
  it("covers getting pipeline stages", async () => {
    let data = JSON.parse(await httph.request('get', 'http://localhost:5000/pipeline-stages', init.alamo_headers, null));
  ...
  it("covers creating pipline", async () => {
    ...
    let data = JSON.parse(await httph.request('post','http://localhost:5000/pipelines', init.alamo_headers, JSON.stringify({name:'test-pipeline'})));
  ...
  it("ensure non-existant pipeline couplings return a 404.", async () => {
    try {
      await httph.request('get', 'http://localhost:5000/pipelines/non-existant-pipeline/pipeline-couplings', {"x-silent-error":true, ...init.alamo_headers}, null);
  ...
  it("ensure pipeline couplings return a blank array if pipeline exists without couplings.", async () => {
    let couplings = await JSON.parse(await httph.request('get', 'http://localhost:5000/pipelines/test-pipeline/pipeline-couplings', init.alamo_headers, null));
    expect(couplings).to.be.an('array');
    expect(couplings.length).to.equal(0);
  });
  
  it("covers getting pipline", async () => {
    let data = JSON.parse(await httph.request('get','http://localhost:5000/pipelines/test-pipeline', init.alamo_headers, null));
    expect(data.name).to.be.a('string');
    expect(data.id).to.be.a('string');
    expect(data.created_at).to.be.a('string');
    expect(data.updated_at).to.be.a('string');
  ...
  it("covers listing piplines", async () => {
    let data = JSON.parse(await httph.request('get','http://localhost:5000/pipelines', init.alamo_headers, null));
  ...
  it("covers not allowing duplicate piplines", async () => {
    try {
      await httph.request('post','http://localhost:5000/pipelines', {"x-silent-error":true, ...init.alamo_headers}, JSON.stringify({name:'test-pipeline'}));
    } catch (e) {
  ...
  it("creating multiple apps for pipelining (1).", async () => {
    ...
    let build_obj = JSON.parse(await httph.request('post', 'http://localhost:5000/apps/' + app1 + '-pipline-test-space1/builds', init.alamo_headers, JSON.stringify(pipeline1_sources)));
    expect(build_obj.id).to.be.a('string');
    // wait for the build to succeed
    await init.wait_for_app_content(`${app1}-pipline-test-space1`, 'pipeline1');
    let release_obj = await init.latest_release(`${app1}-pipline-test-space1`);
    expect(release_obj.id).to.be.a('string');
    pipeline1_release_id = release_obj.id;
  });

  it("creating multiple apps for pipelining (2).", async () => {
    await httph.request('post', 'http://localhost:5000/apps', init.alamo_headers, JSON.stringify({org:"test", space:'pipline-test-space1', name:app2}));
  });

  it("creating multiple apps for pipelining (3).", async () => {
    await httph.request('post', 'http://localhost:5000/apps', init.alamo_headers, JSON.stringify({org:"test", space:'pipline-test-space2', name:app3}));
  });

  it("creating multiple apps for pipelining (4).", async () => {
    await httph.request('post', 'http://localhost:5000/apps', init.alamo_headers, JSON.stringify({org:"test", space:'pipline-test-space3', name:app4}));
  });
  ...
  it("ensure couplings are present in list.", async () => {
    let data = JSON.parse(await httph.request('get', 'http://localhost:5000/pipelines/test-pipeline/pipeline-couplings', init.alamo_headers, null));
  ...
  it("covers promoting one app to another in the same space.", async () => {
    ...
    let data = JSON.parse(await httph.request('post','http://localhost:5000/pipeline-promotions', init.alamo_headers,
  ...
  it("covers promoting one app to multiple apps.", async () => {
    ...
    let config_vars = JSON.parse(await httph.request('patch', `http://localhost:5000/apps/${app4}-pipline-test-space3/config-vars`, init.alamo_headers, JSON.stringify({FOO:"BAR"})));
  ...
  it("covers pulling promotions", async () => {
    let data = JSON.parse(await httph.request('get', 'http://localhost:5000/pipeline-promotions', init.alamo_headers, null));
    let found = false;
  ...
  it("covers pulling promotion targets", async () => {
    expect(pipeline_promotion_id).to.be.a('string');
    await httph.request('get', `http://localhost:5000/pipeline-promotions/${pipeline_promotion_id}/promotion-targets`, init.alamo_headers, null);
  });
  ...
  it("creating a new build on the original source app.", async () => {
    ...
    let build_obj = JSON.parse(await httph.request('post', `http://localhost:5000/apps/${app1}-pipline-test-space1/builds`, init.alamo_headers, JSON.stringify(pipeline2_sources)));
    expect(build_obj.id).to.be.a('string');
  ...
  it("covers ensuring safe promotions halt if source config is different than dest config and its a safe promotion.", async () => {
    ...
    await httph.request('patch', `http://localhost:5000/apps/${app1_id}/config-vars`, init.alamo_headers, JSON.stringify({"TYPEAHEAD":"WHONEEDSIT?"}));
    try {
      await httph.request('post','http://localhost:5000/pipeline-promotions', {"x-silent-error":true, ...init.alamo_headers}, 
        JSON.stringify({safe:true, pipeline:{id:pipeline_id}, source:{app:{id:app1_id}}, targets:[{app:{id:app2_id}}]}))
    } catch (err) {
  ...
  it("covers ensuring promotions do not halt if source config is different than dest config and its an unsafe promotion.", async () => {
    ...
    expect(app1_id).to.be.a('string');
    expect(app2_id).to.be.a('string');
    await httph.request('post','http://localhost:5000/pipeline-promotions', init.alamo_headers, JSON.stringify({pipeline:{id:pipeline_id}, source:{app:{id:app1_id}}, targets:[{app:{id:app2_id}}]}));
  });
  ...
  it("covers ensuring promotions do not halt if config is same and its unsafe.", async () => {
    ...
    await httph.request('patch', `http://localhost:5000/apps/${app1_id}/config-vars`, init.alamo_headers, JSON.stringify({"TYPEAHEAD":null}));
    await httph.request('post','http://localhost:5000/pipeline-promotions', init.alamo_headers, JSON.stringify({pipeline:{id:pipeline_id}, source:{app:{id:app1_id}}, targets:[{app:{id:app2_id}}]}));
  });
  ...
  it("covers ensuring safe promotions halt if destination config is different then source config and its a safe promotion.", async () => {
    ...
    await httph.request('patch', `http://localhost:5000/apps/${app2_id}/config-vars`, init.alamo_headers, JSON.stringify({"TYPEAHEAD2":"WHONEEDSIT?"}));
    try {
      await httph.request('post','http://localhost:5000/pipeline-promotions', {"x-silent-error":true, ...init.alamo_headers}, 
        JSON.stringify({safe:true, pipeline:{id:pipeline_id}, source:{app:{id:app1_id}}, targets:[{app:{id:app2_id}}]}));
    } catch (err) {
  ...
  it("covers ensuring deleting pipeline deletes pipeline couplings.", async () => {
    try {
      await httph.request('get', `http://localhost:5000/pipeline-couplings/${app2_coupling_id}`, {"x-silent-error":true, ...init.alamo_headers}, null);
    } catch (err) {
      expect(err.code).to.equal(404)
    }
  });
