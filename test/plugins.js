...
describe('plugins: creating, listing, updating deleting', function () {
  ...
  it('covers creating plugins', (done) => {
    httph.request('post', 'http://localhost:5000/plugins', alamo_headers, JSON.stringify({
      name: 'testing', repo: 'https://foo.com', owner: 'owner', email: 'email@email.com', description: 'description',
    }),
    (err, data) => {
  ...
  it('covers updating plugins', (done) => {
    httph.request('patch', 'http://localhost:5000/plugins/testing', alamo_headers, JSON.stringify({
      repo: 'https://foo2.com', owner: 'owner2', email: 'email2@email.com', description: 'description2',
    }),
    (err, data) => {
      expect(err).to.be.null;
      expect(data).to.be.a('string');
