...
describe('preview apps: ensure preview apps work appropriately', function () {
  ...
  // const webhook_pr_syncronize = JSON.parse(
  //   fs.readFileSync('./test/support/github-webhook-pr-syncronize.json').toString('utf8'),
  // );
  // const webhook_pr_closed = JSON.parse(
  //   fs.readFileSync('./test/support/github-webhook-pr-closed.json').toString('utf8'),
  // );
  // // commit shows on refs/master or destination branch.
  // const webhook_pr_push_when_closed = JSON.parse(
  //   fs.readFileSync('./test/support/github-webhook-pr-push-when-closed.json').toString('utf8'),
  // );
  ...
  if (process.env.SMOKE_TESTS) {
    it('setup dummy app', async () => {
      const req_data = JSON.stringify({
        org: 'test', space: 'preview', name: app_dummy_name, size: 'gp1', quantity: 1, type: 'web', port: 9000,
      });
      await httph.request('post', 'http://localhost:5000/apps', alamo_headers, req_data);
      addon_attach = JSON.parse(await httph.request(
        'post',
        `http://localhost:5000/apps/${app_dummy_name}-preview/addons`,
        alamo_headers,
        JSON.stringify({ plan: 'amazon-s3:basic' }),
      ));
    });
  }
  ...
  it('setup an auto build', async () => {
    const req_data = JSON.stringify({
      repo: 'https://github.com/akkeris/preview-app-test-repo',
      branch: 'master',
      status_check: 'true',
      auto_deploy: 'true',
      username: 'test',
    ...
    let data = await httph.request(
      'post',
      `http://localhost:5000/apps/${app_name}-preview/builds/auto`,
      alamo_headers,
      req_data,
    );
  ...
  it('setup auto releases', async () => {
    await httph.request(
      'patch',
      `http://localhost:5000/apps/${app_name}-preview/features/auto-release`,
      alamo_headers,
      { enabled: true },
      null,
    );
  ...
  it('setup a build and release', async function () {
    ...
    const build_info_new = JSON.parse(await httph.request(
      'post',
      `http://localhost:5000/apps/${app_name}-preview/builds`,
      alamo_headers,
      JSON.stringify(build_payload),
    ));
    await init.wait_for_build(`${app_name}-preview`, build_info_new.id);
  ...
  it('setup the formation', async () => {
    const data = JSON.parse(await httph.request(
      'patch',
      `http://localhost:5000/apps/${app_name}-preview/formation`,
      alamo_headers,
      JSON.stringify([{
        type: 'web',
        command: 'what',
  ...
  it('setup a sharable (and attached) addon', async () => {
    ...
    if (process.env.SMOKE_TESTS) {
      await httph.request(
        ...
        alamo_headers,
        JSON.stringify({
          app: `${app_name}-preview`,
          addon: addon_attach.id,
        }),
      );
    }
  });
  ...
  it('setup an a non-sharable addon (non vault based)', async () => {
    await httph.request(
      ...
      `http://localhost:5000/apps/${app_name}-preview/addons`,
      alamo_headers,
      JSON.stringify({ plan: 'papertrail:basic' }),
    );
    const drains = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${app_name}-preview/log-drains`,
      alamo_headers,
      null,
    ));
  ...
  it('setup an auth filter)', async () => {
    ...
    const testapp_filter = JSON.parse(await httph.request(
      'post',
    ...
    const testapp_filter_attachment = JSON.parse(await httph.request(
      'post',
      `http://localhost:5000/apps/${app_name}-preview/filters`,
      alamo_headers,
      attachmentPayload,
    ));
  ...
  it('ensure a PR request with feature disabled does not create a preview app', async () => {
    const incoming = JSON.stringify(webhook_pr_opened);
    ...
    const data = await httph.request(
      'post',
      `http://localhost:5000/apps/${app_name}-preview/builds/auto/github`,
      headers,
      incoming,
    );
    expect(data.toString('utf8')).to.equal('{"code":201,"message":"Roger that, message received."}');
    let previews = await httph.request(
      'get',
      `http://localhost:5000/apps/${app_name}-preview/previews`,
      alamo_headers,
      null,
    );
  ...
  it('ensure we can enable preview apps', async () => {
    let result = await httph.request(
      'patch',
      `http://localhost:5000/apps/${app_name}-preview/features/preview`,
      alamo_headers,
      { enabled: true },
      null,
    );
    result = JSON.parse(result);
    let result_sites = await httph.request(
      'patch',
      `http://localhost:5000/apps/${app_name}-preview/features/preview-sites`,
      alamo_headers,
      { enabled: false },
      null,
    );
  ...
  it('ensure a PR request targeted at the wrong destination branch does not cause a preview app', async () => {
    const incoming = JSON.stringify(webhook_pr_opened_to_wrong_branch);
    const hash = git.calculate_hash('testing', incoming);
    const headers = { 'x-github-event': 'pull_request', 'x-hub-signature': hash };
    const data = await httph.request(
      'post',
  ...
  it('ensure a PR request updated at the wrong destination branch does not cause a preview app', async () => {
    const incoming = JSON.stringify(webhook_pr_push_before_pr);
    const hash = git.calculate_hash('testing', incoming);
  ...
  it('ensure a PR request from an outside repository does not create a preview app', async () => {
    const incoming = JSON.stringify(webhook_pr_fail_public_fork);
    const hash = git.calculate_hash('testing', incoming);
    const headers = { 'x-github-event': 'pull_request', 'x-hub-signature': hash };
    const data = await httph.request(
      'post',
  ...
  it('ensure new preview app received config vars correctly', async () => {
    for (let i = 0; i < 20; i++) {
      ...
      const config_vars = JSON.parse(await httph.request(
        'get',
        `http://localhost:5000/apps/${preview_app.app.id}/config-vars`,
        alamo_headers,
        null,
      ));
      if (config_vars.FOO) {
  ...
  it('ensure quantity on formation is 1, but all other settings are the same', async () => {
    const data = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/formation`,
      alamo_headers,
      null,
    ));
    expect(data.length).to.equal(1);
  ...
  it('ensure new preview app received new addon that could not be shared', async () => {
    for (let i = 0; i < 10; i++) {
      // eslint-disable-next-line no-await-in-loop
      const preview_addons = JSON.parse(await httph.request(
        'get',
        `http://localhost:5000/apps/${preview_app.app.id}/addons`,
        alamo_headers,
        null,
      ));
      if (preview_addons.length === 1) {
        // eslint-disable-next-line no-await-in-loop
        const source_addons = JSON.parse(await httph.request(
          'get',
          `http://localhost:5000/apps/${app_name}-preview/addons`,
          alamo_headers,
          null,
        ...
        const source_addon = source_addons.filter((x) => x.addon_service.name === 'papertrail');
  ...
  it('ensure they both have the same drain end point', async () => {
    ...
    expect(drains).to.be.an('array');
    expect(drains.length).to.equal(1);
    const preview_drains = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/log-drains`,
      alamo_headers,
      null,
    ));
  ...
  it('ensure new preview app attaches sharable addons, rather than creating a new one', async () => {
    let attachments = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/addon-attachments`,
      alamo_headers,
      null,
    ));
    let addons = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${app_name}-preview/addons`,
      alamo_headers,
      null,
  ...
  it('ensure new preview app has auto build setup on PR branch', async () => {
    const auto_build = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/builds/auto/github`,
      alamo_headers,
      null,
    ));
  ...
  it('ensure a push to the existing PR causes a new build on the preview app (not the source app)', async () => {
    ...
    const data = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/builds`,
      alamo_headers,
      null,
    ));
    expect(data.length).to.equal(1);
    ...
    // Send second hook
    const incoming = JSON.stringify(webhook_pr_second_push_on_pr);
    const hash = git.calculate_hash('testing', incoming);
    const headers = { 'x-github-event': 'push', 'x-hub-signature': hash };
    const datam = await httph.request(
      'post',
      `http://localhost:5000/apps/${app_name}-preview/builds/auto/github`,
      headers,
      incoming,
    ...
    const builds2 = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/builds`,
      alamo_headers,
      null,
    ...
    const builds3 = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${app_name}-preview/builds`,
      alamo_headers,
      null,
  ...
  it('ensure we can get a preview from a source application', async () => {
    const preview = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${app_name}-preview/previews/${preview_id}`,
      alamo_headers,
      null,
    ));
  ...
  it('ensure a push to the existing PR creates build after preview apps were disabled.', async () => {
    ...
    const builds = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${preview_app.app.id}/builds`,
      alamo_headers,
      null,
  ...
  const prod_app_name = `${app_name.substring(0, app_name.length - 1)}p`;
  it('setup a prod-space app', async () => {
    const req_data = JSON.stringify({
      org: 'test', space: 'default', name: prod_app_name, size: 'gp2', quantity: 1, type: 'web', port: 9000,
    });
    await httph.request('post', 'http://localhost:5000/apps', alamo_headers, req_data);
    ...
    await httph.request(
      'patch',
      `http://localhost:5000/apps/${prod_app_name}-default/features/preview`,
      alamo_headers,
      { enabled: true },
      null,
    );
  ...
  it('ensure a PR request doesnt create a preview app on a prod space app', async () => {
    ...
    const data = await httph.request(
      'post',
      `http://localhost:5000/apps/${prod_app_name}-default/builds/auto/github`,
      headers,
      incoming,
    );
    expect(data.toString('utf8')).to.equal('{"code":201,"message":"Roger that, message received."}');
    let previews = await httph.request(
      'get',
      `http://localhost:5000/apps/${prod_app_name}-default/previews`,
      alamo_headers,
      null,
    );
    previews = JSON.parse(previews.toString());
  ...
  it('ensure we clean up after ourselves (fail-safe)', async () => {
    const apps = JSON.parse(await httph.request('get', 'http://localhost:5000/apps', alamo_headers, null));
    await Promise.all(apps.filter((x) => x.space.name === 'preview' && x.organization.name === 'test').map(async (app) => {
      try {
