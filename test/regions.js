...
describe('regions: list and get available regions', function () {
  ...
  const validate_us_region = function (obj) {
  ...
  it('covers ensuring regions cannot be updated unless its an elevated access request', (done) => {
    httph.request(
      'patch',
      `http://localhost:5000/regions/${process.env.TEST_REGION}`,
      user_alamo_headers,
  ...
  it('covers requiring name on creation', (done) => {
    const payload = {
      ...
      country: 'country',
      locale: 'locale',
      provider: { name: 'provider_name', region: 'provider_region', availability_zones: ['zone1', 'zone2'] },
