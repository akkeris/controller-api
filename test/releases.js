...
describe('releases: list, get, create a release', function () {
  ...
  it('covers parsing command args from end point', (done) => {
    ...
    const args = c.alamo.parse_command_args('node --foo=23423 --asdfasd="asdfasdfasdfasd" -d  "asdf asdf asdf" abcd');
  ...
  it('covers creating a build to release', async () => {
    ...
    test_build = await support.create_build(
      test_app,
      'docker://docker.io/akkeris/test-hooks:latest',
      null,
    ...
    await support.wait_for_build(test_app, test_build);
  });
  ...
  it('covers creating a release from the build', async () => {
    ...
    const release_info = JSON.parse(await httph.request(
      'post',
      `http://localhost:5000/apps/${test_app.id}/releases`,
      support.alamo_headers,
      JSON.stringify({ slug: test_build.id, description: `Deploy ${test_build.id}` }),
    ));
  ...
  it('covers getting release info', async () => {
    const objs = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${test_app.id}/releases`,
      support.alamo_headers,
      null,
    ...
    validate_release_object(JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${test_app.id}/releases/${objs[0].id}`,
      support.alamo_headers,
      null,
    )));
  });
  ...
  it('covers audit events for a feature', async () => {
    const obj = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/audits?app=${test_app.simple_name}&space=${test_app.space.name}`,
      support.alamo_headers,
      null,
    ));
    expect(obj).to.be.an('array');
  ...
  it('covers ensuring release causes an app at expected host to turn up', async () => {
    ...
    await support.wait(1000);
    const resp = await support.wait_for_app_content(test_app.web_url, 'hello');
  ...
  it('ensure we can rollback the app', async () => {
    ...
    validate_release_object(JSON.parse(await httph.request(
      ...
      `http://localhost:5000/apps/${test_app.id}/releases`,
      support.alamo_headers,
      JSON.stringify({ release: release_id }),
    )));
  });
  ...
  it('ensure we get a reasonable error with an invalid release id on rollback of an app', async () => {
    ...
    try {
      await httph.request(
        'post',
        `http://localhost:5000/apps/${test_app.id}/releases`,
        { 'x-silent-error': true, ...support.alamo_headers },
        JSON.stringify({ release: '12345' }),
      );
  ...
  it('expect the image property to be available', async () => {
    const app = await support.get_app(test_app);
  ...
  it('ensure the release statuses is an empty array.', async () => {
    const statuses = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${test_app.id}/releases/${release_id}/statuses`,
      support.alamo_headers,
    ));
  ...
  it('ensure we can create a release status.', async () => {
    const status = JSON.parse(await httph.request(
      'post',
      `http://localhost:5000/apps/${test_app.id}/releases/${release_id}/statuses`,
      support.alamo_headers, {
        state: 'pending',
  ...
  it('ensure we cannot create invalid statuses', async () => {
    try {
      await httph.request(
        'post', `http://localhost:5000/apps/${test_app.id}/releases/${release_id}/statuses`,
        { 'x-silent-error': true, ...support.alamo_headers },
        {
          state: 'foobar',
          name: 'status123',
  ...
  it('ensure we can get a release status.', async () => {
    ...
    const status = JSON.parse(await httph.request(
      'get',
      `http://localhost:5000/apps/${test_app.id}/releases/${release_id}/statuses/${release_status_id}`,
      support.alamo_headers,
      null,
    ));
    validate_release_status_object(status);
