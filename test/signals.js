...
describe('signals and executing debug commands', function () {
  ...
  it('setup: provision resources', async () => {
    ...
    const build = await test.create_build(
      signalsapp,
      'docker://docker.io/akkeris/test-signals:latest',
      null,
  ...
  it('test no signals have been triggered', async () => {
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test that SIGHUP is sent and triggered', async () => {
    await request(
      'post',
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -1 -1'], stdin: '' }),
    );
  ...
  it('test that SIGINT is sent and triggered', async () => {
    await request(
      ...
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -2 -1'], stdin: '' }),
    );
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test that SIGTERM is sent and triggered', async () => {
    await request(
      ...
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -15 -1'], stdin: '' }),
    );
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test that SIGQUIT is sent and triggered', async () => {
    await request(
      ...
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -3 -1'], stdin: '' }),
    );
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test that SIGABRT is sent and triggered', async () => {
    await request(
      ...
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -6 -1'], stdin: '' }),
    );
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test that SIGABRT is sent and triggered', async () => {
    await request(
      ...
      `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
      test.alamo_headers,
      JSON.stringify({ command: ['sh', '-c', 'kill -10 -1'], stdin: '' }),
    );
    const signals = JSON.parse(await request('get', signalsapp.web_url, {}, null));
  ...
  it('test to ensure signals cannot be sent to unknown dynos', async () => {
    try {
      await request(
        'post',
        `http://localhost:5000/apps/${signalsapp.id}/dynos/does-not-exist/actions/attach`,
        { 'x-silent-error': 'true', ...test.alamo_headers },
        JSON.stringify({ command: ['sh', '-c', 'kill -10 -1'], stdin: '' }),
      );
  ...
  it('test to ensure commands that are not allowed cannot be sent.', async () => {
    try {
      await request(
        ...
        `http://localhost:5000/apps/${signalsapp.id}/dynos/web.${dynos[0].name}/actions/attach`,
        { 'x-silent-error': 'true', ...test.alamo_headers },
        JSON.stringify({ command: ['ssh', '-1', '-1'], stdin: '' }),
      );
      expect(false).to.equal(true);
    } catch (e) {
