...
describe("stacks: list and get available stacks", function() {
  ...
  it("covers ensuring stacks cannot be updated unless its an elevated access request", (done) => {
    httph.request('patch', 'http://localhost:5000/stacks/' + default_stack_name, user_alamo_headers, {"name":"should-not-happen"}, (err, data) => {
      expect(err).to.be.an('object');
      expect(err.code).to.equal(403);
      expect(data).to.be.null;
      done();
  ...
  it("covers requiring name on creation", (done) => {
    let payload = {"region":{"id":"fugazi"}, "beta":false, "deprecated":false}
  ...
  it("covers requiring valid name on creation", (done) => {
    let payload = {"name":"fubar!", "region":{"id":"foo"}}
    httph.request('post', 'http://localhost:5000/stacks', alamo_headers, JSON.stringify(payload), (err, data) => {
      expect(err).to.be.an('object');
      expect(err.code).to.equal(422)
  ...
  it("covers requiring valid region on stack creation", (done) => {
    let payload = {"name":"fugazi", "beta":false, "deprecated":false}
    httph.request('post', 'http://localhost:5000/stacks', alamo_headers, JSON.stringify(payload), (err, data) => {
      expect(err).to.be.an('object');
      expect(err.code).to.equal(422)
  ...
  it("covers creating a region (dependency)", (done) => {
    let payload = {"name":"fubar", "description":"description","country":"country","locale":"locale","provider":{"name":"provider_name","region":"provider_region","availability_zones":["zone1","zone2"]},"high_availability":false, "private_capable":false}
  ...
  it("covers creating a stack", (done) => {
    ...
    let payload = {"name":"fugazi", "region":{"id":region_id}, "state":"beta"}
    httph.request('post', 'http://localhost:5000/stacks', alamo_headers, JSON.stringify(payload), (err, data) => {
      if(err) {
        console.error(err)
      }
  ...
  it("covers not being able to create a prod space in a beta stack", (done) => {
    httph.request('post', 'http://localhost:5000/spaces', alamo_headers, JSON.stringify({name:"fugazi123", description:"FFFUUUUGGGAAZZIII!!!", "stack":"fugazi", "compliance":["prod"]}), 
    (err, data) => {
      expect(err).to.be.an('object');
      expect(data).to.be.null;
      expect(err.code).to.equal(409)
  ...
  it("covers not being able to create an internal space in a region not capable of private spaces", (done) => {
    httph.request('post', 'http://localhost:5000/spaces', alamo_headers, JSON.stringify({name:"fugazi124", description:"FFFUUUUGGGAAZZIII!!!", "stack":"fugazi", "compliance":["internal"]}), 
    (err, data) => {
      expect(err).to.be.an('object');
      expect(data).to.be.null;
      expect(err.code).to.equal(409)
