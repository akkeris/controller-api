...
describe('vault: provisioning, etc', function () {
  ...
  it('covers creating a vault service', (done) => {
    ...
    httph.request(
      ...
      alamo_headers,
      JSON.stringify({ plan: vault_plan.id }),
      (err, data) => {
        expect(err).to.be.null;
        expect(data).to.be.a('string');
        const obj = JSON.parse(data);
  ...
  it('covers ensuring an addon marked as unable to be attached twice, wont be attached', (done) => {
    ...
    httph.request(
      ...
      `http://localhost:5000/apps/${appname_brand_new}-default/addons`,
      alamo_headers,
      JSON.stringify({ plan: vault_qa_plan.id }),
      (err, data) => {
  ...
  it('covers ensuring a vault service marked prod cannot be attached to a non-prod space', (done) => {
    ...
    httph.request(
      ...
      `http://localhost:5000/apps/${appname_brand_new}-default/addons`,
      alamo_headers,
      JSON.stringify({ plan: vault_prod_plan.id }),
      (err, data) => {
  ...
  it('covers getting info on a running vault service', (done) => {
    ...
    httph.request(
      'get',
      `http://localhost:5000/apps/${appname_brand_new}-default/addons/${vault_response.id}`,
      alamo_headers,
      null,
      (err, data) => {
        expect(err).to.be.null;
